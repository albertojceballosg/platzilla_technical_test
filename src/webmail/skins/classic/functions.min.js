...
function rcube_init_tabs(a,b){var c=$("#"+a),d=c.children("fieldset");if(d.length){b=b?b:0;d.each(function(a){a!=b&&$(this).hide()});var f=$("<div>").addClass("tabsbar").appendTo(c);d.each(function(c){var d,e,k=$(this),h=k.children("legend");e=$("<a>").text(h.text()).attr("href","#");d=$("<span>").attr({id:"tab"+c,"class":"tablink"}).click(function(){rcube_show_tab(a,c);return!1});h.remove();k.addClass("tabbed");c==b&&d.addClass("tablink-selected");d.append(e).appendTo(f)})}}
...
a),f=d.is(":visible")?-1:1,e=24+($("select",d).length?24:0);d.slideToggle({duration:160,progress:function(b,d){0>f&&(d=1-d);$(".boxlistcontent",a).css("top",c.outerHeight()+e*d+"px")},complete:function(){d.toggleClass("expanded");d.is(":visible")?d.find("input[type=text]").focus():$("a.reset",d).click()}});return!1})}};
function rcube_layer(a,b){this.name=a;this.create=function(a){var b=a.x?a.x:0,c=a.y?a.y:0,e=a.width,g=a.height,l=a.zindex,k=a.vis;a=a.parent;var h=document.createElement("DIV");h.id=this.name;h.style.position="absolute";h.style.visibility=k?2==k?"inherit":"visible":"hidden";h.style.left=b+"px";h.style.top=c+"px";e&&(h.style.width=e.toString().match(/\%$/)?e:e+"px");g&&(h.style.height=g.toString().match(/\%$/)?g:g+"px");l&&(h.style.zIndex=l);a?a.appendChild(h):document.body.appendChild(h);this.elm=

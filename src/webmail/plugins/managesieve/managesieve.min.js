...
rcmail.register_command("plugin.managesieve-setact",function(){rcmail.managesieve_setact()});rcmail.register_command("plugin.managesieve-setget",function(){rcmail.managesieve_setget()});rcmail.env.action.startsWith("plugin.managesieve")&&(rcmail.gui_objects.sieveform?(rcmail.enable_command("plugin.managesieve-save",!0),sieve_form_init()):rcmail.enable_command("plugin.managesieve-add","plugin.managesieve-setadd",!rcmail.env.sieveconnerror),a=rcmail.env.currentset,rcmail.gui_objects.filterslist&&(rcmail.filters_list=
new rcube_list_widget(rcmail.gui_objects.filterslist,{multiselect:!1,draggable:!0,keyboard:!0}),rcmail.filters_list.addEventListener("select",function(a){rcmail.managesieve_select(a)}).addEventListener("dragstart",function(a){rcmail.managesieve_dragstart(a)}).addEventListener("dragend",function(a){rcmail.managesieve_dragend(a)}).addEventListener("initrow",function(a){a.obj.onmouseover=function(){rcmail.managesieve_focus_filter(a)};a.obj.onmouseout=function(){rcmail.managesieve_unfocus_filter(a)}}).init()),
rcmail.gui_objects.filtersetslist&&(rcmail.filtersets_list=new rcube_list_widget(rcmail.gui_objects.filtersetslist,{multiselect:!1,draggable:!1,keyboard:!0}),rcmail.filtersets_list.init().focus(),null!=a&&(a=rcmail.managesieve_setid(a),rcmail.filtersets_list.select(a)),rcmail.filtersets_list.addEventListener("select",function(a){rcmail.managesieve_setselect(a)}),a=rcmail.filtersets_list.rowcount,rcmail.enable_command("plugin.managesieve-set",!0),rcmail.enable_command("plugin.managesieve-setact","plugin.managesieve-setget",
a),rcmail.enable_command("plugin.managesieve-setdel",1<a),$("tr",rcmail.gui_objects.filtersetslist).each(function(a,c){rcmail.managesieve_fixdragend(c)})));rcmail.gui_objects.sieveform&&rcmail.env.rule_disabled&&$("#disabled").attr("checked",!0)});rcube_webmail.prototype.managesieve_add=function(){this.load_managesieveframe();this.filters_list.clear_selection()};
rcube_webmail.prototype.managesieve_del=function(){var a=this.filters_list.get_single_selection();if(confirm(this.get_label("managesieve.filterdeleteconfirm"))){var b=this.set_busy(!0,"loading");this.http_post("plugin.managesieve-action","_act=delete&_fid="+this.filters_list.rows[a].uid,b)}};
...
rcube_webmail.prototype.managesieve_setid=function(a){for(var b in this.env.filtersets)if(this.env.filtersets[b]==a)return b};rcube_webmail.prototype.managesieve_list=function(a){var b=this.set_busy(!0,"loading");this.http_post("plugin.managesieve-action","_act=list&_set="+urlencode(a),b)};rcube_webmail.prototype.managesieve_setget=function(){var a=this.filtersets_list.get_single_selection();this.goto_url("plugin.managesieve-action",{_act:"setget",_set:this.env.filtersets[a]},!1,!0)};
...
rcube_webmail.prototype.managesieve_updatelist=function(a,b){this.set_busy(!0);switch(a){case "del":var c=b.id,d=this.filters_list;d.remove_row(this.managesieve_rowid(b.id));d.clear_selection();this.show_contentframe(!1);this.enable_command("plugin.managesieve-del","plugin.managesieve-act",!1);$("tr",this.filters_list.list).each(function(){if("none"==this.style.display)$(this).detach();else{var a=this.id.substr(6);$(this).off();a>c&&(this.uid=a-1,$(this).attr("id","rcmrow"+this.uid))}});d.init();
break;case "update":var e;e=$("#rcmrow"+this.managesieve_rowid(b.id));b.name&&$("td",e).text(b.name);b.disabled?e.addClass("disabled"):e.removeClass("disabled");$("#disabled",$("iframe").contents()).prop("checked",b.disabled);break;case "add":d=this.filters_list;e=$('<tr><td class="name"></td></tr>');$("td",e).text(b.name);e.attr("id","rcmrow"+b.id);b.disabled&&e.addClass("disabled");d.insert_row(e.get(0));d.highlight_row(b.id);this.enable_command("plugin.managesieve-del","plugin.managesieve-act",
!0);break;case "list":var f,l,g,d=this.filters_list;b.clear&&d.clear();for(e in b.list)g=b.list[e],f=document.createElement("TR"),l=document.createElement("TD"),$(l).text(g.name),l.className="name",f.id="rcmrow"+g.id,g["class"]&&(f.className=g["class"]),f.appendChild(l),d.insert_row(f);b.set?d.highlight_row(b.set):this.enable_command("plugin.managesieve-del","plugin.managesieve-act",!1);break;case "setact":c=this.managesieve_setid(b.name);e=$("#rcmrow"+c);b.active?(b.all&&$("tr",this.gui_objects.filtersetslist).addClass("disabled"),
...
this.env.filtersets[c]=b.name,d.insert_row(e.get(0)),b.index!=d.rowcount-1&&(e.detach(),f=$("tr:visible",d.list).get(b.index),e.insertBefore(f)),d.select(c),this.managesieve_fixdragend(e)}this.set_busy(!1)};
...
function rule_header_select(a){var b=document.getElementById("header"+a),c=document.getElementById("rule_size"+a),d=document.getElementById("rule_op"+a),e=document.getElementById("custom_header"+a+"_list"),f=document.getElementById("rule_mod"+a),l=document.getElementById("rule_trans"+a),g=document.getElementById("rule_comp"+a),m=document.getElementById("rule_date_part"+a),n=document.getElementById("rule_date_header_div"+a),k=b.value;"size"==k?(c.style.display="inline",$.each([d,e,f,l,g],function(){this.style.display=
"none"})):(e.style.display="..."!=k?"none":"inline-block",c.style.display="none",d.style.display="inline",g.style.display="",f.style.display="body"==k||"currentdate"==k||"date"==k?"none":"block",l.style.display="body"==k?"block":"none");m&&(m.style.display="currentdate"==k||"date"==k?"inline":"none");n&&(n.style.display="date"==k?"":"none");rule_op_select(d,a,k);rule_mod_select(a,k);b.style.width="..."==k?"40px":""}
...
rcube_webmail.prototype.managesieve_tip_register=function(a){var b,c=parent.rcmail,d=c?parent.rcmail.env.ms_tip_layer:rcmail.env.ms_tip_layer;for(b in a)$("#"+a[b][0]).data("tip",a[b][1]).mouseleave(function(a){d.hide()}).mouseenter(function(a){a=$(this);var b=a.offset(),e=b.left,g=b.top-12,m=a.width();c&&(b=$("mail"==rcmail.env.task?"#sievefilterform > iframe":"#filter-box",parent.document).offset(),g+=b.top,e+=b.left);d.html(a.data("tip"));g-=d.height();d.css({left:e,top:g,minWidth:m-2+"px"}).show()})};
...
e.children().each(function(){var a=$(this);0==a.text().indexOf(d)&&f._scrollIntoView(a)})},select:function(a,b){$(this).val(b.item.value);return!1}}).click(function(){$(this).autocomplete("search",$(this).val()||" ")})}

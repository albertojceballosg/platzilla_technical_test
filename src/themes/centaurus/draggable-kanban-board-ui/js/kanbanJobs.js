            ...
            settings.items.forEach(function (item , index , array) {
                ...
                var append =  '<div class="'+classes.kanban_board_item_class+'" data-id="'+item.id+'">';
                        ...
                        if(item.link){
                            ...
                            append += '<a rel="'+ item.link +'@' + item.link_text +'" href="#" onclick="KanbanJobUtils.openRecord(this, event)" title="Abrir detalles" class="kanban-task-btn"><i class="fa fa-external-link"></i></a>&nbsp;';
                            append += '<a rel="'+ item.link +'@' + item.link_text +'" href="#" onclick="KanbanJobUtils.viewRecord(this, event)" title="Ver detalles" class="kanban-task-btn"><i class="fa fa-eye"></i></a>&nbsp;';
                            append += '<a rel="'+ item.link +'@' + item.link_text +'" href="#" onclick="KanbanJobUtils.assignRecord(this, event)" title="Asignar expediente" class="kanban-task-btn"><i class="fa fa-refresh"></i></a>&nbsp;';
                            append += '<a rel="'+ item.link +'@' + item.link_text +'" href="#" onclick="KanbanJobUtils.deleteRecord(this, event)" title="Eliminar expediente" class="kanban-task-btn"><i class="fa fa-trash-o"></i></a>&nbsp;';
                            append += '</div>';
                        }

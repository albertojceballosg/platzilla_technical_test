...
{foreach $TAB_PRIV as $tabid => $elements}
	{if ($tabid == 0) || ($TAB_PRIV[$tabid][2] != $TAB_PRIV[$tabid-1][2])}
<div class="panel panel-default">
	<div class="panel-heading">
		<h4 class="panel-title">
			<a class="accordion-toggle{if ($tabid != 0)} collapsed{/if}" data-toggle="collapse" data-parent="#accordion" href="{if ($tabid == 0)}#collapseOne{else}{$tabid}{/if}">
				{$TAB_PRIV[$tabid][2]}
			</a>
		</h4>
	</div>
	<div id="{if ($tabid == 0)}collapseOne{else}{$tabid}{/if}" class="panel-collapse collapse{if ($tabid == 0)} in{/if}">
		<div class="panel-body">
			<table class="table table-bordered">
				...
				<tbody>
	{else}
		{assign var=contadorTabsDistintas value=$contadorTabsDistintas+1}
	{/if}

...
<link rel="stylesheet" type="text/css" href="{$THEME_PATH}/css/libs/dataTables.fixedHeader.css">
<link rel="stylesheet" type="text/css" href="{$THEME_PATH}/css/libs/dataTables.tableTools.css">
					...
					<div class="main-box clearfix">
						...
						<div class="main-box clearfix">
							<h1>
				                ...
				                {else}
				                        {if $RECORD_ID}
				                            <a href="javascript:;" onclick="window.history.back();">{$APP[$MODULE]}</a> {$PRODUCT_NAME}
										{else}
				                            {$APP[$MODULE]}
										{/if}
				                {/if}
						...
						<div class="main-box-body clearfix">
							...
							<form name="basicSearch" action="index.php" onsubmit="callSearch('Basic');return false;">
								<div class="row">
									<div class="col-lg-12" style="">
										<div classs="col-md-6" style="padding:5px;">
												<input type="text" name="search_text" id="search_txt" class="form-control">
										</div>
										<div classs="col-md-6">
											{*<b>{$APP.LBL_IN}</b>&nbsp;*}
						...
						<!--  pedido [TT11231] -->
										<input type="hidden" name="patron_enable" id="patron_enable" value="{$PATRON}">
						<!-- fin pedido [TT11231] -->
										...
										{/if}
										<div classs="col-md-6 text-center" style="padding:5px;">
											<input type="button" name="search" value=" &nbsp;{$APP.LBL_SEARCH_NOW_BUTTON}&nbsp; " onClick="callSearch('Basic');" class="btn btn-info btn-sm">
										</div>
							...
							<div class="table-responsive">
								...
								<div class="" id="table-example_wrapper">

									<div class="" id="dataTables_length">
									...
									{* Botón mostrar todos los registros *}
									<div class="row">
										<div class="col-md-12">
											{if $recid_var_value neq ''}
				                               <input id="all_contacts" alt="{$APP.LBL_SELECT_BUTTON_LABEL} {$APP.$MODULE}" title="{$APP.LBL_SELECT_BUTTON_LABEL} {$APP.$MODULE}" accessKey="" class="btn btn-primary btn-sm" value="{$APP.SHOW_ALL}&nbsp;{$APP.$MODULE}" LANGUAGE=javascript onclick="window.location.href=showAllRecords();" type="button"  name="button">
				                       		{/if}
										</div>
									</div>
...
{literal}
	function serializeNonEmptyFormData (formName) {
		var fields                            = jQuery ('form[name=' + formName + ']', opener.document).find ('.for-filter'),
			n                                 = fields.length,
...
{ldelim}
    ...
    {ldelim}
		...
		var obj = getObj(data_td_id);
		if (obj) {ldelim}
			obj.className = 'searchAlph';
		{rdelim}
    {rdelim}
	...
	urlstring += "&customreturnfunc={$smarty.request.customreturnfunc}";

	if(record_id!='')
		urlstring += '&record_id='+record_id;
	urlstring += serializeNonEmptyFormData ('EditView');
	new Ajax.Request(
		'index.php',
...
//----- pedido [TT11231]-----//
	if(getObj('patron_enable').value != '')
		urlstring +='&patron=enable';
//----- fin pedido [TT11231]-----//
    ...
    if(getObj('ctrlid').value != '')
            urlstring +='&'+getObj('ctrlid').name+'='+getObj('ctrlid').value;
    if(getObj('ctrlname').value != '')
            urlstring +='&'+getObj('ctrlname').name+'='+getObj('ctrlname').value;

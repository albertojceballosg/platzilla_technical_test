...
<div class="row">
	<form>
	{if $DEL_DENIED neq ""}
	<span id="action_msg_status"  align="left"><font color=red><b>{$MOD.LBL_PERM_DENIED} {$DEL_DENIED}</b> </font></span>
	{/if}
	...
	{assign var=poscount value=$poscount+1}
		...
		<tr>
			<td   colspan="3" style="padding: 5px;" align="right" >
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table"><tr>
		<td  id="repposition{$poscount}" width="5%" align="right"><input name="newReportInThisModule" value="{$MOD.LBL_CREATE_REPORT}..." class="btn btn-success btn-sm" onclick="gcurrepfolderid={$reportfolder.id};fnvshobj(this,'reportLay')" type="button"></td>
			<td  width="75%" align="right">
					<input type="button" name="Edit" value=" {$MOD.LBL_RENAME_FOLDER} " class="btn btn-primary" onClick='EditFolder("{$reportfolder.id}","{$reportfolder.fname}","{$reportfolder.fdescription}"),fnvshobj(this,"orgLay");'>&nbsp;
			</td>
			<td align="right">
					<input type="button" name="delete" value=" {$MOD.LBL_DELETE_FOLDER} " class="btn btn-warning btn-sm" onClick="DeleteFolder('{$reportfolder.id}');">
			</td>
		</tr>
		</table>
		</div>
		...
		<table  border="0" cellpadding="3" cellspacing="1" width="100%" class="table">
			...
			<th align="left" width="35%">{$MOD.LBL_REPORT_NAME}</th>
			<th  align="left" width="50%">{$MOD.LBL_DESCRIPTION}</th>
			<th  width="10%">{$MOD.LBL_TOOLS}</th>
			</tr>
			{foreach name=reportdtls item=reportdetails from=$reportfolder.details}
			<tr onmouseover="this.className='lvtColDataHover'" onmouseout="this.className='lvtColData'" bgcolor="white">
			<td>
...
<div style="display: none;left:193px;top:106px;width:155px;" id="folderLay" onmouseout="fninvsh('folderLay')" onmouseover="fnvshNrm('folderLay')" class="modal fade in">
...
<table class="table" width="100%">
	...
	<a href="javascript:;" onClick='MoveReport("{$folder.id}","{$folder.fname}");' >- {$folder.name}</a>
	{/foreach}
	</td>
	</tr>
</table>

		...
		<div style="width: auto; display: block;">
			<header class="main-box-header clearfix" style="margin-top: 0; padding-bottom: 0; padding-top: 0;">
				...
				<div class="filter-block pull-right">
					<a href="#" class="btn btn-primary pull-right" data-block-number="{$BLOCK_NUMBER}" onclick="WizardUtils.addField (this); return false;">
			...
			<table id="lvt-table" width="100%" cellspacing="0" cellpadding="0" border="0" class="lvt small block-fields">
				...
				<tr>
					...
					<th class="lvtCol" width="30%">{$MOD.LBL_ETIQUETA_CAMPO}</th>
...
{if (isset ($IS_FIRST_BLOCK)) && ($IS_FIRST_BLOCK == true)}
	{include file='Settings/ModuleManager/WizardStep3FieldDetail.tpl'
		...
		FIELD_PRECISION=''
		FIELD_PREFIX=strtoupper(substr(str_replace('_', '', $MODULE_NAME), 0, 3))|cat:'-'
		FIELD_SEQUENCE='001'
		FIELD_TYPE=4
		FIELD_VALUE=''
		GLOBAL_PICKLIST=''
...
{for $i=$row to $n}
	{if ($FIELD_BLOCK_NUMBERS[$i] == $BLOCK_NUMBER)}
		{include file='Settings/ModuleManager/WizardStep3FieldDetail.tpl'
			...
			GLOBAL_PICKLIST=$GLOBAL_PICKLISTS[$i]
			VISIBLE=true
		}
	{/if}
{/for}

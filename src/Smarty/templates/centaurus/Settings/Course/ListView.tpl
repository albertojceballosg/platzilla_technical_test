{strip}
    {*assign var='SELECTED_TAB' value='how_use' *}
    <div id="email-box" class="clearfix" style="padding-bottom: 20px;">
    <table class="table" width="100%" cellspacing="0" cellpadding="5" border="0">
        <tbody>
        <tr>
            <td rowspan="2" valign="top">
                <div class="infographic-box" style="width: 30px; padding: 0;"><i
                            class="fa fa-graduation-cap emerald-bg"></i>
                </div>
            </td>
            <td class="heading2" valign="bottom">
                <ol class="breadcrumb">
                    <li>
                        <a href="index.php?module=Settings&action=index&parenttab=Settings">CONFIGURACIÓN</a>
                    </li>
                    <li class="active" style="text-transform: uppercase">{$MOD['LBL_COURSE']}</li>
                </ol>
            </td>
        </tr>
        <tr>
            <td class="small" valign="top">{$MOD['LBL_COURSE_DESCRIPTION']}</td>
        </tr>
        </tbody>
    </table>
    {if (!empty ($MESSAGE))}
        <div class="alert alert-{if ($IS_ERROR)}danger{else}success{/if} fade in">
            <strong>{if ($IS_ERROR)}Error!{else}Listo!{/if}</strong> {$MESSAGE}
        </div>
    {/if}
    <div class="main-box clearfix">
        <ul class="nav nav-tabs">
            <li {if ($SELECTED_TAB eq 'courses')} class="active"{/if}>
                <a data-toggle="tab" href="#coursese-tab">Microcursos</a>
            </li>
            <li {if ($SELECTED_TAB eq 'courses_categories')} class="active"{/if}>
                <a data-toggle="tab" href="#courses-categories-tab">Categorías</a>
            </li>
            <li {if ($SELECTED_TAB eq 'courses_series')} class="active"{/if}>
                <a data-toggle="tab" href="#courses-series-tab">Series</a>
            </li>

        </ul>
        <div class="main-box-body clearfix">
            <div class="tab-content">
                {* Courses *}
                <div id="coursese-tab" class="tab-pane fade in{if ($SELECTED_TAB eq 'courses')} active{/if}">
                    <header class="main-box-header clearfix text-right">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="pull-left">
                                    <div class="row">
                                        <div class="col-xs-6">
                                    <select class="form-control" name="forcategory" id="forcategory" title="Categoría"
                                            onchange="CoursePanelUtils.filterCategories(this)">
                                        <option value="" selected>Todas las Categorias</option>
                                        {foreach $CATEGORIES as $cat}
                                            <option value="{$cat->getId()}"> {$cat->getName()}</option>
                                        {/foreach}
                                    </select>
                                        </div>
                                        <div class="col-xs-6">
                                    <select class="form-control" name="forserie" id="forserie" title="Serie"
                                            onchange="CoursePanelUtils.filterSerie(this)">
                                        <option value="" selected>Todos las series</option>
                                        {foreach $SERIES as $ser}
                                            <option value="{$ser->getId()}"> {$ser->getName()}</option>
                                        {/foreach}
                                    </select>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="pull-right">
                                    <a href="index.php?module=Courses&action=EditView" target="_blank"
                                       class="btn btn-primary"><i
                                                class="fa fa-plus-circle"></i> Crear curso</a>
                                </div>
                            </div>
                        </div>

                    </header>
                    <div class="table-responsive">
                        <table class="table table-striped table-hover">
                            <thead>
                            <tr>
                                <th class="col-title" style="width: 10%;text-align: center">Foto</th>
                                <th class="col-from">Course</th>
                                <th class="col-to" style="width: 10%">Categoría</th>
                                <th class="col-to" style="width: 10%;">Serie</th>
                                <th class="col-to" style="width: 6%;text-align: center">Costo</th>
                                <th class="col-to" style="width: 6%">Estado</th>
                                <th class="col-actions" style="width: 18%;text-align: center">Acciones</th>
                            </tr>
                            </thead>
                            <tbody id="course-panel-table">
                            {if ($MICROCOURSES neq NULL) }
                                {foreach $MICROCOURSES as $course}
                                    {assign var='courseId' value=$course->getId ()}
                                    {assign var='courseCategoryId' value=$course->getCategoryId ()}
                                    {assign var='courseCategory' value=$course->getCategory ()}
                                    {assign var='courseSerie' value=$course->getSerie ()}
                                    {assign var='courseLevel' value=$course->getLevel ()}
                                    {assign var='courseName' value=$course->getName ()}
                                    {assign var='coursePrice' value=$course->getPrice ()}
                                    {assign var='isPaid' value=$course->isPaid ()}
                                    {assign var='courseImage' value=$course->getImageCourse ()}
                                    {assign var='courseImageType' value=$course->getImageType ()}
                                    {assign var='courseStatus' value=$course->getStatus ()}
                                    <tr id="cat-{$courseCategory->getId()}-row-{$course->getId()}-ser-{$courseSerie->getId()}">
                                        <td class="col-title">
                                            <a href="index.php?module=Courses&action=EditView&record={$courseId}"
                                               class="center-block"
                                               target="_blank"
                                               style="margin-bottom: 1px!important;">
                                                {if !empty ($courseImage)}
                                                    <img id="course-photo-{$courseId}"
                                                         src="data:{$courseImageType}; base64, {$courseImage}"
                                                         class="center-block img-thumbnail img-responsive">
                                                {else}
                                                    <img class="img-thumbnail img-responsive center-block"
                                                         src='data:image/png;base64,'/>
                                                {/if}</a>
                                        </td>
                                        <td class="col-from">{$courseName}</td>
                                        <td class="col-to">{$courseCategory->getName()}</td>
                                        <td class="col-to">{$courseSerie->getName ()}</td>
                                        <td class="col-to" style="width: 6%;text-align: center">{$coursePrice}</td>
                                        <td class="col-to">{$MOD[$courseStatus]}</td>
                                        <td class="col-actions" style="width: 18%;text-align: center!important;">
                                            <div class="btn-group">
                                                <a href="index.php?module=Courses&action=EditView&record={$courseId}"
                                                   target="_blank" class="btn btn-default btn-sm btn-info"
                                                   title="Editar curso"><i class="fa fa-pencil-square-o" aria-hidden="true">
                                                    </i></a>
                                                <button type="button" class="btn btn-sm btn-warning"
                                                        data-record="{$courseId}"
                                                        onclick="CoursePanelUtils.clonarCourse(this)"
                                                        title="Duplicar curso">
                                                    <i class="fa fa-files-o" aria-hidden="true"></i></button>
                                                <button type="button" class="btn btn-sm btn-primary"
                                                        data-record="{$courseId}"
                                                        onclick="CoursePanelUtils.changeImage(this, event)"
                                                        title="Cambiar foto"><i class="fa fa-camera" aria-hidden="true">

                                                    </i></button>
                                                <button type="button" class="btn btn-sm btn-default"
                                                        title="{if $courseStatus eq 'ACTIVE'}Desactivar{else}Activar{/if} curso"
                                                        data-record="{$courseId}"
                                                        data-status="{$courseStatus}"
                                                        onclick="CoursePanelUtils.changeStatusCourse (this)">
                                                    {if $courseStatus eq 'ACTIVE'}
                                                        <i class="fa fa-check-square-o" aria-hidden="true"></i>
                                                    {else}
                                                        <i class="fa fa-square-o" aria-hidden="true"></i>
                                                    {/if}
                                                </button>
                                                <button type="button" class="btn btn-sm btn-sm btn-danger"
                                                        data-record="{$courseId}"
                                                        onclick="CoursePanelUtils.deleteCourse(this)"
                                                        title="Eliminar"><i class="fa fa-trash-o"></i></button>
                                            </div>
                                        </td>
                                    </tr>
                                {/foreach}
                            {else}
                                <tr class="lvtColData">
                                    <td colspan="7" class="text-center">No hay Microcursos</td>
                                </tr>
                            {/if}
                            </tbody>
                        </table>
                    </div>
                </div>
                {* Courses Categories *}
                <div id="courses-categories-tab"
                     class="tab-pane fade in{if ($SELECTED_TAB eq 'courses_categories')} active{/if}">&nbsp;
                    <header class="main-box-header clearfix text-right">
                        <div class="pull-right">
                            <a href="index.php?module=Settings&action=CourseEditViewCategory&parenttab=Settings"
                               class="btn btn-primary"><i
                                        class="fa fa-plus-circle"></i> Crear categoría</a>
                        </div>
                    </header>
                    {* Table Categories *}
                    <div class="table-responsive">
                        <table class="table table-striped table-hover">
                            <thead>
                            <tr>
                                <th class="col-from">Categorías</th>
                                <th class="col-to" style="width: 10%">Estado</th>
                                <th class="col-actions" style="width: 10%;text-align: center">Acciones</th>
                            </tr>
                            </thead>
                            <tbody id="how-use-table">
                            {if ($CATEGORIES neq NULL) }
                                {foreach $CATEGORIES as $category}
                                    {assign var='categoryId' value=$category->getId ()}
                                    {assign var='categoryName' value=$category->getName ()}
                                    {assign var='categoryStatus' value=$category->getStatus ()}
                                    <tr id="row-category-{$category->getId()}">
                                        <td class="col-title">
                                            <a href="index.php?module=Settings&action=CourseEditViewCategory&record={$categoryId}"
                                               class="center-block"
                                               target="_blank"
                                               style="margin-bottom: 1px!important;">{$categoryName}</a>
                                        </td>
                                        <td class="col-to">{$MOD[$categoryStatus]}</td>
                                        <td class="col-actions" style="width: 10%;text-align: center">
                                            <div class="btn-group">
                                                <a href="index.php?module=Settings&action=CourseEditViewCategory&record={$categoryId}"
                                                   class="btn btn-default btn-sm btn-info"
                                                   title="Editar categoría"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                                                <button type="button" class="btn btn-sm btn-default"
                                                        data-record="{$categoryId}"
                                                        data-status="{$categoryStatus}"
                                                        onclick="CoursePanelUtils.changeStatusCategory (this)"
                                                        title="{if $categoryStatus eq 'ENABLED'}Desactivar{else}Activar{/if} categoría">
                                                    {if $categoryStatus eq 'ENABLED'}
                                                        <i class="fa fa-check-square-o" aria-hidden="true"></i>
                                                    {else}
                                                        <i class="fa fa-square-o" aria-hidden="true"></i>
                                                    {/if}
                                                </button>
                                                <button type="button" class="btn btn-sm btn-sm btn-danger"
                                                        data-record="{$categoryId}"
                                                        onclick="CoursePanelUtils.deleteCategory(this)"
                                                        title="Eliminar"><i class="fa fa-trash-o"></i></button>
                                            </div>
                                        </td>
                                    </tr>
                                {/foreach}
                            {else}
                                <tr class="lvtColData">
                                    <td colspan="3" class="text-center">No hay Microcursos</td>
                                </tr>
                            {/if}
                            </tbody>
                        </table>
                    </div>
                </div>
                {* Courses Series *}
                <div id="courses-series-tab" class="tab-pane fade in{if ($SELECTED_TAB eq 'courses_series')} active{/if}">&nbsp;
                    <header class="main-box-header clearfix text-right">
                        <div class="pull-right">
                            <a href="index.php?module=Settings&action=CourseEditViewSerie&parenttab=Settings"
                               class="btn btn-primary"><i
                                        class="fa fa-plus-circle"></i> Crear serie</a>
                        </div>
                    </header>
                    {* Table Series *}
                    <div class="table-responsive">
                        <table class="table table-striped table-hover">
                            <thead>
                            <tr>
                                <th class="col-from">Categorías</th>
                                <th class="col-to" style="width: 10%">Estado</th>
                                <th class="col-actions" style="width: 10%;text-align: center">Acciones</th>
                            </tr>
                            </thead>
                            <tbody id="how-use-table">
                            {if ($SERIES neq NULL) }
                                {foreach $SERIES as $serie}
                                    {assign var='serieId' value=$serie->getId ()}
                                    {assign var='serieName' value=$serie->getName ()}
                                    {assign var='serieStatus' value=$serie->getStatus ()}
                                    <tr id="row-serie-{$serie->getId()}">
                                        <td class="col-title">
                                            <a href="index.php?module=Settings&action=CourseEditViewSerie&record={$serieId}"
                                               class="center-block"
                                               target="_blank"
                                               style="margin-bottom: 1px!important;">{$serieName}</a>
                                        </td>
                                        <td class="col-to">{$MOD[$serieStatus]}</td>
                                        <td class="col-actions" style="width: 10%;text-align: center">
                                            <div class="btn-group">
                                                <a href="index.php?module=Settings&action=CourseEditViewSerie&record={$serieId}" class="btn btn-default btn-sm btn-info" title="Editar categoría"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                                                <button type="button" class="btn btn-sm btn-default"
                                                        data-record="{$serieId}"
                                                        data-status="{$serieStatus}"
                                                        onclick="CoursePanelUtils.changeStatusSerie (this)"
                                                        title="{if $serieStatus eq 'ENABLED'}Desactivar{else}Activar{/if} categoría">
                                                    {if $serieStatus eq 'ENABLED'}
                                                        <i class="fa fa-check-square-o" aria-hidden="true"></i>
                                                    {else}
                                                        <i class="fa fa-square-o" aria-hidden="true"></i>
                                                    {/if}
                                                </button>
                                                <button type="button" class="btn btn-sm btn-sm btn-danger"
                                                        data-record="{$serieId}"
                                                        onclick="CoursePanelUtils.deleteSerie(this)"
                                                        title="Eliminar"><i class="fa fa-trash-o"></i></button>
                                            </div>
                                        </td>
                                    </tr>
                                {/foreach}
                            {else}
                                <tr class="lvtColData">
                                    <td colspan="3" class="text-center">No hay Series</td>
                                </tr>
                            {/if}
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <script type="text/javascript" src="modules/Settings/course-panel-utils.js"></script>
{/strip}
{strip}
	...
	<form action="index.php" method="post" id="SaveEditCustomButtons" name="index">
		...
		<input type="hidden" id="actionCrud" name="actionCrud" value="Save">
		<div class="row">
			<div class="col-lg-12">
				<div class="col-lg-9 pull-left">
					<h1><a
							href="index.php?module=Settings&action=CustomButtons&parenttab=Settings">{$MOD.LBL_CUSTOM_BUTTONS_CREATE}</a>
					</h1>
				</div>
				...
				<div class="main-box no-header">
					<div class="main-box-body clearfix" id="">
						<div id="cb-dv-title" class="form-group">
							...
							<input type="text" id="title" name="title" class="form-control" title="Texto del botón"
								placeholder="Texto del botón" maxlength="30" />
						...
						<div id="cb-dv-description" class="form-group">
							<label for="description"
								...
								style="color: red;">*</span>
							<input type="text" id="description" name="description" class="form-control" title="Descripción"
						...
						<div id="cb-dv-modulo" class="form-group">
							<label for="modulo">{$MOD.LBL_CUSTOM_BUTTONS_MODULE}</label>&nbsp;<span
								style="color: red;">*</span>
							{if ($IS_INSTANCE) && (!empty ($MODULE_NAME))}
								<input type="hidden" name="modulo" value="{$MODULE_NAME}" />
							{/if}

							<select id="modulo" {if (!$IS_INSTANCE) || (empty ($MODULE_NAME))} name="modulo" {/if}
								class="form-control" {if ($IS_INSTANCE) && (!empty ($MODULE_NAME))} disabled="disabled"
								{else} onchange="CBUtils.getCustomButtonField(this)" 
								{/if}>
								<option value="">Seleccione</option>
								{foreach item=module from=$MODULESFREE}
									<option value="{$module.name}"
										{if (!empty ($MODULE_NAME)) && ($MODULE_NAME == $module.name)} selected="selected"
										{/if}>{$module.tablabel}</option>
								{/foreach}
							</select>
						...
						<div id="cb-dv-vista" class="form-group">
							...
							<select id="vista" name="vista" class="form-control" onclick="CBUtils.getViewsAvailable(this);">
								{foreach item=vista from=$VISTASDISPONIBLES}
									<option value="{$vista.name}">{$vista.label}</option>
								{/foreach}
							</select>
						...
						<div class="form-group condition-groups">
							...
							<div class="action-bar text-center">
								<button type="button" class="btn btn-link" data-group="0"
									onclick="CBUtils.addFilterGroup (this);" title="Agregar grupo de condiciones">
						...
						<div id="cb-dv-clickaction" class="form-group">
							<label for="clickaction"
								id="label_clickaction">{$MOD.LBL_CUSTOM_BUTTONS_CLICKACTION}</label>&nbsp;
							<input type="text" id="clickaction" name="clickaction" class="form-control"
								title="Acción a ser Ejecutada por el botón"
								placeholder="Acción a ser Ejecutada por el botón" />
						...
						<div id="cb-dv-linkaction" class="form-group">
							...
							<input type="text" id="linkaction" name="linkaction" class="form-control"
								title="Acción a ser Ejecutada por el botón"
								placeholder="Acción a ser Ejecutada por el botón" disabled="disabled" />
										...
										<optgroup label="{$scope}">
											{foreach $tasks as $task}
												...
												<option value="{$taskName}"
													{if (isset ($CUSTOMBUTTON.backgroundtaskname)) && ($taskName == $CUSTOMBUTTON.backgroundtaskname)}

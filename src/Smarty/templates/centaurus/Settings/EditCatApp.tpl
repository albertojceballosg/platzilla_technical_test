...
<form action="index.php?module=Settings&action=SaveEditCatApps" method="post" name="index"
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="col-lg-9 pull-left">
                <h1>
                    <a href="index.php?module=Settings&action=CategoryApps&parenttab=Settings">{$MOD.LBL_CREATE_CATEGORYAPPS_BUTTON_LABEL} </a>
                </h1>
            </div>
            <div class="col-lg-3 pull-right text-right">
                <button class="btn btn-primary btn-sm" type="submit" id="btnsave">{$MOD.LBL_SAVE}</button>
                <a class="btn btn-warning btn-sm" type="submit"
                   href="index.php?module=Settings&action=CategoryApps">{$MOD.LBL_CANCEL}</a>
            </div>
        </div>
    </div>
            ...
            <div class="main-box no-header">
                <div class="main-box-body clearfix" id="">
                    ...
                    <div class="form-group">
                        <label for="app_code" id="label_app_code">{$MOD.LBL_CONFIG_APPS_CODE}</label>&nbsp;<font
                                color="red">*</font>
                        <input type="text" placeholder="" id="app_code" name="code" value="{$CONFIGAPPLICATION.code}"
                               class="form-control" tite="Identificador de la aplicación">
                    </div>
                    <div class="form-group">
                        <label for="app_name" id="label_app_name">{$MOD.LBL_CONFIG_APPS_NAME}</label>&nbsp;<font
                                color="red">*</font>
                        <input type="text" placeholder="" id="app_name" name="name" value="{$CONFIGAPPLICATION.name}"
                               class="form-control">
                    </div>
                    {if (!empty ($AVAILABLE_MENUS))}
                        <div class="form-group">
                            <label for="app_status">{$MOD.LBL_CONFIG_PARENTTAB_LABEL}</label>
                            <select id="menu-label" name="menuLabel" class="form-control">
                                <option value="" {if $CONFIGAPPLICATION.parenttab_label eq 'null' || $CONFIGAPPLICATION.parenttab_label || $CONFIGAPPLICATION.parenttab_label eq ''} selected="selected" {/if} >{$MOD.LBL_CONFIG_PARENTTAB_LABEL_UNDEFINED}</option>
                                {foreach $AVAILABLE_MENUS as $menu}
                                    <option value="{$menu}" {if $menu eq $CONFIGAPPLICATION.parenttab_label}  selected="selected" {/if} >{$menu}</option>
                                {/foreach}
                            </select>
                        </div>
                    {/if}
                    <div class="form-group">
                        <label for="app_descripcion"
                               id="label_app_descripcion">{$MOD.LBL_CONFIG_APPS_DESCRIPTION_LIST}</label>
                        <textarea rows="3" id="app_descripcion" name="descripcion"
                                  class="form-control">{$CONFIGAPPLICATION.app_description}</textarea>
                    </div>
                    <div class="form-group">
                        <label for="app_status">{$MOD.LBL_CONFIG_APPS_STATUS}</label>
                        <select id="app_status" name="status" class="form-control">
                            <option value="Activa" {if $CONFIGAPPLICATION.status eq 'Activa'} selected="selected" {/if} >
                                Activa
                            </option>
                            <option value="Inactiva" {if $CONFIGAPPLICATION.status eq 'Inactiva'} selected="selected" {/if} >
                                Inactiva
                            </option>
                        </select>
                    </div>

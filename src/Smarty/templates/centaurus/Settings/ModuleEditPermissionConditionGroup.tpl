		...
		<div class="filter-group-header list-group-item">
			<div class="row">
				...
				<div class="col-xs-2 text-right">
					<button type="button" class="btn btn-success" onclick="PermissionUtils.addFilter (this);" title="Agregar filtro" style="display: inline-block;"><i class="fa fa-plus"></i></button>
					<button type="button" class="btn btn-default" onclick="PermissionUtils.deleteFilterGroup (this);" title="Eliminar grupo" style="display: inline-block;"><i class="fa fa-trash-o"></i></button>
				</div>
			</div>
		</div>
...
{if (!empty ($filters))}
	{foreach $filters as $filter}
		{include file='Settings/ModuleEditPermissionCondition.tpl' FILTER=$filter}
	{/foreach}
{/if}
			</ul>
		</div>

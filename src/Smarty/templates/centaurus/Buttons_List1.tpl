...
{if $MODULE eq "simuladorvar" or $MODULE eq "escenarios" or $MODULE eq "simulador"}
	{include file='modules/simulador/menusimulador.tpl'}
{/if}
...
<!-- No List View in Settings - Action is index -->
	...
	{else}
		<td style="padding-left:10px;padding-right:50px" class="moduleName" nowrap><a  href="index.php?action=ListView&module={$MODULE}&parenttab={$CATEGORY}"><h1>{$MODULELABEL}</h1></a></td>
	{/if}
	<td width=100% nowrap>
		{if $CLIENT_VIEW neq 'true'}
		...
		<td class="small">
			...
			{if $MODULE eq 'Vendors' || $MODULE eq 'HelpDesk' || $MODULE eq 'Contacts' || $MODULE neq 'Reports'|| $MODULE eq 'Leads' || $MODULE eq 'Accounts' || $MODULE eq 'Potentials' || $MODULE eq 'Documents' || $CUSTOM_MODULE eq 'true' }
				...
				{/if}
				{if $CHECK.Export eq 'yes' && $MODULE neq 'Reports'}
			...
			{/if}
			{/if}
			{if $MODULE eq 'Contacts' || $MODULE eq 'Leads' || $MODULE eq 'Accounts' || $MODULE eq 'HelpDesk'|| $MODULE eq 'Potentials'|| $MODULE eq 'Vendors'}
				{if $VIEW eq true}
		...
		<td class="small">
			<!-- All Menu -->
				<table border=0 cellspacing=0 cellpadding=5>
				<tr>
				{if $CHECK.moduleSettings eq 'yes' and 0}
	        		<td style="padding-left:10px;"><a href='index.php?module=Settings&action=LayoutBlockList&formodule={$MODULE}&parenttab=Settings'><img src="{'settingsBox.png'|@vtiger_imageurl:$THEME}" alt="{$MODULE|getTranslatedString:$MODULE} {$APP.LBL_SETTINGS}" title="{$MODULE|getTranslatedString:$MODULE} {$APP.LBL_SETTINGS}" border="0"></a></td>
				{/if}
				</tr>
				</table>
		</td>

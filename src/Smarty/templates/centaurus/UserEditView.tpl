...
{ldelim}
	...
	var user_name = window.document.EditView.email1.value;
...
/* Johana Romero - [ TT11338 ] Nueva estructura de Usuarios de instancias - PLATZILLA - Guarda el usuario en vtiger_instanciaslogins */
function saveUserInstancia(codeInstancia){ldelim}
	//var urlstring = 'module=panelusuarios&action=panelusuariosAjax&ajax=true&file=panelusuarios&funcion=guardarUser&codeInstancia='+codeInstancia+'&login='+jQuery('#email1').val()+'&name='+jQuery('input[name=user_name]').val();
	var urlstring = 'module=panelusuarios&action=panelusuariosAjax&ajax=true&file=panelusuarios&funcion=guardarUser&codeInstancia='+codeInstancia+'&login='+jQuery('#email1').val()+'&name='+jQuery('#email1').val();
    new Ajax.Request(
	    'index.php',
	    {ldelim}queue: {ldelim}position: 'end', scope: 'command'{rdelim},
	        method: 'post',
	...
	<div class="col-lg-12">
		<div class="col-lg-12">
			<ol class="breadcrumb">
				...
				<li>
					{if $MODE eq 'edit'}
						<b class="small">{$UMOD.LBL_EDIT_VIEW} "{$USERNAME}"</b>
					{else}
...
<tbody><tr>
	...
	<tr>
		<td nowrap align="right">
			<input title="{$APP.LBL_CANCEL_BUTTON_TITLE}" accesskey="{$APP.LBL_CANCEL_BUTTON_KEY}" class="btn btn-cancel pull-right" name="button" value="  {$APP.LBL_CANCEL_BUTTON_LABEL}  " onclick="window.history.back()" style="margin-left:10px" type="button" />
			<!-- Johana Romero - [ TT11338 ] Nueva estructura de Usuarios de instancias - PLATZILLA - checkea si es instancia o no y ejecuta  saveUserInstancia()-->
			<!--<input title="{$APP.LBL_SAVE_BUTTON_TITLE}" accesskey="{$APP.LBL_SAVE_BUTTON_KEY}" class="btn btn-primary pull-right"  name="button" value="  {$APP.LBL_SAVE_BUTTON_LABEL}  "  onclick="this.form.action.value='Save'; {if $smarty.session.esInstancia }return verify_data(EditView);saveUserInstancia('{$smarty.session.plat}'); {/if} return verify_data(EditView);" style="margin-left:10px" type="button" />-->
			<input title="{$APP.LBL_SAVE_BUTTON_TITLE}" accesskey="{$APP.LBL_SAVE_BUTTON_KEY}" class="btn btn-primary pull-right"  name="button" value="  {$APP.LBL_SAVE_BUTTON_LABEL}  "  onclick="this.form.action.value='Save'; {if $smarty.session.esInstancia }saveUserInstancia('{$smarty.session.plat}'); {/if} return verify_data(EditView);" style="margin-left:10px" type="button" />
		</td>
	</tr>
	<tr><td class="padTab" align="left">
		...
		<tr><td colspan="2">
			<table align="center" border="0" cellpadding="0" cellspacing="0" width="99%" class="table">
			<tr>
			    <td align="left" valign="top">
					...
					<tr><td align="left">
					   ...
					   {/foreach}
				<br>
			    	<table class="tableHeading" border="0" cellpadding="5" cellspacing="0" width="100%" style="display:none">
			    	<tr>
				     <td class="big">	
			    	...
			    	<table border="0" cellpadding="5" cellspacing="0" width="100%" class="table" style="display:none">
				{foreach item=homeitems key=values from=$HOMEORDER}
					<tr><td class="dvtCellLabel" align="right" width="30%">{$UMOD.$values|@getTranslatedString:'Home'}</td>
					    {if $homeitems neq ''}
					    	<td class="text-center" align="center" width="5%">
					    		<!--div class="radio" style="border: 1px solid #ff00c3"-->
					   			<input name="{$values}" value="{$values}" checked type="radio">
					   			<!--/div-->
					   		</td>
					   		<td class="text-left" align="left" width="20%">{$UMOD.LBL_SHOW}
					   		</td> 		
					    	<td class="text-center" align="" width="5%">
					    		<!--div class="radio" style="border: 1px solid #ff00c3"-->
					   				<input name="{$values}" value="" type="radio">
					   			<!--/div-->
					   		</td>
					   		<td class="text-left">{$UMOD.LBL_HIDE}</td> 		
					    {else}	
				...
				<!-- End of Header -->
				<table border="0" cellpadding="5" cellspacing="0" width="100%" style="display:none">

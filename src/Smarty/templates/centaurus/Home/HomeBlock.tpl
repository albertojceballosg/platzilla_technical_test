...
background-image: -moz-linear-gradient(top, #F3B14E, #FFA100);
background-image: -webkit-gradient(linear, 0 0, 0 100%, from(#F3B14E), to(#FFA100));
background-image: -webkit-linear-gradient(top, #F3B14E, #FFA100);
background-image: -o-linear-gradient(top, #F3B14E, #FFA100);
background-image: linear-gradient(to bottom, #F3B14E 0%, #ffa100 100%);
background-repeat: repeat-x;
border-color: #FFA100 #FFA100 #387038;
border-color: rgba(0, 0, 0, 0.1) rgba(0, 0, 0, 0.1) rgba(0, 0, 0, 0.25);
filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='#ff62c462', endColorstr='#ffFFA100', GradientType=0);
...
{elseif $HOME_STUFFTYPE eq "Default"}
	...
	<table border=0 cellspacing=0 cellpadding=2 width=100% style="padding:5px">
	...
	<tr>
		...
		{/foreach}
		{if $INSTANCIA_FACTURACION eq 'true' && $HOME_STUFF.Details.ModuleName eq 'Invoice'}
		<tr style="background-color: lightblue;">
	...
	{else}
		<tr>
			<td width=5%>
			{if $HOME_STUFF.Details.ModuleName eq 'Invoice'}
				<div class="componentName" style="border-left: none;text-align: center;padding: 50px;">
					<a class="btn btn-orange fancybox iframe" style="width: 115px;margin-top: 77px;margin-left: -10px;text-decoration: none;" href="index.php?module=Invoice&action=index">Cree ahora su primera factura!</a>
				</div>
			{else}
				<div class="componentName">{$APP.LBL_NO_DATA}</div>
			{/if}

{strip}
    {* carousel *}
    <div class="col-xs-12 category" style="margin-top: 25px">
        <h2 class="pull-left">{$audience}</h2>
        <div class="col-xs-12 course-portraits">
            {math equation='(x / y) + z' x=$COURSES_DATA[$audience]|count y=4 z=0 assign='numIndicators'}
            <div id="Carousel-{$index}" class="carousel slide">
                <ol class="carousel-indicators {if $numIndicators|floor lt 1} hide{/if}">
                    {foreach $COURSES_DATA[$audience] as $course}
                        {if (($IS_INSTANCE) && ($courseStatus == Course::STATUS_INACTIVE) || ($course@iteration gt ($numIndicators|floor)) )}
                            {continue}
                        {/if}
                ...
                <div class="carousel-inner">
                    ...
                    {foreach from=$COURSES_DATA[$audience] item= $course name=foo}
                        {*$course->getUserCourseStatus ()|var_dump*}
                        ...
                        {assign var='courseSeenBy' value=$course->getSeenBy ()}
                        {assign var='courseStatus' value=$course->getStatus ()}
                        {assign var='courseUserStatus' value=$course->getUserCourseStatus ()}
                        {assign var='courseFirstLesson' value=$courseLessons[0]}
                        {if ($IS_INSTANCE) && ($courseStatus == Course::STATUS_INACTIVE)}
                            {continue}
                        {/if}
                        ...
                        <div class="col-md-3">
                            <a href="index.php?module=Courses&action=CourseView&record={$courseId}"
                               ...
                               style="margin-bottom: 1px!important;">
                                ...
                                {/if}</a>
                            <p class="course-name text-justify">{$courseName}{if $courseSeenBy eq NULL}
                                    <br>
                                    <span class="help-block" style="color: red">[Nuevo]</span>
                                {/if}</p>
                            <p class="course-category text-center hide">{$courseCategory->getName()}</p>
                            <p class="hide">
                                ...
                                <span class="pull-right">{if ($coursePrice > 0) ||($paidLesson > 0)}{if $isPaid}
                                        <strong>Comprado</strong>
                                    {else} {number_format($coursePrice, 2, ',', '.')} EUR {/if} {else}Gratis{/if}</span>
                            </p>
                            {if (!$IS_INSTANCE)}
                                <p class="pull-left">
                                    Status: {if ($courseStatus == Course::STATUS_ACTIVE)}Activo{elseif ($courseStatus == Course::STATUS_INACTIVE)}Inactivo{/if}</p>
                            {/if}
                                <div class="action-bar pull-right" style="margin-top: 2px">
                                    <i class="fa fa-circle fa-2x" aria-hidden="true" title="{$COURSES_STATUS_TITLE[$courseUserStatus]}" style="color: {$COURSES_STATUS_COLOR[$courseUserStatus]};{*border:1px solid #dee2e6!important*}"></i>
                            {if (!$IS_INSTANCE)}
                        ...
                        {/if}
                        {assign var='caroucelItem' value=($caroucelItem + 1)}
                        {if $caroucelItem eq 5}
                            {assign var='caroucelItem' value=1}
                        {/if}
                    {/foreach}
                    {if in_array($caroucelItem, array(2,3,4))}
                        </div><!--.row-->
                    </div><!--.item-->
                    {/if}
                </div><!--.carousel-inner-->

...
{elseif $MODULE eq 'Contacts'}
	{$ERROR_MESSAGE}
    <form name="EditView" method="POST" ENCTYPE="multipart/form-data" action="index.php" onsubmit="VtigerJS_DialogBox.block();return validateRole();" role="form">
...
{elseif $MODULE eq 'Calendar'}
	<input type="hidden" name="activity_mode" value="{$ACTIVITY_MODE}" onsubmit="VtigerJS_DialogBox.block();" role="form">
...
{elseif $MODULE eq 'PurchaseOrder' || $MODULE eq 'SalesOrder' || $MODULE eq 'Invoice' || $MODULE eq 'Quotes'}
	...
	<form id="frmEditView" name="EditView" method="POST" ENCTYPE="multipart/form-data" action="index.php"  role="form">
	<input type="hidden" name="hidImagePath" id="hidImagePath" value="{$IMAGE_PATH}"/>
	<input type="hidden" name="action" value="DevolverStock">
	<input type="hidden" name="ASSOCIATEDPRODUCTS" value="{$ASSOCIATEDPRODUCTS}" />
	<!-- End of code added -->

	{if $MODULE eq 'Invoice' || $MODULE eq 'PurchaseOrder' ||  $MODULE eq 'SalesOrder'}
       		 <input type="hidden" name="convertmode">
       		 <input type="hidden" name="devolucion" value="1" />
	{/if}

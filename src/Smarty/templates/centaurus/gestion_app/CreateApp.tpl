...
<form action="index.php?module=gestion_app&action=SaveEditApps" method="post" id="SaveEditApps" name="index" enctype="multipart/form-data" onsubmit="">


<div class="row">
  <div class="col-lg-12">
  		<div class="col-lg-9 pull-left">
      		<h1><a href="index.php?module=gestion_app&action=ConfigApps&parenttab=gestion_app">{$MOD.LBL_TITLE_MODAL_CREATE_APP} </a></h1>
      	</div>
      ...
      <div class="main-box-body clearfix" id="">
			...
			<!--form role="form"-->
				...
				<div class="form-group">
					...
					<input type="text" placeholder="" id="app_code" name="code" class="form-control" tite="Identificador de la aplicación">
				</div>
				<div class="form-group"  >
					<label for="app_name" id="label_app_name">{$MOD.LBL_CONFIG_APPS_NAME}</label>&nbsp;<font color="red">*</font>
					<input type="text" placeholder="" id="app_name" name="name" class="form-control">
				</div>
				...
				<div class="form-group"  >
					...
					<input type="text" placeholder="" id="app_url" name="url" class="form-control">
				</div>			
				<div class="form-group">
					<label for="app_descripcion" id="label_app_descripcion">{$MOD.LBL_CONFIG_APPS_DESCRIPTION_LIST}</label>
					<textarea rows="3" id="app_descripcion" name="descripcion" class="form-control"></textarea>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<select id="app_category" name="category[]" multiple="multiple" class="form-control">
						...
						{foreach key=keyC item=category from=$CATEGORIES}
							<option value="{$category.catappid}" >{$category.name}</option>
						{/foreach}
				...
				<div class="form-group">
	            	...
	            	<input type="file" name="binFile" id="binFile" class="" style="display:none" value="{$ORGANIZATIONLOGONAME}"  accept="image/png"  onchange="validatePngFilenameImage(this,0.8);if(this.value!=''){ldelim} jQuery('#binFileButton').val(this.value);{rdelim}">[{$ORGANIZATIONLOGONAME}]

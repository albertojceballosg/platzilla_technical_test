		...
		<div class="filter-group-header list-group-item">
			<div class="row">
				...
				<div class="col-xs-2 text-right">
					<button type="button" class="btn btn-info" onclick="CustomViewUtils.addAdvancedFilter (this);" title="Agregar filtro" style="display: inline-block;"><i class="fa fa-plus"></i></button>
...
{if (!empty ($filters))}
	{foreach $filters as $filter}
		{include file='CustomViewAdvancedFilter.tpl' FILTER=$filter}
	{/foreach}
{/if}
			</ul>
		</div>
	...
	<div class="filter-group-operator">
		<select name="advancedfiltergroups[{$groupId}][operator]" class="form-control operator{if (empty ($groupOperator))} hidden{/if}" title=""{if (empty ($groupOperator))} disabled="disabled"{/if}>
			<option value="and"{if ($groupOperator == 'and')} selected="selected"{/if}>y</option>
			<option value="or"{if ($groupOperator == 'or')} selected="selected"{/if}>o</option>
		</select>
	</div>
</div>
{/strip}
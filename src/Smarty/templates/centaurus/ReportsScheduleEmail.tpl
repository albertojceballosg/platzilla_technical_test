...
<table class="table" bgcolor="#ffffff" border="0" cellpadding="5" cellspacing="0"  width="100%">
	<tr valign='top'>
		<td colspan="1">
			<span>{'LBL_SCHEDULE_EMAIL'|@getTranslatedString:'Reports'}</span>
			<br>
			...
			<div>
				<table class="table" border="0" cellpadding="5" cellspacing="1" width="100%" valign="top">
					<tr valign="top">						
						...
						<td width="90%"><strong>{'LBL_SCHEDULE_REPORT'|@getTranslatedString:'Reports'}</strong></td>
					</tr>
					<tr valign="top">
						<td colspan="2">
							<table width="100%" class="table" border="0" cellpadding="5" cellspacing="0" align="top">
								<tr align="left" valign="top">
									<td><strong>{'LBL_SCHEDULE_FREQUENCY'|@getTranslatedString:'Reports'}</strong></td>
								</tr>
								<tr align="left" valign="top">
									<td valign=top >
										<span id="scheduledTypeSpan">
											<select class="form-control" name="scheduledType" id="scheduledType" onchange="javascript: setScheduleOptions();">
										...
										<span id="scheduledMonthSpan" style="display: {if $schtypeid eq 6}inline{else}none{/if};">&nbsp;<strong>{'LBL_SCHEDULE_EMAIL_MONTH'|@getTranslatedString:'Reports'}</strong>
											<select class="form-control" name="scheduledMonth" id="scheduledMonth">
										...
										<span id="scheduledDOMSpan" style="display: {if $schtypeid eq 5 || $schtypeid eq 6}inline{else}none{/if};">&nbsp;<strong>{'LBL_SCHEDULE_EMAIL_DAY'|@getTranslatedString:'Reports'}</strong>:
											<select class="form-control" name="scheduledDOM" id="scheduledDOM">
										...
										<span id="scheduledDOWSpan" style="display: {if $schtypeid eq 3 || $schtypeid eq 4}inline{else}none{/if};">&nbsp;<strong>{'LBL_SCHEDULE_EMAIL_DOW'|@getTranslatedString:'Reports'}</strong>:
											<select class="form-control" name="scheduledDOW" id="scheduledDOW">
										...
										<span id="scheduledTimeSpan" style="display: {if $schtypeid > 1}inline{else}none{/if};">&nbsp;<strong>{'LBL_SCHEDULE_EMAIL_TIME'|@getTranslatedString:'Reports'}</strong>:
											<input class="form-control" type="text" name="scheduledTime" id="scheduledTime" value="{$schtime}" size="5" maxlength="5" /> {'LBL_TIME_FORMAT_MSG'|@getTranslatedString:'Reports'}
										</span>
								...
								<tr align="left" valign="top">
									<td><strong>{'LBL_REPORT_FORMAT'|@getTranslatedString:'Reports'}</strong>:</td>
								</tr>
								<tr align="left" valign="top">
									<td valign=top>
										{'LBL_SELECT'|@getTranslatedString:'Reports'}:&nbsp;
										<select id="scheduledReportFormat" name="scheduledReportFormat" class="form-control">
								...
								<tr align="left" valign="top">
									<td valign=top><strong>{'LBL_USERS_AVAILABEL'|@getTranslatedString:'Reports'}</strong></td>
								</tr>
								<tr align="left" valign="top">
									<td>
										<table class="table">
											<tr>
												<td width="45%" valign=top>
													{'LBL_SELECT'|@getTranslatedString:'Reports'}:&nbsp;
													<select id="recipient_type" name="recipient_type"  onChange="showRecipientsOptions()">
													...
													<input type="hidden" name="findStr1" >&nbsp;
												</td>
												<td width="10%">&nbsp;</td>
												<td width="45%"><strong>{'LBL_USERS_SELECTED'|@getTranslatedString:'Reports'}</strong></td>
											</tr>
											...
											<tr>
												...
												<td width="50">
													<div align="center">
														<input type="button" name="addButton" value="&nbsp;&rsaquo;&rsaquo;&nbsp;" onClick="addOption()" class="btn btn-default"/><br /><br />
														<input type="button" name="delButton" value="&nbsp;&lsaquo;&lsaquo;&nbsp;" onClick="delOption()" class="btn btn-default"/>
													</div>
												</td>
												<td valign=top><br>
													<select id="selectedRecipients" name="selectedRecipients" multiple size="10" >
													{$SELECTED_RECIPIENTS}
													</select>

	...
	<table border=0 cellspacing=0 cellpadding=5 width=100% class="table">
		<tr>
			...
			<th  width="30%">
				...
				<select name="sourcefield" id="sourcefield"  {if $DEPENDENCY_MAP.sourcefield neq ''} disabled {/if}>
			...
			<th  width="30%">
				...
				<select name="targetfield" id="targetfield"  {if $DEPENDENCY_MAP.sourcefield neq ''} disabled {/if}>
					{foreach key=fld_nam item=fld_lbl from=$ALL_LISTS}
			...
			<th nowrap align="right" >
				...
				<input type="submit" value="{$APP.LBL_SAVE_BUTTON_LABEL}" name="save" class="btn btn-primary" onclick="saveDependency('{$MODULE}');" />
				{else}
				<input type="submit" value="{$APP.LBL_NEXT_BUTTON_LABEL}" name="next" class="btn btn-primary" onclick="editNewDependencyPicklist('{$MODULE}');" />
				{/if}
		 		<input type="button" value="{$APP.LBL_CANCEL_BUTTON_LABEL}" name="cancel" class="btn btn-cancel" onclick="location.href='index.php?module=PickList&action=PickListDependencySetup&parenttab=Settings&moduleName={$MODULE}';" />
		 	</th>
		</tr>
		...
		<tr >
			...
			<td align="right" valign="top">
				<input type="button" class="btn btn-primary" onclick="show('sourceValuesSelectionDiv');placeAtCenter($('sourceValuesSelectionDiv'));"
						...
						<table cellspacing="0" cellpadding="5" border="0" class="small" width="100%">
							...
							<tr>
								<td valign="top" colspan="4" align="center">
									<input type="button" value="{$APP.LBL_APPLY_BUTTON_LABEL}" name="apply" class="btn btn-primary" onclick="loadMappingForSelectedValues(); fnhide('sourceValuesSelectionDiv');">
									<input type="button" value="{$APP.LBL_CANCEL_BUTTON_LABEL}" name="cancel" class="btn btn-cancel" onclick="fnhide('sourceValuesSelectionDiv');">
								</td>
							</tr>
						</table>
					</div>

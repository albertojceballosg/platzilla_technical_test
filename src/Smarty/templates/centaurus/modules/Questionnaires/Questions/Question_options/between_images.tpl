{math equation= rand() assign= "idOption"}
{if isset($ANSWERS_OPTIONS) && !empty($ANSWERS_OPTIONS)}
    {assign var="photos" value=null}
    <div class="row-question-view justify-content-center" style="margin-bottom: 20px">
        <div class="col-md-8 rounded border" style="padding: 10px; background-color: #f9f8f7">
            <div class="table-responsive">
                <table class="table rules justify-content-center">
                    <thead>
                    <tr>
                        <th colspan="3" class="" style="width: 60%">Opción</th>
                        <th class="" style="width: 30%">Valor</th>
                        <th class="" style="width: 10%">Acciones</th>
                    </tr>
                    </thead>
                    <tbody id="TBODY-BETWEEN_IMAGES-{$idOption}" data-next-option="{$NUM_ROWS}">
                    {foreach $ANSWERS_OPTIONS as $answerOption}
                        {math equation= rand() assign= "idRowOption"}
                        {*assign var="photos" value=$answerOption->getImage()*}
                        <tr class="rule ">
                            <input type="hidden" name="question[{$ID}][response][{$answerOption->getSequence()}][answereid]" value="{$answerOption->getId()}">
                            <td style="vertical-align: top!important; width: 20%">
                                <div class="fileUpload btn btn-simple pull-left" style="width: 9em;">
                                    <span>Examinar</span>
                                    <input type="file" name="question[{$ID}][{$answerOption->getSequence()}]" class="upload" value="" tabindex=""
                                           onchange="QuestionUtils.showPreview(this, '{$idRowOption}'); QuestionUtils.validatePhotoSize(this,'{$UPLOAD_MAXSIZE}');">
                                    <input type="hidden" name="question[{$ID}][response][{$answerOption->getSequence()}][image_type]" id="photo-{$idRowOption}"
                                           value="{$answerOption->getImageType()}">
                                </div>
                            </td>
                            <td style="width: 15%;vertical-align: top!important;">
                                <img id="option-photo-{$idRowOption}" class="img-responsive center-block"
                                     src='{$answerOption->getImage()}'/>
                            </td>
                            <td style="vertical-align: top!important;width: 65%">
                                <div class="form-group field-container {*pull-right*}">
                                    <div class="row">
                                        <div class="col-md-2">
                                            <input type="radio" class=" pull-right col-md-12" name="question[{$ID}][response][0][selected]"
                                                   value="{$answerOption->getSequence()}" {if $answerOption->getSelected() neq NULL}checked {/if}>
                                        </div>
                                        <div class="col-md-10">
                                            <input type="text" tabindex="" placeholder="Etiqueta"
                                                   name="question[{$ID}][response][{$answerOption->getSequence()}][label]"
                                                   value="{$answerOption->getMainLabel()}" class="form-control">
                                        </div>
                                    </div>
                                </div>
                                <textarea id="question_feedback" name="question[{$ID}][response][{$answerOption->getSequence()}][feedback]" class="form-control"
                                          tabindex="" placeholder="Feed back"  rows="2">{$answerOption->getFeedBack()}</textarea>
                            </td>
                            <td class="" style="vertical-align: top!important;">
                                <input type="text" tabindex="" placeholder="Valor de la opción"  style="margin-bottom: 2px"
                                       name="question[{$ID}][response][{$answerOption->getSequence()}][value]"
                                       value="{$answerOption->getValue()}" class="form-control">

                            </td>
                            <td class="">
                                </button>&nbsp;<button type="button" class="btn btn-link" data-tamplate="BETWEEN_IMAGES" title="Eliminar"
                                                       onclick="QuestionUtils.delResponseOption (this, '{$idRowOption}');">
                                    <i class="fa fa-trash-o"></i>
                                </button>
                            </td>
                        </tr>
                    {/foreach}
                    </tbody>
                </table>
            </div>
            <div class="action-bar text-center">
                <div class="col-md-12 text-center">
                    <small>Seleccione la respuesta esperada.</small>
                </div>
                <button type="button" class="btn btn-link"
                        title="Agregar opción" data-tamplate="BETWEEN_IMAGES"
                        onclick="QuestionUtils.addResponseOption (this, '{$idOption}')"><i class="fa fa-plus"></i>
                </button>
            </div>
        </div>
    </div>
{else}
    <div class="row-question-view justify-content-center" style="margin-bottom: 20px">
        <div class="col-md-8 rounded border" style="padding: 10px; background-color: #f9f8f7">
            <div class="table-responsive">
                <table class="table rules justify-content-center">
                    <thead>
                    <tr>
                        <th colspan="3" class="" style="width: 75%">Opción</th>
                        <th class="" style="width: 20%">Valor</th>
                        <th class="" style="width: 5%">Acciones</th>
                    </tr>
                    </thead>
                    <tbody id="TBODY-BETWEEN_IMAGES-{$idOption}" data-next-option="1">
                    <tr class="rule ">
                        <td style="vertical-align: top!important;width: 10%">
                            <div class="fileUpload btn btn-simple pull-left" style="width: 9em;">
                                <span>Examinar</span>
                                <input type="file" name="question[{$ID}][0]" class="upload" value="" tabindex=""
                                       onchange="QuestionUtils.showPreview(this, '{$idOption}'); QuestionUtils.validatePhotoSize(this,'{$UPLOAD_MAXSIZE}');">
                                <input type="hidden" name="question[{$ID}][response][0][image_type]"
                                       id="photo-{$idOption}" value="">
                            </div>
                        </td>
                        <td style="width: 15%;vertical-align: top!important;padding-top: 6px">
                            <img id="option-photo-{$idOption}" class="img-responsive center-block"
                                 src='data:image/png;base64,'/>
                        </td>
                        <td style="vertical-align: top!important;width: 60%">
                            <div class="form-group field-container pull-left">
                                <div class="row">
                                    <div class="col-md-3">
                                        <input type="radio" class=" pull-right"
                                               name="question[{$ID}][response][0][selected]" value="0">
                                    </div>
                                    <div class="col-md-9">
                                        <input type="text" tabindex="" placeholder="Etiqueta"
                                               name="question[{$ID}][response][0][label]" value="" class="form-control">
                                    </div>
                                    <div class="col-md-12">
                                       <textarea id="question_feedback" name="question[{$ID}][response][0][feedback]" class="form-control"
                                                 tabindex="" placeholder="Feed back"  rows="2"></textarea>
                                    </div>
                                </div>
                            </div>

                        </td>
                        <td class="" style="vertical-align: top!important;width: 25%">
                            <input type="text" tabindex="" placeholder="Valor"  style="margin-bottom: 2px"
                                   name="question[{$ID}][response][0][value]" value="" class="form-control">
                        </td>
                        <td class="">
                            </button>&nbsp;<button type="button" class="btn btn-link" data-tamplate="BETWEEN_IMAGES"
                                                   title="Eliminar"
                                                   onclick="QuestionUtils.delResponseOption (this, '{$idOption}');">
                                <i class="fa fa-trash-o"></i>
                            </button>
                        </td>
                    </tr>
                    </tbody>
                </table>
            </div>
            <div class="action-bar text-center">
                <div class="col-md-12 text-center">
                    <small>Seleccione la respuesta esperada.</small>
                </div>
                <button type="button" class="btn btn-link"
                        title="Agregar opción" data-tamplate="BETWEEN_IMAGES"
                        onclick="QuestionUtils.addResponseOption (this, '{$idOption}')"><i class="fa fa-plus"></i>
                </button>
            </div>
        </div>
    </div>
{/if}
<script type="text/html" id="BETWEEN_IMAGES-TEMPLATE-{$idOption}">
    <tr class="rule ">
        <td style="vertical-align: top!important; width: 10%">
            <div class="fileUpload btn btn-simple pull-left" style="width: 9em;">
                <span>Examinar</span>
                <input type="file" name="question[{$ID}][__ID__]" class="upload" value="" tabindex=""
                       onchange="QuestionUtils.showPreview(this, '__ID_ROW__'); QuestionUtils.validatePhotoSize(this,'{$UPLOAD_MAXSIZE}');">
                <input type="hidden" name="question[{$ID}][response][__ID__][image_type]" id="photo-__ID_ROW__"
                       value="">
            </div>
        </td>
        <td style="width: 15%; vertical-align: top!important;">
            <img id="option-photo-__ID_ROW__" class="img-responsive center-block"
                 src='data:image/png;base64,'/>
        </td>
        <td style="vertical-align: top!important;width: 60%">
            <div class="form-group field-container pull-left">
                <div class="row">
                    <div class="col-md-3">
                        <input type="radio" class=" pull-right" name="question[{$ID}][response][0][selected]"
                               value="__ID__">
                    </div>
                    <div class="col-md-9">
                        <input type="text" tabindex="" placeholder="Etiqueta"
                               name="question[{$ID}][response][__ID__][label]" value="" class="form-control">
                    </div>
                    <div class="col-md-12">
                        <textarea id="question_feedback" name="question[{$ID}][response][__ID__][feedback]" class="form-control"
                                  tabindex="" placeholder="Feed back"  rows="2"></textarea>
                    </div>
                </div>
            </div>
        </td>
        <td class="" style="vertical-align: top!important;width: 25%">
            <input type="text" tabindex="" placeholder="Valor de la opción"  style="margin-bottom: 2px"
                   name="question[{$ID}][response][__ID__][value]" value="" class="form-control">
        </td>
        <td class="">
            </button>&nbsp;<button type="button" class="btn btn-link" data-tamplate="BETWEEN_IMAGES" title="Eliminar"
                                   onclick="QuestionUtils.delResponseOption (this, '__ID_ROW__');">
                <i class="fa fa-trash-o"></i>
            </button>
        </td>
    </tr>
</script>
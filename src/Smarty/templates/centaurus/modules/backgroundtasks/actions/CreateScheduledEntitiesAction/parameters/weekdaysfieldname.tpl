...
<div class="col-xs-12 parameter mandatory">
	...
	{foreach $AVAILABLE_FIELDS as $field}
		{if ($field.uitype == Field::UI_TYPE_GLOBAL_PICKLIST) && ($field.fieldname == 'sys_weekdays')}
	...
	{if (!empty ($availableFields))}
		...
		{/foreach}
	{else}
				<option value="">Imposible ejecutar la acción: el módulo {$SELECTED_MODULE_NAME} no tiene campos de días de la semana</option>
	{/if}
			</select>
		</div>
	</div>

...
<div class="col-xs-12 parameter mandatory">
	...
	{foreach $AVAILABLE_FIELDS as $field}
		{if ($field.uitype == Field::UI_TYPE_GLOBAL_PICKLIST) && ($field.fieldname == 'sys_frequency')}
	...
	{if (!empty ($availableFields))}
		...
		{/foreach}
	{else}
				<option value="">Imposible ejecutar la acción: el módulo {$SELECTED_MODULE_NAME} no tiene campos de frecuencia</option>
	{/if}
			</select>
		</div>
	</div>

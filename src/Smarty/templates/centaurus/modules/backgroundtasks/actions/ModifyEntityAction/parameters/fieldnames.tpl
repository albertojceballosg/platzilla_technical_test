				...
				<div class="row parameter">
					...
					<div class="form-group col-xs-12 col-md-3">
						<select id="{$parameterName}-{$ACTION_SEQUENCE}"
							...
							class="form-control parametertype" data-uitype="{$uiType}@{$ACTION_SEQUENCE}" title=""
							...
							{if ($AVAILABLE_FIELDS neq NULL)}
								<option value="SOURCE FIELD"
									...
									selected="selected" {/if}>Selecciona el campo correspondiente del módulo donde comienza la tarea
								</option>
							{/if}
					...
					<div class="form-group col-xs-12 col-md-6 field-container">
						...
						{if ($AVAILABLE_FIELDS neq NULL)}
							<select name="actions[{$ACTION_SEQUENCE}][parameters][{$parameterName}][{$fieldName}][valueformula]"
								...
								{if ($type neq BackgroundTaskParameterConfigurationInterface::PARAMETER_TYPE_SOURCE_FIELD)}
								...
								{foreach $AVAILABLE_FIELDS as $field}
									{if (!in_array ($field.uitype, array (Field::UI_TYPE_ATTACHMENTS, Field::UI_TYPE_GRID)))}
										<option value="{$field.fieldname}" {if ($value == $field.fieldname)} selected="selected" {/if}>
							...
							<select name="actions[{$ACTION_SEQUENCE}][parameters][{$parameterName}][{$fieldName}][valueformula]"
								...
								{if (!empty ($AVAILABLE_USERS))}
									...
									{foreach $AVAILABLE_USERS as $availableUser}
										<option value="{$availableUser->getId ()}" {if ($value == $availableUser->getId ())} selected="selected"
											{/if}>{trim("{$availableUser->getFirstName ()} {$availableUser->getLastName ()}")}</option>
									{/foreach}
								{else}

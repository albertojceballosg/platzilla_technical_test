	...
	<div class="condition-group-header list-group-item">
		<div class="row">
			...
			<div class="col-xs-1 text-right">
				<button type="button" class="btn btn-link" onclick="TaxUtils.deleteConditionGroup (this);" title="Eliminar grupo de condiciones"><i class="fa fa-trash-o"></i></button>
			</div>
		</div>
	</div>
...
{if (!empty ($groupConditions))}
	{foreach $groupConditions as $condition}
		{include file="modules/Taxes/ConditionEditView.tpl" CONDITION=$condition}
	{/foreach}
{/if}
		</ul>
	</div>
	<div class="condition-group-footer list-group-item">
		<div class="row text-center">
			<button type="button" class="btn btn-link" onclick="TaxUtils.addCondition (this);" title="Agregar condición"><i class="fa fa-plus"></i></button>
		</div>
	</div>
</div>
<div class="condition-group-operator">
	<select name="conditiongroups[{$groupId}][operator]" class="form-control operator{if (empty ($groupOperator))} hidden{/if}" title=""{if (empty ($groupOperator))} disabled="disabled"{/if}>
		<option value="{TaxConditionGroup::OPERATOR_AND}"{if ($groupOperator == TaxConditionGroup::OPERATOR_AND)} selected="selected"{/if}>y</option>
		<option value="{TaxConditionGroup::OPERATOR_OR}"{if ($groupOperator == TaxConditionGroup::OPERATOR_OR)} selected="selected"{/if}>o</option>
	</select>
</div>
{/strip}
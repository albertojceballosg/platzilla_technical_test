...
<div class="row" style="height: 1250px">
	{*$AD_QUEUES|var_dump*}
	<form role="form" id="ebook-bording-form"  {*action="index.php?module=store&action=CreateFormativeInstance" method="post" onsubmit="return BulletinBoardUtils.createFormativeInstance ();"*}>
		<input type="hidden" name="module" value="store"/>
		<input type="hidden" name="action" id="action-form-bording" value="SendEmailPassword"/>
		<input type="hidden" name="ebookId" id="ebookId" value="{if $EBOOK neq NULL}{$EBOOK->getId()}{else}0{/if}"/>
		<input type="hidden" name="ebookName" value="{if $EBOOK neq NULL}{$EBOOK->getPublicName()}{/if}"/>
		<input type="hidden" name="codeToken" id="code-token-bordig" value="{$PASS_WORD}">
		<input type="hidden" name="Ajax" value="true"/>
		<div class="row setup-content">
			<div class="col-xs-12">
                {include file="modules/store/EbookOnBoardForm.tpl"}
			</div>
		</div>

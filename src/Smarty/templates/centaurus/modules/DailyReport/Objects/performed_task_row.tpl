<tr numrowtr="{$key}" id="row_planned_activities-{$idRow}" class="tabla-field-row">
        ...
        <div class="row" style="padding-right: 1px;margin-right: 1px">
            <div class="col-md-12">
                <div class="form-group field-container" id="td_tarea{$idRow}">
                    <div class="input-group col-xs-12 {if $plannedTask->getActivity()->getActivityCondition() neq 'PLANNED_AND_RECORDED'}has-error{/if}" style="width: 100%;">
                        <input type="text" id="reported_task-display-{$idRow}" value="{if $plannedTask->getActivity()->getSubject() neq NULL}{$plannedTask->getActivity()->getSubject()}{else}Sin descripción{/if}               "
                        ...
                        <div class="input-group-addon" style="border:1px solid #dee2e6!important">
                            <a onclick="DailyReportUtils.openModal(this, event,'{$idProgressJob}')"
                               href="index.php?module=daily_report&action=AjaxEditViewUtils&function=FETCH_JOBS&rowid={$idRow}&Ajax=true" title="Seleccionar trabajo">
                                <i class="fa fa-plus-circle"></i>
                            </a>
                        </div>
                    </div>
                    <input type="hidden" id="reported_task-id-{$idRow}" name="planned_activities[reported_task_id][]"  value="{$plannedTask->getActivity()->getRelatedId()}" class="for-filter">
                    <input type="hidden" id="reported_task-module-{$idRow}" name="planned_activities[reported_task_module][]"  value="orden_de_trabajo" class="for-filter">
                </div>
            </div>
                ...
                <div class="col-md-12">
                    <div class="form-group field-container" id="td_tarea{$idRow}">
                        <div class="input-group col-xs-12 {if $plannedTask->getActivity()->getActivityCondition() neq 'PLANNED_AND_RECORDED'}has-error{/if}" style="width: 100%;">
                            ...
                            <div class="input-group-addon" style="border:1px solid #dee2e6!important">
                                <a onclick="DailyReportUtils.openModal(this, event,'{$idProgressJob}')"
                                   href="index.php?module=daily_report&action=AjaxEditViewUtils&function=DAILY_TASK_MATRIX&rowid={$idRow}&Ajax=true" title="Seleccionar tarea">
                                    <i class="fa fa-plus-circle"></i>
                                </a>
                            </div>
                        </div>
                        <input type="hidden" id="reported_task-id-{$idRow}" name="planned_activities[reported_task_id][]"  value="{$plannedTask->getActivity()->getActivityId()}" class="for-filter">
                        <input type="hidden" id="reported_task-module-{$idRow}" name="planned_activities[reported_task_module][]"  value="" class="for-filter">
                    </div>
                </div>
                <div class="col-md-4 hide"></div>
            </div>
    ...
    <td class="" id="td_planned_activities_task_progress_perc_{$idRow}_Campo1" style="vertical-align: top">
        <input autocomplete="off"
               onkeyup="DailyReportUtils.updateNumFields(this, '')"
                numrow="1"
               value="{$plannedTask->getProgress()}" name="planned_activities[task_progress_perc][]" id="task_progress_perc-{$idRow}"
    ...
    <td class="" id="td_planned_activities_informe_de_avance{$idRow}-Campo1" style="vertical-align: top">
        <textarea name="planned_activities[task_advanced_report][]" id="task_advanced_report-{$idRow}"
                  class="form-control daily-report-scroll tinymce-class edit-tinyMce">
            {foreach $plannedTask->getReport() as $report}
                {$report->getReport()}
            {/foreach}
        </textarea>
        <input type="hidden" id="task_advanced_report-id-{$idRow}" name="planned_activities[report_id][]" value="{$plannedTask->getReportIds()}" class="for-filter">
    ...
    <td class="" id="td_planned_activities_time_reported{$idRow}_Campo1" style="vertical-align: top">
        <input autocomplete="off"
               ...
               numrow="1" value="{$plannedTask->getDurationTime()}" name="planned_activities[time_reported][]" id="time_reported-{$idRow}"
               class="form-control numericvalidate performed-tasks-total-time" style="min-width:80px" type="text">
    </td>
    {* Acciones *}

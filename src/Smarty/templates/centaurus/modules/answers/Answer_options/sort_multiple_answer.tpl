...
{/if}
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12 col-xs-12">
            <div class="row">
                ...
                <div class="col-lg-6 col-md-6 col-xs-6">
                    <p style="margin-left: 15px">{$askingForSecuence}.&nbsp;{$askingForQuestion}</p>
                </div>
        ...
        <div class="col-lg-12 col-md-12 col-xs-12">
            <div class="col-lg-6 col-md-6 col-xs-6">
                ...
                <!--
                <div class="col-lg-12 col-md-12 col-xs-12">
                {if $askingForResponseOption neq NULL && false}
        ...
        <div class="col-lg-12 col-md-12 col-xs-12" style="padding-left: 20px">
            {if ($SUMMARY_TABLE neq NULL) && ($TOTAL_CATEGRIES neq NULL)}
                <table class="table table-striped table-bordered" style="width: 98%">
                    <thead>
                    <tr>
                        <td>Áreas de Aplicación </td>
                        {section name=bar start=1 loop=$TOTAL_CATEGRIES step=1}
                            <td style="text-align: center;width: 8%;"><small style="text-align: center">Importancia<br>{$smarty.section.bar.index}&nbsp;(%)</small></td>
                        {/section}
                    ...
                    {foreach $SUMMARY_TABLE as $rows}
                        {assign var="j" value=0}
                        {assign var="k" value=0}
                        <tr>
                            {foreach $rows['a'] as $rowA}
                                <td>
                                {if $j > 0}
                                    {$rowA|string_format:"%.2f"}
                                {else}
                                    {$rowA}
                        ...
                        <tr>
                            {foreach $rows['b'] as $rowB}
                                <td style="border-bottom: 1px solid #FF0000 !important;">
                                    {if $k > 0}
                                        {$rowB|string_format:"%.2f"}
                                    {else}
                                        {$rowB}
    ...
    <script type="text/javascript">
        ResponseEvaluationUtils.getStackedBars('{$GRAPHIC_DATA}', '{$askingForQuestion}');
    </script>
{/strip}
        ...
        <div class="col-lg-12" style="padding-right: 20px;height: 46px!important;">
                ...
                {if (in_array ($action, array ('ListView', 'index')))}
                            ...
                            <tr>
                                ...
                                <td style="vertical-align: top">
                                    <form action="index.php" id="how-to-use-form" class="form-inline" method="get">
                                        ...
                                        <select class="form-control input-sm pull-left" id="how-to-use"
                                                name="howusename"
                                                onchange="ListViewTabUtils.changeUseMode (this)">
                                            {foreach $AVAILABLE_MODES as $modes}
                                                <option value="{$modes->getId()}" {if $HOW_USE_ID eq $modes->getId()} selected{/if}>{$modes->getName()}</option>
                                            {/foreach}
                                        </select>
                                    </form>
                                </td>
                    ...
                    {else}
                        <h1 style="margin-left: 0">
                            {if ($CAN_CREATE_RECORDS)}<!-- wa 08-02 -->
                ...
                {elseif (in_array ($action, array ('DetailView', 'EditView'))) && (!empty ($ENTITY_IDENTIFIER_VALUE))}
                    {if (!$IS_MODAL)}
                        <h1>
                            ...
                            {if false}{* That is no requerided for this modeile*}
                                <a href="index.php?module={$MODULE}&action=EditView&return_action=DetailView&parenttab={$CATEGORY}{if $smarty.request.frontendsid}&frontendsid={$smarty.request.frontendsid}{/if}&mode=create"
                                   class="{*btn btn-success*}" style="margin-left:.5em; margin-right: 0;">
                                    <i class="fa fa-plus-circle "
                                       ...
                                       style="padding-right: 0.2em;"></i></a>
                            {else}
                                <samp style="padding-right: 0.2em;">&nbsp;</samp>
                            {/if}
                            ...
                            <small {if $ENTITY_IDENTIFIER_VALUE|strlen gte 38}class="protip"
                                   data-pt-title="{$ENTITY_IDENTIFIER_VALUE}"{/if}
                                   style="font-weight: bold"> {$ENTITY_IDENTIFIER_VALUE|truncate:38}</small>
                            {if (!empty ($TOTAL_SYNCS))}
                                <i class="fa fa-exchange"
                                   style="font-size: 0.5em; line-height: 1.1em; margin-left: 0.75em;"
                                   title="Compartido"></i>
            ...
            <div class="pull-right row" style="display: inline">
                ...
                {else} {* no, it is not  modal page on ListView, it is a detailView page *}
                    ...
                    {if $HIDDEN_GNL_BUTTON}
                        <div class="btn-group">
                            <ul class="nav nav-tabs nav-platzilla" id="detal-view-group-tab-{$idDetailView}" data-tab="{$SELECTED_TAB}">
                                ...
                                {if ($action eq 'DetailView')}
                                    ...
                                    {* Destination view*}
                                    <li {if ($SELECTED_TAB eq 'diagnostic-destination')}class="active" {/if}>
                                        <a data-toggle="tab" href="#tab-diagnostic-destination-view-{$idDetailView}"
                                           style=" font-size: 15px!important;"
                                           id="diagnostic-destination-btn-tab"
                                           title="Selección de destinos"
                                           onclick="DiagnosticRerportUtils.activeTabDestination (event, '{$MODULE}','{$idDetailView}', '{$ID}')">
                                            Destino
                                        </a>
                                    </li>
                                    {* Action plan view*}
                                    <li {if ($SELECTED_TAB eq 'diagnostic-plan')}class="active" {/if}>
                                        <a data-toggle="tab" href="#tab-diagnostic-plan-view-{$idDetailView}"
                                           style=" font-size: 15px!important;"
                                           id="diagnostic-plan-btn-tab"
                                           title="Selección de plan de acción"
                                           onclick="DiagnosticRerportUtils.activeTabActionPlan (event, '{$MODULE}','{$idDetailView}', '{$ID}')">
                                           Plan de acción
                                        </a>
                                    </li>
                                    {* Evolution view*}
                                    <li {if ($SELECTED_TAB eq 'diagnostic-evolution')}class="active" {/if}>
                                        <a data-toggle="tab" href="#tab-diagnostic-evolution-view-{$idDetailView}"
                                           style=" font-size: 15px!important;"
                                           id="diagnostic-evolution-btn-tab"
                                           title="Evolución de la empresa"
                                           onclick="{*DetailViewTabUtils.activeRelatedListTab (event)*}">
                                            Evolución
                                        </a>
                                    </li>
                                {/if}
                    ...
                    {else}
                        ...
                        {elseif (($action eq 'RecordHistory'))}
                            <ul class="nav nav-tabs nav-platzilla" id="detal-view-group-tab"

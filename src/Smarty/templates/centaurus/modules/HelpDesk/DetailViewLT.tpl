...
<head>
	...
	{if $PLAT_STYLE}
	<link rel="stylesheet" type="text/css" media="all" href="{$PLAT_STYLE}">
	{/if}
...
{literal}
	<style type="text/css">
		@import url("themes/globalstyle.css");
		...
		.col-left{width:680px;height:100%}
		.col-right{width:250px;margin-left:10px;height:100%}
	</style>
	<!--[if lte IE 6]>
...
{/literal}
	<!-- End -->
	<style type="text/css">
		{* Deprecated... debe eliminarse*}
		{foreach item=option key=k from=$COLORED_HEADERS}
			.tab_{$k|replace:' ':'_'} {ldelim}
				background: -webkit-gradient(linear, left top, left bottom, from({$option.from_color}), to({$option.to_color})) !important;
			{rdelim}
		{/foreach}
		{foreach item=color key=k from=$COLORED_HEADERS_PARENT}
			{if $color}
				.tab_{$k|replace:' ':'_'} {ldelim}
			...
			<table border=0 cellspacing=0 cellpadding=0 width=98% align=center>
				<tr>
								...
								<td>
									<table border=0 cellspacing=0 cellpadding=3 width=100% class="small">
										<tr>
											...
											{if ($SinglePane_View eq 'false' && $IS_REL_LIST neq false && $IS_REL_LIST|@count > 0) || ($SinglePane_View_custom eq 'true' && ($MODULE eq 'Accounts' || $MODULE eq 'promociones'))}
												<td class="dvtUnSelectedCell" onmouseout="fnHideDrop('More_Information_Modules_List');" onmouseover="fnDropDown(this,'More_Information_Modules_List');" align="center" nowrap>
													<a href="index.php?action=CallRelatedList&module={$MODULE}&record={$ID}&parenttab={$CATEGORY}&platdb={$PLATDB}">{$APP.LBL_MORE} {$APP.LBL_INFORMATION}</a>
													<div onmouseover="fnShowDrop('More_Information_Modules_List')" onmouseout="fnHideDrop('More_Information_Modules_List')"
														...
														<table border="0" cellpadding="0" cellspacing="0" width="100%">
															{foreach key=_RELATION_ID item=_RELATED_MODULE from=$IS_REL_LIST}
																<tr><td><a class="drop_down" href="index.php?action=CallRelatedList&module={$MODULE}&record={$ID}&parenttab={$CATEGORY}&selected_header={$_RELATED_MODULE}&relation_id={$_RELATION_ID}&platdb={$PLATDB}">{$_RELATED_MODULE|@getTranslatedString:$MODULE}</a></td></tr>
															{/foreach}
														</table>
													</div>
												</td>
					...
					echo vtlib_process_widget($this->_tpl_vars['CUSTOM_LINK_DETAILVIEWWIDGET'], $this->_tpl_vars);
																	...
																	<tr>
																		<td style="padding:5px;" >{$CAMPOS_PERSONALIZADOS}</td>
																	</tr>
																	{if $CAMPOS_TIPO_GRID neq ''}
																...
																{if ($SinglePane_View eq 'true' && $IS_REL_LIST|@count > 0) || $SinglePane_View_custom eq 'true'}
																	{include file= 'RelatedListNew.tpl'}
																{/if}
																<tr>
																<td>
																<div>															<!-- End the form related to detail view -->
																{$DETALLES_ADICIONALES_MODULO}
																</div>
																</td>
																</tr>
															</table>
														...
														{if $MODULE eq 'Documents'}
															<tr><td align="left" style="padding-left:10px;">
																	{if $DLD_TYPE eq 'I' && $FILE_STATUS eq '1' && $FILE_EXIST eq 'yes'}
																		<br><a href="index.php?module=uploads&action=downloadfile&fileid={$FILEID}&entityid={$NOTESID}&platdb={$PLATDB}"  onclick="javascript:dldCntIncrease({$NOTESID});" class="webMnu"><img src="{'fbDownload.gif'|@vtiger_imageurl:$THEME}" hspace="5" align="absmiddle" title="{$APP.LNK_DOWNLOAD}" border="0"/></a>
																		<a href="index.php?module=uploads&action=downloadfile&fileid={$FILEID}&entityid={$NOTESID}&platdb={$PLATDB}" onclick="javascript:dldCntIncrease({$NOTESID});">{$MOD.LBL_DOWNLOAD_FILE}</a>
									...
									<table border=0 cellspacing=0 cellpadding=3 width=100% class="small">
										<tr>
											...
											{if $SinglePane_View eq 'false' && $IS_REL_LIST neq false && $IS_REL_LIST|@count > 0}
												<td class="dvtUnSelectedCell" align=center nowrap><a href="index.php?action=CallRelatedList&module={$MODULE}&record={$ID}&parenttab={$CATEGORY}&platdb={$PLATDB}">{$APP.LBL_MORE} {$APP.LBL_INFORMATION}</a></td>
											{/if}

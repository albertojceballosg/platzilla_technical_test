...
{foreach key=num item=leccion from=$LECCIONES}
	...
	{if $leccion.url_video}
							<a href="#curso-completo" data-toggle="modal" onclick="irLeccion({$num});"><span class="label label-success" style="font-size: 12px;">Ver</a>
	...
	{if $leccion.eval}
		{foreach $leccion.eval as $evalu}
					<tr>
						...
						<td align="left">{$evalu.descripcion}</td>
						<td class="lec-link">
			{if $leccion.test eq '0'}
							<a href="#curso-completo" data-toggle="modal" onclick="realizarTest({$num});"><span class="label label-success" style="font-size: 12px; background-color: #8A2624">Test</a>
			{elseif $leccion.test eq '1'}
							<a href="#curso-completo" data-toggle="modal" onclick="realizarTest({$num});"><span class="label label-success" style="font-size: 12px; background-color: #8A2624">Repetir Test</a>
...
<div class="modal fullscreen-modal fade" tabindex="-1" id="curso-completo" role="dialog" aria-labelledby="gridSystemModalLabel">
		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">Formaci&oacute;n -
						<small>{$FIELDS.titulo}</small>
					</h4>
				</div>
...
{for $lec=0 to $num}
	...
	{elseif $LECCIONES[$lec].materiales && $LECCIONES[$lec].ext_arch eq 'pdf' && $LECCIONES[$lec].url_pagina eq '' }
										<div id="ma_pdf" class="mat_pdf "><embed src="{$LECCIONES[$lec].material}" width="90%" height="400"></embed></div>
	{elseif $LECCIONES[$lec].url_pagina}
		{if strpos($LECCIONES[$lec].url_pagina,'http://')}
										<div id="ma_url" class="mat_pag "><embed src="{$LECCIONES[$lec].url_pagina}" width="90%" height="800"></embed></div>
		{else}
										<div id="ma_url" class="mat_pag "><embed src="http://{$LECCIONES[$lec].url_pagina}" width="90%" height="800"></embed></div>
		{/if}
	{/if}
									</div>
	...
	{if $LECCIONES[$lec].introduccion}
										...
										<p></p>
										<p style="text-align: justify; margin-top: 5px">{$LECCIONES[$lec].introduccion|replace:'\r\n':'<br>'}</p>
										<hr>
	{/if}
	{if $LECCIONES[$lec].contenido}
										<h2 style="text-align: center">Contenido</h2>
										<p style="text-align: justify; margin-top: 5px">{$LECCIONES[$lec].contenido|replace:'\r\n':'<br>'}</p>
										<hr>
	{/if}
	{if $LECCIONES[$lec].actividades}
										<h2 style="text-align: center">Actividades</h2>
										<p style="text-align: justify; margin-top: 5px">{$LECCIONES[$lec].actividades|replace:'\r\n':'<br>'}</p>
										<hr>
	{/if}
								...
								<div class="main-box clearfix">
									<header class="main-box-header clearfix" style="text-align: center;">
										<div style="text-align: center">
											<h1>Prueba: {$LECCIONES[$lec].titulo}</h1><br>
											<hr>
										</div>
			...
			{/if}
										<form role="form" name="eval_{$lec}" id="eval_{$lec}">
											<div class="form-group">
												...
												<input type="hidden" id="tipo_prueba" name="tipo_prueba" value="{$evalu.tipo_prueba}">
												<input type="hidden" , id="idformacion_de_curso" name="idformacion_de_curso" value="{$FIELDS.record_id}">
											</div>
			{foreach key=np item=evapreg from=$LECCIONES[$lec].preg}
				{if $evapreg.tipo_de_pregunta==='Verdadero/Falso'}
											<div class="form-group">
				...
				{/if}
				{if $evapreg.tipo_de_pregunta==='Respuesta Multiple' || $evapreg.tipo_de_pregunta==='Multiple Choice' }
											<div class="form-group">
												<label><h2>{$evapreg.pregunta}</h2></label>
					{foreach $evapreg.respuestas as $evalu3}
												<div class="checkbox">
													...
													<label for="check_{$evalu3.id}"><h2>{$evalu3.respuesta}</h2></label>
												</div>
					{/foreach}
												<hr>
											</div>
	...
	<script type="text/javascript">
		...
		function irEnviarRespuesta (forma) {
			...
			new jQuery.ajax ({
				...
				url:  "index.php",
				data: {
					module:   "formacion_de_cursos",
					action:   "formacion_de_cursosAjax",
					file:     "ListViewCursos",
					save:     true,
					datapost: datapost
				}
		...
		function irEnviarRespuesta1 (forma) {
			var datapost = jQuery ('#' + forma).serialize ();
			new jQuery.ajax ({
				type: "POST",
				url:  "index.php",

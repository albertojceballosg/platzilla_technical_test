...
<div id="folders{$curso[0].formacion_cur_caid}" class="folder shadow-this">
	...
	<div class="col-lg-3" style="width: 33%;">
		<a href="index.php?module=formacion_de_cursos&action=ListViewCursos&record={$cur.formacion_de_cursosid}" alt="{$cur.titulo}" title="{$cur.titulo}">
			<div class="box box-solid box-{$cur.color}" style="height: 300px;">
				...
				<div class="box-body" style="font-size: 15px;font-family: 'Source Sans Pro', sans-serif;color:#444;">
					<p>{$cur.formacion_de_cursosid} {$cur.descripcion|truncate:80:"...":true}</p>
						...
						<div class="starRating">
							<div>
								<div>
									<div>
										<div>
											<input id="rating{$cur.formacion_de_cursosid}" type="radio" name="rating[{$cur.formacion_de__cursosid}]" value="1" disabled {if $cur.puntuacion >= 1}checked{/if}>
											<label for="rating{$cur.formacion_de_cursosid}"><span>1</span></label>
										</div>
										<input id="rating{$cur.formacion_de_cursosid}" type="radio" name="rating[{$cur.formacion_de_cursosid}]" value="2" disabled {if $cur.puntuacion >= 2}checked{/if}>
										<label for="rating{$cur.formacion_de_cursosid}"><span>2</span></label>
									</div>
									<input id="rating{$cur.formacion_de_cursosid}" type="radio" name="rating[{$cur.formacion_de_cursosid}]" value="3" disabled {if $cur.puntuacion >= 3}checked{/if}>
									<label for="rating{$cur.formacion_de_cursosid}"><span>3</span></label>
								</div>
								<input id="rating{$cur.formacion_de_cursosid}" type="radio" name="rating[{$cur.formacion_de_cursosid}]" value="4" disabled {if $cur.puntuacion >= 4}checked{/if}>
								<label for="rating{$cur.formacion_de_cursosid}"><span>4</span></label>
							</div>
							<input id="rating{$cur.formacion_de_cursosid}" type="radio" name="rating[{$cur.formacion_de_cursosid}]" value="5" disabled {if $cur.puntuacion >= 5}checked{/if}>
							<label for="rating{$cur.formacion_de_cursosid}"><span>5</span></label>
						</div>
					</div>
				</div>
			</div>
...
{foreach item=curso from=$CURSOS}
		...
		<div class="main-box clearfix project-box gray-box">
			<div class="main-box-body clearfix" id="user-profile">
				<div class="project-box-header gray-bg">
					<div class="name">
						<a href="index.php?module=formacion_de_cursos&action=ListViewCursos&record={$curso.formacion_de_cursosid}" title="{$curso.titulo}">{$curso.titulo|truncate:35:"...":true}</a>
					</div>
				</div>
				<div class="">
				...
				<div class="project-box-ultrafooter clearfix">
					...
					<div id="newsfeed">
						<div class="story-content pull-right" style="padding-top: 7px;padding-left: 0">
	...
	{else}
	<div class="col-lg-3">
		<a href="javascript:void(0)" onclick="OpenCloseFolder({$curso[0].formacion_de_cur_caid})" style="cursor:pointer;">

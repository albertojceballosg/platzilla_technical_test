				...
				{foreach item=rg2 key=k from=$TODOS_REGISTROS_GENERICOS}
						<div class="cd-timeline-block">
							...
							<div class="cd-timeline-content">
								<h2>[{$rg2.activityid}] {$rg2.subject}</h2>
								<p>{$rg2.status|@getTranslatedString:$MODULE}<br/>
								Estado: {$rg2.eventstatus|@getTranslatedString:$MODULE}<br/>
								Asignado a: {$rg2.user_name}
								</p>
								<div class="clearfix">
									<a class="btn btn-primary pull-right" href="index.php?action=DetailView&module=Calendar&record={$rg2.activityid}">Ver</a>
								</div>
	...
	if (jQuery('#graph-flot-donut').length) {ldelim}
		var dataDonut = [
			{foreach item=arrX key=key from=$TIPOS_GENERICOS}
				{ldelim}label:"{$arrX|@getTranslatedString:$MODULE}",data:{$REGISTROS_GENERICOS[$arrX]|@count}{rdelim},
			{/foreach}

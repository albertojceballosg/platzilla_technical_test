...
<li class="filter list-group-item" data-id="{$filterId}">
	<div class="row">
		<div class="col-xs-4">
			<select name="advancedfiltergroups[{$groupId}][filters][{$filterId}][column]" class="form-control filter-field" title="Campo" onchange="CustomViewUtils.setFilterField (this);">
...
{if (!empty ($AVAILABLE_COLUMNS))}
	...
	{foreach $AVAILABLE_COLUMNS as $columnName => $columnLabel}
        ...
        {continue}
		{/if}
		{if (in_array ($dummy[4], array ('I', 'N', 'NN')))}
...
{/if}
		...
		<div class="col-xs-3">
			<select name="advancedfiltergroups[{$groupId}][filters][{$filterId}][comparator]" class="form-control comparator" title="Operador" onchange="CustomViewUtils.setHelpToField (this);">
				<option value=""></option>
		...
		<div class="col-xs-3">
			<input type="text" name="advancedfiltergroups[{$groupId}][filters][{$filterId}][value]" value="{$filterValue}" class="form-control value" placeholder="" title="Valor" />
		</div>
		<div class="col-xs-1">
			<select name="advancedfiltergroups[{$groupId}][filters][{$filterId}][operator]" class="form-control operator{if (empty ($filterOperator))} hidden{/if}" title=""{if (empty ($filterOperator))} disabled="disabled"{/if}>

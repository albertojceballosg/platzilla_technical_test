...
<form name="paypal" id="paypal" action='modules/Invoice/expresscheckout.php' METHOD='POST' target="_top">
		<!-- <input type="hidden" name="module" value="Invoice">
		<input type="hidden" name="action" value="expresscheckout"> -->
		<input type="hidden" name="text_redirect" value="Redireccionando a PayPal">
		<input type="hidden" name="text_bad_amount" value="Valor Erroneo">
		<input type="hidden" name="text_bad_param" value="Parametro Invalido">
		<input type="hidden" name="text_bad_conexion" value="ERROR: Problema en la conexion a PayPal">
		<input type="hidden" name="text_item_label" value="empresa-facil.com: Pago de Servicio">
		<input type="hidden" name="Payment_Amount" value="{$Payment_Amount}">
		<input type="hidden" name="expType" value="instant">
		<input type="hidden" name="plat" value="{$PLATFORM}">
		<input type="hidden" name="record" value="{$ID}">
		<input type="image" src="https://www.paypal.com/en_US/i/btn/btn_xpressCheckout.gif" border="0" style="display:none" name="submit" alt="Go to PayPal">
					...
					<td>						
						   ...
						   <tr>
								...
								<td class="dvtTabCache" align="right" style="width:100%">
									{$CUSTOM_BUTTONS}
									{if $PAYPAL eq 'permitted'}
									<input title="PayPal" class="crmbutton small edit" onclick="J('#paypal').submit();" type="button" name="PayPal" value="&nbsp;PayPal&nbsp;" style="background-image: url(themes/images/PayPal.png);background-size: 28px;background-repeat: no-repeat;width: 80px;color: #000;text-align: right;background-color: #fff;">&nbsp;
									{/if}
									{if $EDIT_DUPLICATE eq 'permitted' && !($MODULE eq 'myinvoice' && $CLIENT_VIEW eq 'true')}
									...
									{/if}
									{if $DELETE eq 'permitted' && !($MODULE eq 'myinvoice' && $CLIENT_VIEW eq 'true')}
					...
					<td>						
						   ...
						   <tr>
								...
								<td class="dvtTabCacheBottom" align="right" style="width:100%">
									{if $EDIT_DUPLICATE eq 'permitted' && ($MODULE neq 'myinvoice' && $CLIENT_VIEW eq 'true')}
									<input title="{$APP.LBL_EDIT_BUTTON_TITLE}" accessKey="{$APP.LBL_EDIT_BUTTON_KEY}" class="crmbutton small edit" onclick="DetailView.return_module.value='{$MODULE}'; DetailView.return_action.value='DetailView'; DetailView.return_id.value='{$ID}';DetailView.module.value='{$MODULE}'; submitFormForAction('DetailView','EditView');" type="button" name="Edit" value="&nbsp;{$APP.LBL_EDIT_BUTTON_LABEL}&nbsp;">&nbsp;
									{/if}
									{if $EDIT_DUPLICATE eq 'permitted' && $MODULE neq 'Documents' && ($MODULE neq 'myinvoice' && $CLIENT_VIEW eq 'true')}
									<input title="{$APP.LBL_DUPLICATE_BUTTON_TITLE}" accessKey="{$APP.LBL_DUPLICATE_BUTTON_KEY}" class="crmbutton small create" onclick="DetailView.return_module.value='{$MODULE}'; DetailView.return_action.value='DetailView'; DetailView.isDuplicate.value='true';DetailView.module.value='{$MODULE}'; submitFormForAction('DetailView','EditView');" type="button" name="Duplicate" value="{$APP.LBL_DUPLICATE_BUTTON_LABEL}">&nbsp;
									{/if}
									{if $DELETE eq 'permitted' && ($MODULE neq 'myinvoice' && $CLIENT_VIEW eq 'true')}
									<input title="{$APP.LBL_DELETE_BUTTON_TITLE}" accessKey="{$APP.LBL_DELETE_BUTTON_KEY}" class="crmbutton small delete" onclick="DetailView.return_module.value='{$MODULE}'; DetailView.return_action.value='index'; {if $MODULE eq 'Accounts'} var confirmMsg = '{$APP.NTC_ACCOUNT_DELETE_CONFIRMATION}' {else} var confirmMsg = '{$APP.NTC_DELETE_CONFIRMATION}' {/if}; submitFormForActionWithConfirmation('DetailView', 'Delete', confirmMsg);" type="button" name="Delete" value="{$APP.LBL_DELETE_BUTTON_LABEL}">&nbsp;
									{/if}

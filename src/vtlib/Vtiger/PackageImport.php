...
class Vtiger_PackageImport extends Vtiger_PackageExport {
	...
	function initImport($zipfile, $overwrite) {
		...
		if($module != null) {
			...
			$unzip->unzipAllEx( ".",
				...
				Array(
					// Templates folder
					'templates' => "Smarty/templates/centaurus/modules/$module",
					// Cron folder
					'cron' => "cron/modules/$module",
					// module folder
					"modules/$module" => $plat."modules/$module",
				)
			);
	...
	function import_Module() {
		...
		$this->import_ModuleIncrementalCode($this->_modulexml, $moduleInstance);
	...
	function import_Tables($modulenode) {
		 ...
		 */
		$schemafile = fopen($_SESSION['plat']."/modules/$modulenode->name/schema.xml", 'w');
		if($schemafile) {
	 ...
	 * @access private
	 */
	function import_Grids($modulenode, $moduleInstance) {
		if(empty($modulenode->grids) || empty($modulenode->grids->grid)) return;
		foreach($modulenode->grids->grid as $gridnode) {
			// $gridInstance = $this->import_Grid($modulenode, $moduleInstance, $gridnode);
			$this->import_GridFields($gridnode, $moduleInstance);
		}
	}
	 ...
	 * @access private
	 */
	function import_ModuleIncrementalCode($modulenode, $moduleInstance) {
		if(empty($modulenode->modulecode)) return;

		global $adb;
		$adb->pquery("insert into vtiger_modentity_num (num_id, semodule, prefix, start_id, cur_id, active) values ((select id+1 from vtiger_modentity_num_seq), ?, ?, ?, ?, ?)",
		...
		$adb->pquery("update vtiger_modentity_num_seq set id=id+1");
	}
	 ...
	 * @access private
	 */
	function import_GridField ($gridnode, $moduleInstance, $fieldnode) {
		global $adb;

		$result = $adb->pquery ('SELECT * FROM vtiger_field WHERE fieldname=? AND tabid=? AND uitype=?', array ($gridnode->name, $moduleInstance->id, 2202));
		...
		$adb->pquery (
			"INSERT INTO vtiger_subfields (fieldid, name, label, sequence, uitype, length, `precision`, defaultvalue, `values`, relmodule) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
			array ($fieldId, $fieldnode->name, $fieldnode->label, $fieldnode->sequence, $fieldnode->uitype, $fieldnode->length, $fieldnode->precision, $fieldnode->defaultvalue, $values, $fieldnode->relmodule)
		);
	}

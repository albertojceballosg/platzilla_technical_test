...
class Vtiger_Link {
	 ...
	 */
	static function getAllByType($tabid, $type=false, $parameters=false, $relid = null, $parentid = null) {
		global $adb, $current_user;
		self::__initSchema();

		$multitype = false;
		$relatedListCondition = '';
		
		if ($relid > 0) {
			$relatedListCondition = " AND relatedlistid = ".$relid;
		}
		...
		if($type) {
			...
			if(is_array($type)) { 
				...
				if($tabid === self::IGNORE_MODULE) {
					...
					$sql.= $relatedListCondition;
					$result = $adb->pquery($sql, Array($adb->flatten_array($params)));
				} else {
					$sql = 'SELECT * FROM vtiger_links WHERE tabid=? AND linktype IN ('.
						Vtiger_Utils::implodestr('?', count($type), ',') .')';
					
					$params = array_merge(Array($tabid), $adb->flatten_array($type));
				...
				if($tabid === self::IGNORE_MODULE) {
					$result = $adb->pquery('SELECT * FROM vtiger_links WHERE linktype=?'.$relatedListCondition, Array($type));
				} else {
					$result = $adb->pquery('SELECT * FROM vtiger_links WHERE tabid=? AND linktype=?'.$relatedListCondition, Array($tabid, $type));				
				}
			}
		} else {
			$result = $adb->pquery('SELECT * FROM vtiger_links WHERE tabid=?'.$relatedListCondition, Array($tabid));
		}

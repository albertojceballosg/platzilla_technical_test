...
class Vtiger_ModuleBasic {
	...
	function __create() {
		...
		$customized = isset ($this->customized) ? $this->customized : 1; // To indicate this is a Custom Module
		$tabSequence = isset ($this->tabsequence) ? $this->tabsequence : -1;
		...
		$adb->pquery("INSERT INTO vtiger_tab (tabid,name,presence,tabsequence,tablabel,modifiedby,
			...
			Array($this->id, $this->name, $this->presence, $tabSequence, $this->label, NULL, NULL, $customized, $this->ownedby, $this->version,$this->parent));
	...
	function initTables($basetable=false, $basetableid=false) {
		...
		Vtiger_Utils::CreateTable($this->basetable,"($this->basetableid INT PRIMARY KEY, FOREIGN KEY fk_1_$this->basetable($this->basetableid) REFERENCES vtiger_crmentity(crmid) ON DELETE CASCADE ON UPDATE CASCADE)",true);
		Vtiger_Utils::CreateTable($this->customtable,
			"($this->basetableid INT PRIMARY KEY, FOREIGN KEY fk_2_$this->customtable($this->basetableid) REFERENCES vtiger_crmentity(crmid) ON DELETE CASCADE ON UPDATE CASCADE)", true);

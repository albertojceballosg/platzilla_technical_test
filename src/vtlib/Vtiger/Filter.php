...
class Vtiger_Filter {
	...
	function __create($moduleInstance) {
		...
		$adb->pquery (
			'INSERT INTO vtiger_profile2customview (profileid, cvid, tabid, permissions) SELECT p.profileid, ?, ?, 0 FROM vtiger_profile p',
	...
	function __delete() {
		global $adb;
		$adb->pquery ('DELETE FROM vtiger_profile2customview WHERE cvid=?', array ($this->id));
	...
	function addField($fieldInstance, $index = 0) {
		...
		/*$adb->pquery("UPDATE vtiger_cvcolumnlist SET columnindex=columnindex+1 WHERE cvid=? AND columnindex>=? ORDER BY columnindex DESC",
			Array($this->id, $index));*/
		//obtener ultimo valor de index guardado
		$query = $adb->pquery("SELECT columnindex FROM vtiger_cvcolumnlist WHERE cvid = ? order by columnindex DESC LIMIT 1 ", Array($this->id));
		if ($adb->num_rows($query) > 0){
			$index = $adb->query_result($query,0,'columnindex') + 1;
		}
	...
	static function deleteForModule($moduleInstance) {
		...
		if($adb->num_rows($cvidres)) {
			$adb->pquery ('DELETE FROM vtiger_profile2customview WHERE tabid=?', array ($moduleInstance->id));
			$cvids = Array();

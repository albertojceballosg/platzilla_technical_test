<?php
	...
	if (($delete) && (!empty ($recordId))) {
		$row    = IndicatorsPanelHelper::getDataIdRel ($adb, $recordId);
		$result = $adb->pquery ('DELETE FROM vtiger_box_score_data WHERE box_score_dataid IN (?,?)', array ($recordId, $row['box_score_dataid']));
		if ($result) {
			$result = $adb->pquery ('DELETE FROM vtiger_user2boxscore WHERE boxscorename=? AND userid=?', array ($row['name'], $current_user->id));
			echo 'delete_on';
		} else {
			echo 'delete_off';
		}

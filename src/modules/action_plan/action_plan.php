<?php
	...
	class action_plan extends CRMEntity {
		public $db;
		public $log;
		public $table_name = 'vtiger_action_plan';
		public $table_index = 'action_planid';
		...
		public $customFieldTable = array ('vtiger_action_plancf', 'action_planid');
		/** @var array Mandatory for Saving, Include tables related to this module. */
		public $tab_name = array ('vtiger_crmentity', 'vtiger_action_plan', 'vtiger_action_plancf');
		/** @var array Mandatory for Saving, Include tablename and tablekey columnname here. */
		public $tab_name_index = array (
			'vtiger_crmentity'         => 'crmid',
			'vtiger_action_plan'   => 'action_planid',
			'vtiger_action_plancf' => 'action_planid',
		);
		/**
		...
		public $list_fields = array (
			'Código' => array ('action_plan', 'cod_action_plan'),
			'Nombre' => array ('action_plan', 'action_plan_name'),
			'Tipo de negocio' => array ('action_plan', 'business_type'),
			'Etapa actual de la empresa' => array ('action_plan', 'business_phase'),
			'Resumen' => array ('action_plan', 'plan_summary')
		);
		/**
		...
		public $list_fields_name = array (
			'Código' => 'cod_action_plan',
			'Nombre' => 'action_plan_name',
			'Tipo de negocio' => 'business_type',
			'Etapa actual de la empresa' => 'business_phase',
			'Resumen' => 'plan_summary'
		);
		/**
		 * @var string Make the field link to detail view from list view (Fieldname)
		 * $list_link_field = 'payslipname';
		 */
		public $list_link_field = 'action_plan_name';
        
        /**
         * @var string Make the field link to detail view from list view alwys set
         * $defaultListLink = '';
         */
        public $defaultListLink = 'cod_action_plan';
		 ...
		 */
		public $popup_fields = array ('action_plan_name');
		 ...
		 */
		public $mandatory_fields = array ('action_plan_name');

		public $fieldsAlwaysVisible = array (
			'cod_action_plan',
			'plan_duration',
			'plan_status',
			'assigned_user_id',
		...
		/**
		 * @param array $conditionedFieldIds
		 *
		 * @throws Exception
		 */
		public function hideFields (&$conditionedFieldIds) {
			$fieldsName = $this->db->sql_expr_datalist ($this->fieldsAlwaysVisible);
			$result     = $this->db->query (
				"SELECT
				f.fieldid
			FROM
				vtiger_field f
				INNER JOIN vtiger_tab t ON t.tabid=f.tabid AND t.name='action_plan'
			WHERE
				f.fieldname NOT IN {$fieldsName}"
			);
		...
		public function save_module ($module) {
			...
			try {
				KeyObjectiveResult::getInstance ($adb)->saveKeyObjectiveResult ($this->id, $this->mode);
			} catch (Exception $e) {

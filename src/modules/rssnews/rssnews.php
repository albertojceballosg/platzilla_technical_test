<?php
	...
	class rssnews extends CRMEntity {
		public $db = null;
		public $log;
		// @codingStandardsIgnoreStart
		/** @var string $table_name */
		public $table_name = 'vtiger_rssnews';
		/** @var string $table_index */
		public $table_index = 'rssnewsid';
		/** @var array $column_fields */
		public $column_fields = array ();
		/** @var boolean $IsCustomModule Indicator if this is a custom module or standard module */
		public $IsCustomModule = true;
		/** @var array $tab_name Mandatory for Saving, Include tables related to this module. */
		public $tab_name = array ('vtiger_crmentity', 'vtiger_rssnews');
		/** @var array $tab_name_index Mandatory for Saving, Include tablename and tablekey columnname here. */
		public $tab_name_index = array (
			'vtiger_crmentity' => 'crmid',
			'vtiger_rssnews'   => 'rssnewsid',
		);
		/** @var array $list_fields Mandatory for Listing (Related listview) */
		public $list_fields = array (
			'LBL_RSSNEWS_FIELD_CODE'             => array ('rssnews', 'code'),
			'LBL_RSSNEWS_FIELD_HEADLINE'         => array ('rssnews', 'headline'),
			'LBL_RSSNEWS_FIELD_KEYWORDS'         => array ('rssnews', 'keywords'),
			'LBL_RSSNEWS_FIELD_MEDIA'            => array ('rssnews', 'media'),
			'LBL_RSSNEWS_FIELD_PUBLICATION_DATE' => array ('rssnews', 'publicationdate'),
			'LBL_RSSNEWS_FIELD_URL'              => array ('rssnews', 'url'),
		);
		/** @var array $list_fields_name Format: Field Label => fieldname */
		public $list_fields_name = array (
			'LBL_RSSNEWS_FIELD_PUBLICATION_DATE' => 'publicationdate',
			'LBL_RSSNEWS_FIELD_MEDIA'            => 'media',
			'LBL_RSSNEWS_FIELD_HEADLINE'         => 'headline',
		);
		/** @var string $list_link_field Make the field link to detail view from list view (Fieldname) */
		public $list_link_field = 'headline';
		/** @var array $search_fields For Popup listview and UI type support. Format: Field Label => array (tablename, columnname). tablename should not have prefix 'vtiger_' */
		public $search_fields = array (
			'LBL_RSSNEWS_FIELD_PUBLICATION_DATE' => array ('rssnews', 'publicationdate'),
			'LBL_RSSNEWS_FIELD_MEDIA'            => array ('rssnews', 'media'),
			'LBL_RSSNEWS_FIELD_HEADLINE'         => array ('rssnews', 'headline'),
		);
		/** @var array $search_fields_name Format: Field Label => fieldname */
		public $search_fields_name = array (
			'LBL_RSSNEWS_FIELD_CODE'     => 'code',
			'LBL_RSSNEWS_FIELD_HEADLINE' => 'headline',
			'LBL_RSSNEWS_FIELD_MEDIA'    => 'media',
			'LBL_RSSNEWS_FIELD_URL'      => 'url',
		);
		/** @var array $popup_fields For Popup window record selection */
		public $popup_fields = array ('publicationdate', 'media', 'headline');
		/** @var array $sortby_fields Placeholder for sort fields - All the fields will be initialized for Sorting through initSortFields */
		public $sortby_fields = array ();
		/** @var string $def_basicsearch_col For Alphabetical search */
		public $def_basicsearch_col = 'publicationdate';
		/** @var string $def_detailview_recname Column value to use on detail view record text display */
		public $def_detailview_recname = 'headline';
		/** @var array $required_fields Required Information for enabling Import feature */
		...
		/** @var array $special_functions Callback function list during Importing */
		public $special_functions = array ('set_import_assigned_user');
		/** @var string $default_order_by */
		public $default_order_by = 'publicationdate';
		/** @var string $default_sort_order */
		public $default_sort_order = 'DESC';
		/** @var array $mandatory_fields Used when enabling/disabling the mandatory fields for the module. */
		public $mandatory_fields = array ('headline', 'media', 'publicationdate', 'url');
		 ...
		 * @throws Exception
		 */
		public function undo_import ($module, $userId) {
			global $adb;
			$count  = 0;
			$sql    = "SELECT bean_id FROM vtiger_users_last_import WHERE assigned_user_id=? AND bean_type='$module' AND deleted=0";
			$result = $adb->pquery ($sql, array ($userId));
			if (!$result) {
				throw new Exception ('Error getting last import for undo');
			}
			while ($row = $adb->fetchByAssoc ($result)) {
				$sqlUpdate = 'UPDATE vtiger_crmentity SET deleted=1 WHERE crmid=?';
				if (!$adb->pquery ($sqlUpdate, array ($row ['bean_id']))) {
		...
		public function save ($moduleName, $fileId = '') {
			if (!$this->column_fields ['assigned_user_id']) {
				$this->column_fields ['assigned_user_id'] = $_SESSION ['authenticated_user_id'];
			}
			parent::save ($moduleName, $fileId);
		}

...
class Vtiger_DependencyPicklist {
	...
	static function getPicklistDependencyDatasource($module) {
		...
		// Usar PicklistRelationshipManager para leer de vtiger_picklist2picklist y vtiger_master_picklist_relationship
		$relationshipManager = PicklistRelationshipManager::getInstance($adb);
		$relationships = $relationshipManager->fetchPicklistRelationshipByModule($module);
		
		if (!empty($relationships)) {
			foreach ($relationships as $relationship) {
				$sourceField = $relationship->getMotherPicklistName();
				$targetField = $relationship->getDaughterPicklistName();
				$masters = $relationship->getPicklistRelationshipMaster();
				
				if (!empty($masters)) {
					foreach ($masters as $master) {
						$relationshipValues = $master->getRelationshipValues();
						if (!empty($relationshipValues)) {
							foreach ($relationshipValues as $sourceValue => $targetValues) {
								$picklistDependencyDatasource[$sourceField][$sourceValue][$targetField] = $targetValues;
							}
						}
					}
				}
				
				// Agregar valores por defecto (__DEFAULT__)

...
function getUserFldArray($fld_module,$roleid){
	...
	$query="select vtiger_field.fieldlabel,vtiger_field.columnname,vtiger_field.fieldname, vtiger_field.uitype, vtiger_field.generatedtype" .
...
function getAssignedPicklistValues($tableName, $roleid, $adb, $lang=array()){
	...
	if($adb->num_rows($result)){
		...
		$sql = "SELECT distinct ".$adb->sql_escape_string($tableName)." FROM ". $adb->sql_escape_string("vtiger_$tableName")
				...
				. ' and roleid in ('.generateQuestionMarks($roleids).') order by ' . $adb->sql_escape_string("vtiger_$tableName") . '.' . $tableName.'id';

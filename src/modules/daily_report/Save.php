<?php
	...
	// Validar unicidad de informe diario por fecha y usuario
	$dailyReportDate = isset ($focus->column_fields ['daily_report_date']) ? $focus->column_fields ['daily_report_date'] : null;
	$ownerId = isset ($focus->column_fields ['assigned_user_id']) ? $focus->column_fields ['assigned_user_id'] : null;
	if (!empty ($dailyReportDate) && !empty ($ownerId)) {
		$excludeRecordId = ($mode == 'edit') ? $record : null;
		$existingRecord = $focus->ValidateReportDate ($ownerId, $currentModule, $dailyReportDate, $excludeRecordId);
		if (!empty ($existingRecord)) {
			$_SESSION ['flashmessage'] = array (
				'iserror' => true,
				'message' => 'Ya existe un informe diario para esta fecha y usuario. No se puede crear un duplicado.',
			);
			header ("Location: index.php?module=$currentModule&action=EditView&record=$existingRecord&parenttab=$parentTab");
			exit();
		}
	}

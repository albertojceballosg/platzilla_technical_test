<?php
	...
	require_once ('modules/daily_report/handlers/DailyReportActivities.class.php');
	require_once ('modules/daily_report/handlers/progressOfWork.class.php');

	class daily_report extends CRMEntity {
		public $db;
		public $log;
		public $table_name = 'vtiger_daily_report';
		public $table_index = 'daily_reportid';
		...
		public $customFieldTable = array ('vtiger_daily_reportcf', 'daily_reportid');
		/** @var array Mandatory for Saving, Include tables related to this module. */
		public $tab_name = array ('vtiger_crmentity', 'vtiger_daily_report', 'vtiger_daily_reportcf');
		/** @var array Mandatory for Saving, Include tablename and tablekey columnname here. */
		public $tab_name_index = array (
			'vtiger_crmentity'         => 'crmid',
			'vtiger_daily_report'   => 'daily_reportid',
			'vtiger_daily_reportcf' => 'daily_reportid',
		);
		/**
		...
		public $list_fields = array (
			'Código' => array ('daily_report', 'cod_daily_report'),
			'Título' => array ('daily_report', 'daily_report_title'),
			'Colaborador' => array ('daily_report', 'user_name'),
			'Jornada laboral' => array ('daily_report', 'workday_size'),
			'Fecha' => array ('daily_report', 'daily_report_date'),
			'Total horas reportadas' => array ('daily_report', 'total_hours_reported'),
			'Distribución del tiempo laboral' => array ('daily_report', 'time_distribution'),
			'Estado' => array ('daily_report', 'daily_report_status')
		);
		/**
		...
		public $list_fields_name = array (
			'Código' => 'cod_daily_report',
			'Título' => 'daily_report_title',
			'Colaborador' => 'user_name',
			'Jornada laboral' => 'workday_size',
			'Fecha' => 'daily_report_date',
			'Total horas reportadas' => 'total_hours_reported',
			'Distribución del tiempo laboral' => 'time_distribution',
			'Estado' => 'daily_report_status'
		);
		/**
		 * @var string Make the field link to detail view from list view (Fieldname)
		 * $list_link_field = 'payslipname';
		 */
		public $list_link_field = 'cod_daily_report';
        
        /**
         * @var string Make the field link to detail view from list view alwys set
         * $defaultListLink = '';
         */
        public $defaultListLink = 'cod_daily_report';
		 ...
		 */
		public $popup_fields = array ('cod_daily_report');
		 ...
		 */
		public $mandatory_fields = array ('cod_daily_report');
		...
		public function save_module ($module) {
			...
			try {
				DailyReportActivities::getInstance ($adb)->buildupDailyReport ($current_user->id, $this->id, $this->column_fields ['cod_daily_report'], $this->mode);
				ProgressOfWork::getInstance ($adb)->buildProgressOfWork ($current_user->id, $this->id, $this->mode);
			} catch (Exception $e) {
		...
		public function setPreloadData ($current_user, $moduleName, $focus) {
			
			$appFieldParameters = (isset ($_REQUEST['afp'])) ? $_REQUEST ['afp'] : null;
			
			if (!empty ($appFieldParameters)) {
				$parameter = base64_decode ($appFieldParameters);
				$dummy     = explode ('@', $parameter);
				if (preg_match('/^\d{2}-\d{2}-\d{4}$/', $dummy[0])) {
					$dateParts = explode ('-', $dummy[0]);
					$dummy[0]  = "{$dateParts[2]}-{$dateParts[1]}-{$dateParts[0]}";
				}
				
				$this->column_fields['daily_report_date'] = $dummy [0];
				if ($focus->mode == 'create') {
					$record = $this->ValidateReportDate ($current_user->id, $moduleName, $dummy[0]);
					if (!empty ($record)) {
						$focus->id = $record;
						$focus->mode = 'edit';
			...
			// Solo establecer workday_size del usuario si es un informe nuevo (no tiene valor guardado)
			if (empty($this->column_fields['workday_size'])) {
				$this->column_fields['workday_size'] = $current_user->working_hours;
			}
		}
		
		/**
		 * Valida si ya existe un informe diario para la fecha y usuario especificados.
		 *
		 * @param integer $userId
		 * @param string $moduleName
		 * @param string $date
		 * @param integer|null $excludeRecordId ID del registro a excluir de la validación (para edición)
		 *
		 * @return mixed|null
		 * @throws Exception
		 */
		public function ValidateReportDate ($userId, $moduleName, $date, $excludeRecordId = null) {
			$params = array (0, $moduleName, $date, $userId);
			$excludeClause = '';
			if (!empty($excludeRecordId)) {
				$excludeClause = ' AND vtiger_crmentity.crmid != ?';
				$params[] = $excludeRecordId;
			}
			$result = $this->db->pquery (
				"SELECT vtiger_crmentity.crmid
				 FROM vtiger_crmentity
				 INNER JOIN vtiger_daily_report ON vtiger_daily_report.daily_reportid = vtiger_crmentity.crmid
				 WHERE
				     vtiger_crmentity.deleted = ? AND
				     vtiger_crmentity.setype = ? AND
				     vtiger_daily_report.daily_report_date = ? AND

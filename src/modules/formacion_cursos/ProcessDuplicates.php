...
if($mode == 'mergesave') {
	...
	if ($record_count == 2) {
		if (
			isPermitted($currentModule,'EditView',$exploded_id[0]) == 'yes' &&
			isPermitted($currentModule,'EditView',$exploded_id[1]) == 'yes' &&
			isPermitted($currentModule,'Delete',$exploded_id[0]) == 'yes' &&
			isPermitted($currentModule,'Delete',$exploded_id[1]) == 'yes'
		) {
		...
		if (
			isPermitted($currentModule,'EditView',$exploded_id[0]) == 'yes' &&
			isPermitted($currentModule,'EditView',$exploded_id[1]) == 'yes' &&
			isPermitted($currentModule,'EditView',$exploded_id[2]) == 'yes' &&
			isPermitted($currentModule,'Delete',$exploded_id[0]) == 'yes' &&
			isPermitted($currentModule,'Delete',$exploded_id[1]) == 'yes' &&
			isPermitted($currentModule,'Delete',$exploded_id[2]) == 'yes'
		) {
			$smarty->assign('EDIT_DUPLICATE', 'permitted');
		}
	}

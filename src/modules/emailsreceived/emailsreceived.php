<?php
	...
	class emailsreceived extends CRMEntity {
		public $db;
		public $log;
		public $table_name    = 'vtiger_emailsreceived';
		public $table_index   = 'emailsreceivedid';
		...
		public $customFieldTable = array ('vtiger_emailsreceivedcf', 'emailsreceivedid');
		/** @var array Mandatory for Saving, Include tables related to this module. */
		public $tab_name = array ('vtiger_crmentity', 'vtiger_emailsreceived', 'vtiger_emailsreceivedcf');
		/** @var array Mandatory for Saving, Include tablename and tablekey columnname here. */
		public $tab_name_index = array (
			'vtiger_crmentity'        => 'crmid',
			'vtiger_emailsreceived'   => 'emailsreceivedid',
			'vtiger_emailsreceivedcf' => 'emailsreceivedid',
		);
		/**
		...
		public $list_fields = array (
			'Subject' => array ('emailsreceived', 'subject'),
			'From'    => array ('emailsreceived', 'from_'),
			'To'      => array ('emailsreceived', 'to_'),
			'CC'      => array ('emailsreceived', 'cc'),
			'BCC'     => array ('emailsreceived', 'bcc'),
			'Date'    => array ('emailsreceived', 'maildate'),
			'Body'    => array ('emailsreceived', 'body'),
		);
		/**
		 ...
		 * 'Payslip Name'=> 'payslipname'
		 */
		public $search_fields_name = array ('Subject' => 'subject');
		/**
		...
		public function save_module () {
			...
			$setClauses = array ();
			$arguments  = array ();
			if (!empty ($this->column_fields ['uid'])) {
				$setClauses [] = 'uid=?';
				$arguments []  = $this->column_fields ['uid'];
			}
			if (!empty ($this->column_fields ['folder'])) {
				$setClauses [] = 'folder=?';
				$arguments []  = $this->column_fields ['folder'];
			}
			if (empty ($setClauses)) {
				return;
			}
			...
			$arguments [] = $this->id;
			$adb->pquery ("UPDATE vtiger_emailsreceived SET {$setClause} WHERE emailsreceivedid=?", $arguments);
		}

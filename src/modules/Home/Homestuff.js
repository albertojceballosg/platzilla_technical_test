...
Accion de cambiar el filtro de Actividades Recientes
*/
function getActivityRecent(value){	
	new Ajax.Request(
		'index.php',
		{queue: {position: 'end', scope: 'command'},
			method: 'post',
			postBody:'module=Home&action=HomeAjax&file=index&mode=getActivity&opc='+value,
			onComplete: function(response){				
				jQuery('#div_timeline').html(response.responseText)
			}
		}
	);
}
...
/* Modificado por Johana Romero
Pedido [TT11193] Adecuación Página Resumen de la aplicación - Platzilla 
Accion de cambiar el filtro de Indicadores Claves
*/
function getIndicadores(value){	
	new Ajax.Request(
		'index.php',
		{queue: {position: 'end', scope: 'command'},
			method: 'post',
			postBody:'module=Home&action=HomeAjax&file=index&mode=getIndicadores&opc='+value,
			onComplete: function(response){				
				jQuery('#div_indicadores').html(response.responseText)
			}
		}
	);
}

/* [TT11207] Ajustes Página Tours Platzilla - 11/07/16 - Johana Romero - Obtiene los modulos a partir de un index para el limit del sql */
function getInfo(index){
	...
	new Ajax.Request(
        ...
        {queue: {position: 'end', scope: 'command'},
                method: 'post',
                postBody: 'module=Home&action=HomeAjax&file=tour&mode=divmodulos&page='+index,
                onComplete: function(response) {
                	//jQuery('#test').empty();
            		jQuery('#test').html(response.responseText);      

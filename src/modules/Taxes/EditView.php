<?php
	...
	try {
		$taxId = PlatzillaUtils::purify ($_GET, 'record');
		if (isset ($_SESSION ['flashmessage']['data'])) {
			$tax = Tax::getInstance ();
			$tax->unserialize ($_SESSION ['flashmessage']['data']);
			unset ($_SESSION ['flashmessage']['data']);
		} else if (!empty ($taxId)) {
			$tax = TaxManager::getInstance ($adb)->fetchTax ($taxId);
		} else {
			$tax = null;
		}
	...
	$smarty->display ('modules/Taxes/EditView.tpl');

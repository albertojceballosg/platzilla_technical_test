...
function AddFolder()
{
	...
	if(fldrname.match(/[?]+$/) || fldrname.match(/[?]+/))
	{
		...
		return false;
               ...
               // var folder_id = getObj('folder_id').value = '';
                var folder_id = getObj('folder_id').value;
                console.log("el folder_id es " + folder_id);

                var mode = getObj('fldrsave_mode').value;
                if(mode == 'save')
                {
                        url ='&savemode=Save&foldername='+foldername+'&folderdesc='+folderdesc+'&folder_id='+folder_id;
                }

                console.log("el url es " + 'action=DocumentsAjax&mode=ajax&file=SaveFolder&module=Documents'+url);
                ...
                new Ajax.Request(
					...
					onComplete: function(response) {
					console.log("response es " + response.responseText);
					var item = response.responseText;
					jQuery('#status').hide();
					console.log(item);
					if(item.indexOf('Failure') > -1)
					...
					else
					{
						...
						jQuery('.md-overlay').attr('style','visibility:hidden');
						jQuery('#crear-carpeta').removeClass('md-show');
						var newfolder='<div class="col-lg-3 col-sm-6 col-xs-12 openfolder" onclick="openFolder('+val[1]+',\'\')">'+
											'<div class="main-box infographic-box">'+
												'<i class="fa fa-folder emerald-bg"></i>'+
												'<span class="headline">'+foldername+'</span>'+
												'<span class="value">'+
													'<span class="timer" data-from="0" data-to="0" data-speed="1000" data-refresh-interval="50">0</span>'+
												'</span>'+
											'</div>'+
										'</div>';
						jQuery('#folderContent').append(newfolder);
					}
                                }
                        }
                );
                ...
                {queue: {position: 'end', scope: 'command'},
                    ...
                    onComplete: function(response) {
                        ...
                        console.log(response);
                       	...
                       	if(item == 'NOT_PERMITTED') {
							alert(alert_arr.NOT_PERMITTED);
							return false;

						}else if(item == 'FAILURE_FILE')
                        {
                                alert(alert_arr.LBL_FOLDER_SHOULD_BE_EMPTY);
                                return false;
                                ...
                                alert(alert_arr.LBL_SUCCESS_WHILE_DELETING_FOLDER);
                                document.location='index.php?module=Documents&action=index';

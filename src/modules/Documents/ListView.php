<?php
	...
	global $adb, $app_strings, $current_user, $currentModule, $image_path, $mod_strings, $list_max_entries_per_page, $theme;
...
//Retreive the List View Table Header
	...
	if ($request_folderid == '' || $_REQUEST['foldermodule'] != '') {

		$sqldoc = "SELECT * FROM vtiger_seattachmentsrel vs
				INNER JOIN `vtiger_attachments` va ON va.`attachmentsid`=vs.`attachmentsid`
				INNER JOIN vtiger_crmentity crm ON crm.`crmid`=vs.`crmid`
				WHERE crm.`setype`!='Documents' AND crm.deleted=0";

		if ($_REQUEST['foldermodule'] && $_REQUEST['foldermodule'] != '') {
			$sqldoc .= " and crm.`setype`='" . $_REQUEST['foldermodule'] . "'";
		}
		$q = $adb->query ($sqldoc);
		while ($r = $adb->fetchByAssoc ($q)) {
			$r['isimage'] = false;
			if (preg_match ('/image/i', $r['type'])) {
				$r['isimage'] = true;
			}
			$ext                                 = substr ($r['name'], strrpos ($r['name'], '.') + 1);
			$ext                                 = strtolower ($ext);
			$r['ext']                            = $ext;
			$documentsbymodule[ $r['setype'] ][] = $r;
		}

		$smarty->assign ('MOD_FOLDERS', $documentsbymodule);
	}

	$foldersAux           = $folders;
	$foldersAux           = $focus->getFolders ($focus, $controller, $request_folderid);
	$permisologiaCarpetas = getPermisologiaPorPerfil (fetchUserProfileId ($current_user->id));

	$permisologiaDocumentos = getPermisologiaDocumento ();

	if ($request_folderid) {
		$folderPadre = getCarpetaPadreID ($request_folderid);
	...
	$smarty->assign ('FOLDERSAUX', $foldersAux);
	$smarty->assign ('PERMISOLOGIACARPETAS', $permisologiaCarpetas);
	$smarty->assign ('PERMISOLOGIADOCUMENTOS', $permisologiaDocumentos);
...
// END
	...
	if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] != '' || $_REQUEST['mode'] == 'ajax') {
		if ($_REQUEST['do'] == 'LIST_FOLDERS') {
			$html = $smarty->fetch ('modules/Documents/FoldersListView.tpl');
		} else {
			$smarty->assign ('FOLDERID_ACTIVE', $request_folderid);
			$html = $smarty->fetch ('modules/Documents/FoldersListView.tpl');
			$html .= $smarty->fetch ('modules/Documents/DocumentsListViewEntries.tpl');
		}
		$response = array ('success' => 'true', 'html' => $html, 'foldername' => $selectedfoldername);
		die(json_encode ($response));
	} else {
		$smarty->display ('modules/Documents/DocumentsListView.tpl');
	}

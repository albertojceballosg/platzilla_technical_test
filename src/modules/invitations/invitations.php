<?php
	...
	class invitations extends CRMEntity {
		public $db            = null;
		public $log;
		public $table_name    = 'vtiger_invitations';
		public $table_index   = 'invitationid';
		...
		public $tab_name = array ('vtiger_crmentity', 'vtiger_invitations');
		/** @var array Mandatory for Saving, Include tablename and tablekey columnname here. */
		public $tab_name_index = array (
			'vtiger_crmentity'   => 'crmid',
			'vtiger_invitations' => 'invitationid',
		);
		/** @var array Mandatory for Listing (Related listview) */
		public $list_fields = array (
			'LBL_INVITATIONS_FIELD_GUEST'  => array ('invitations', 'guest'),
			'LBL_INVITATIONS_FIELD_STATUS' => array ('invitations', 'invitationstatus'),
		);
		/** @var array Format: Field Label => fieldname */
		public $list_fields_name = array ('LBL_INVITATIONS_FIELD_GUEST' => 'guest');
		/** @var string Make the field link to detail view from list view (Fieldname) */
		public $list_link_field = 'guest';
		/** @var array For Popup listview and UI type support. Format: Field Label => array (tablename, columnname). tablename should not have prefix 'vtiger_' */
		public $search_fields = array ('LBL_INVITATIONS_FIELD_GUEST' => array ('invitations', 'guest'));
		/** @var array Format: Field Label => fieldname */
		public $search_fields_name = array ('LBL_INVITATIONS_FIELD_GUEST' => 'guest');
		/** @var array For Popup window record selection */
		public $popup_fields = array ('guest');
		...
		public function vtlib_handler ($moduleName, $eventType) {
			if ($eventType == 'module.postinstall') {
				require_once ("modules/$moduleName/lib/InvitationsInstaller.class.php");
				InvitationsInstaller::getInstance ()->setupDatabaseVariables ();
			} else if ($eventType == 'module.preuninstall') {
				require_once ("modules/$moduleName/lib/InvitationsInstaller.class.php");
				InvitationsInstaller::getInstance ()->tearDownDatabaseVariables ();
			}
		}

		public function validate ($arguments) {
			global $adb;
			$invitationId = $arguments ['record'];
			$guest        = $arguments ['guest'];
			$entityId     = $arguments ['entityid'];
			$sql          = 'SELECT
									i.invitationstatus
								FROM
									vtiger_invitations i
									INNER JOIN vtiger_crmentity crme ON crme.crmid=i.invitationid
								WHERE
									crme.deleted=0 AND
									i.guest=? AND
									i.entityid=? AND
									i.invitationid<>?';
			$result       = $adb->pquery ($sql, array ($guest, $entityId, $invitationId));
			if ($adb->num_rows ($result) > 0) {
				throw new Exception ('Ya existe una invitación para ese invitado y ese documento');
			}
			$result = $adb->pquery ('SELECT i.invitationstatus FROM vtiger_invitations i WHERE (i.invitationid=?)', array ($invitationId));
			$row    = $adb->fetch_array ($result);
			if ($row ['invitationstatus']) {
				throw new Exception ('No se puede modificar una invitación ya enviada');
			}
		}

		public function save ($module_name, $fileid = '') {
			require_once ('modules/invitations/lib/NotificationsManager.class.php');
			NotificationsManager::getInstance ()->sendInvitation ($this->id);
			...
			if ($this->column_fields ['invitationstatus']) {
				return;
			}
			require_once ('modules/invitations/lib/NotificationsManager.class.php');
			NotificationsManager::getInstance ()->sendInvitation ($this->id);
			require_once ('modules/invitations/lib/InvitationsManager.class.php');
			InvitationsManager::getInstance ()->updateStatus ($_SESSION ['plat'], $this->id, 'Enviada');
		}

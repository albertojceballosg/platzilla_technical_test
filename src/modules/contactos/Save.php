<?php
	...
	$returnTab       = (isset ($_REQUEST ['return_tab'])) && ($_REQUEST ['return_tab'] != '') ? vtlib_purify ($_REQUEST ['return_tab']) : null;
	...
	$tab = (!empty($returnTab)) ? "&tab={$returnTab}" : '';
	header ("Location: index.php?module=$returnModule&action=$returnAction&record=$returnId&parenttab=$parentTab&mode={$modeEvent}&start={$pageNumber}{$search}{$urlPlatDb}{$tab}");

...
function changeSteps()
{
	if(getObj('step1').style.display != 'none')
	{
		if (trim(document.NewRep.reportname.value) == "")
		{
			...
			return false;
		}if (document.NewRep.reportfolder.value == ""){
...
/*
* javascript functions ReportRun.tpl
*
*/
...
function selectReport (selectElement) {
	...
	if ((select.val () === null) || (select.val () === undefined) || (select.val ().trim () === '')) {
		return;
	}
	form.submit ();
}
...
function goToPrintCustomReport(id, file) {

	if(!checkAdvancedFilter()) return false;
	var advft_criteria = $('advft_criteria').value;
	var advft_criteria_groups = $('advft_criteria_groups').value;

	window.open("index.php?module=Reports&action=ReportsAjax&file="+file+"&record="+id+'&advft_criteria='+advft_criteria+'&advft_criteria_groups='+advft_criteria_groups,"{$MOD.LBL_Print_REPORT}","width=800,height=650,resizable=1,scrollbars=1,left=100");
}

<?php
	...
	if(vtlib_isModuleActive($oRep->primodule)==false) {
		...
		echo "<td style='border-bottom: 1px solid rgb(204, 204, 204);' nowrap='nowrap' width='70%'><span class='genHeaderSmall'>".$mod_strings['LBL_NO_ACCESS'].' : '.$oRep->primodule.' </span></td>';
		echo '</tr>';
		echo '<tr>';
		echo "<td class='small' align='right' nowrap='nowrap'>";
		echo "<a href='javascript:window.close();'>$app_strings[LBL_CLOSE]</a><br>								   		     </td>";
		echo '</tr>';
		echo '</tbody></table>';
		echo '</div>';
		echo '</td></tr></table>';
	...
	if(vtlib_isModuleActive($_REQUEST['reportmodule'])==false || isPermitted($_REQUEST['reportmodule'],'index')!= 'yes') {
		...
		/** @noinspection HtmlUnknownTarget */
		echo "<td rowspan='2' width='11%'><img src='themes/images/denied.gif'></td>";
		echo "<td style='border-bottom: 1px solid rgb(204, 204, 204);' nowrap='nowrap' width='70%'><span class='genHeaderSmall'>".$mod_strings['LBL_NO_ACCESS'].' : '.getTranslatedString($_REQUEST['reportmodule'],$_REQUEST['reportmodule']).' </span></td>';
		echo '</tr>';
		echo '<tr>';
		echo "<td class='small' align='right' nowrap='nowrap'>";
		echo "<a href='javascript:window.close();'>$app_strings[LBL_CLOSE]</a><br>								   		     </td>";

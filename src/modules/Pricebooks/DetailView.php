<?php
	...
	if (!empty ($_SESSION ['platInstancia'])) {
		...
		try {
			...
			if ((empty ($subscription)) || ($subscription->getStatus () == PlatformSubscription::STATUS_INACTIVE)) {
				throw new Exception ('Tu suscripción se encuentra inactiva. Te invitamos a revisar los detalles en la pestaña Suscripción de tu cuenta Platzilla');
			}
	...
	if (!empty ($record)) {
		...
		$chatMessages = array ();
		$result       = $adb->pquery ('SELECT * FROM vtiger_chat WHERE module = ? AND record = ?', array ($currentModule, intval ($entity->id)));
		if (($result) && ($adb->num_rows ($result) > 0)) {
			while ($row = $adb->fetchByAssoc ($result, -1, false)) {
				$chatMessages [] = $row;
			}
	...
	$smarty->assign ('CHATS', $chatMessages);
	$smarty->assign ('CHECK', $toolButtons);
	$smarty->assign ('CONDITION_GROUPS', PricebooksUtils::getConditionGroups ($adb, $entity->id));
	...
	$smarty->display ('modules/Pricebooks/DetailView.tpl');

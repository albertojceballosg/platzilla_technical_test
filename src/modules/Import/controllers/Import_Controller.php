 ...
 *************************************************************************************/
	...
	class Import_Controller {
		...
		/**
		 * @param Import_API_UserInput $userInputObject
		 * @param Users $user
		 *
		 * @return void
		 */
	...
	//			}
		...
		public function copyFromFileToDB() {
			...
			if ($fileReader->getStatus() == 'success') {
				...
				Import_Utils::showErrorPage (
					getTranslatedString ('ERR_FILE_READ_FAILED', 'Import').' - ' . getTranslatedString ($fileReader->getErrorMessage(), 'Import')
				);
				return false;
			}
		}

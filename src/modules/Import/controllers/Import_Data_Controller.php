 ...
 ************************************************************************************ */
	...
	//require_once 'vtlib/Vtiger/Mailer.php';  # Se quitó porque se eliminó la clase base en la depuración | MA | 16-02-2016
				...
				if ($fieldInstance->getFieldDataType() == 'owner') {
					...
					if (!empty($fieldValue)) {
						...
						if ((empty($entityId) || $entityId == 0) && !empty($referenceModuleName)) {
							$isPermitted = isPermitted($referenceModuleName, 'EditView');
							if($isPermitted == 'yes') {
					...
					if (!in_array($encodePicklistValue, $allPicklistValues) && false) {

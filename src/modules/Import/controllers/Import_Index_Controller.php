 ...
 *************************************************************************************/
	...
	class Import_Index_Controller {
		...
		/**
		 * Function to get the list of fields which are not editable
		...
		public static function loadBasicSettings ($userInputObject, $user) {
			...
			$importIndexController = new Import_Index_Controller();
			$viewer                = new Import_UI_Viewer ();
			
			if (isset ($_SESSION ['flashmessage'])) {
				$viewer->assign ('IS_ERROR', $_SESSION ['flashmessage']['iserror']);
				$viewer->assign ('MESSAGE', $_SESSION ['flashmessage']['message']);
			...
			$viewer->assign ('AVAILABLE_FIELDS', $importIndexController->getMergableFields ($moduleName));
			$viewer->assign ('ENTITY_FIELDS', $importIndexController->getEntityFields ($moduleName));
		...
		public static function validateFileUpload($userInputObject) {
			...
			if(!$fileCopied && !file_exists($temporaryFileName)) {
		...
		public function deleteMap($userInputObject, $user) {
			$adb = PearDatabase::getInstance();
			$moduleName = $userInputObject->get('module');
		...
		/**
		 * @param Import_API_UserInput $requestObject
		 * @param Users $user
		 *
		 * @return void
		 */
		public static function process ($requestObject, $user) {
			...
			if ($mode == 'undo_import') {
				...
				exit;
			} else if ($mode == 'discarded_records') {
				Import_ListView_Controller::discardedRecords ($requestObject, $user);
				$isPDF  = $requestObject->get('pdf');
				if ($isPDF) {
					return;
				} else {
					exit;
				}
			} else if ($mode == 'delete_map') {
				Import_Index_Controller::deleteMap ($requestObject, $user);
				exit;
			} else if ($mode == 'clear_corrupted_data') {
				Import_Utils::clearUserImportInfo ($user);
			} else if ($mode == 'cancel_import') {

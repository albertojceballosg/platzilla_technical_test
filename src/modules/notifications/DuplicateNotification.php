<?php
	...
	try {
		...
		if (empty ($recordId)) {
			throw new Exception ('No has suministrado el ID de la notificacion a duplicar');
		}

		$notification    = NotificationUtils::duplicateNotification ($adb, $recordId);
		...
		$smarty->assign ('RECORD', null);
		$smarty->assign ('FILTER_TYPE', NotificationUtils::getTypeOfData ());

...
if($mode == 'delete') {
	...
	if(!empty($_REQUEST['idlist'])) {
		...
		if(!empty($ids)) {
			$focus->duplicate_patron($currentModule, $forCRMRecord, $destinationModule, $ids);
		}
			 
}
header("Location: index.php?module=$module&record=$forCRMRecord&action=$action&parenttab=$parenttab");
?>
...
class Calendar
{
	...
	*/
	...
	public static function getRelatedEntities (PearDatabase $adb, $activityId) {
		if (empty ($activityId)) {
			return null;
		}
		...
		$result = $adb->pquery (
			...
			FROM
				vtiger_activity a
				INNER JOIN vtiger_seactivityrel sear ON sear.activityid=a.activityid
				
				INNER JOIN vtiger_crmentity crmrel ON crmrel.crmid=sear.crmid
				INNER JOIN vtiger_tab t ON t.name=crmrel.setype
				INNER JOIN vtiger_entityname en ON en.modulename=t.name
			WHERE
				a.activityid>0 AND
				a.activitytype<>'Emails' AND
				a.activityid=?
			ORDER BY
				a.date_start DESC
				LIMIT 1",
		...
		while ($row = $adb->fetchByAssoc ($result)) {
			$moduleLabel = getTranslatedString ($row ['tablabel']);
			$fieldName = str_replace (',', ",' - ',", $row ['fieldname']);
			$relatedResult = $adb->query ("SELECT CONCAT({$fieldName}) AS fieldvalue FROM {$row ['tablename']} WHERE {$row ['entityidcolumn']}='{$row ['crmid']}'");
			if ((!$relatedResult) || ($adb->num_rows ($result) == 0)) {
				$row ['label_entity'] = '';
			} else {
				$relatedEntity = $adb->fetchByAssoc ($relatedResult, -1, false);
				$row ['label_entity'] = "<a title=\"{$moduleLabel}\" href=\"index.php?module={$row ['name']}&parenttab=&action=DetailView&record={$row ['crmid']}\">{$relatedEntity ['fieldvalue']}</a>";
				$row ['url_redirect'] = "index.php?module={$row ['name']}&parenttab=&action=DetailView&record={$row ['crmid']}";
				$row ['redirect_name'] = $relatedEntity ['fieldvalue'];
			}

			$row ['name'] = $moduleLabel;
			$entities [] = $row;
		}

	...
	function readAppointment($userid, &$from_datetime, &$to_datetime, $view)
	{
		...
		$userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>
        	...
        	$q= "select vtiger_activity.*, vtiger_crmentity.*,
					...
					FROM vtiger_activity
						inner join vtiger_crmentity on (vtiger_activity.activityid = vtiger_crmentity.crmid and deleted = 0)
					...
					WHERE vtiger_activity.activitytype not in ('Emails','Task') $and ";
        ...
        while ( $a < $n )
        {
			...
			if ($result['date_start'] == '0000-00-00')
				$result['date_start'] = $result['due_date'];
			...
			for($j = $from; $j <= $to; $j=$j+(60*60*24))
			{
				...
				if (($to - $from) > (30*(60*60*24))) {
					break;//Actividades mal cargadas cuelgan el sistema
				}					

(function (jQuery) {
	...
	var setEditImportFieldFromModule = function (moduleName) {
		var templateContents = jQuery ('#grid-RelationShip-edit-template').html (),
			table            = modal.find ('#editGridImportValuesTable'),
		...
		if (listValue !== '') {
			...
			row.insertBefore (table.find ('.edit-import-bar'));
			modal.find ('.edit-import-bar').removeClass ('hide');
			modal.find ('#gridEditImportValues').removeClass ('hide');
			modal.find ('#importEditModuleReference').val (moduleName);
	...
	var setGridImportAction = function () {
		var templateContents                                        = jQuery ('#grid-RelationShip-edit-template').html (),
			table                                                   = modal.find ('#editGridImportValuesTable'),
			row, listFieldName, listFieldLabel, listValue, selected = '', f, op, k, moduleFieldValue, FieldValue, moduleAndField, moduleName, option;

		if (!templateContents) {
			return;
		}
		for (f = 0; f < gridProperties.typeField.length; f++) {
			if ((gridProperties.typeField[ f ] == 10) && (gridProperties.actionField[ f ] != null)) {
				moduleFieldValue = Object.values (gridProperties.actionField[ f ]);
				FieldValue = Object.keys (gridProperties.actionField[ f ]);
				break;
			}
		}
		moduleAndField = moduleFieldValue[ 0 ].split ('@');
		moduleName = moduleAndField[ 0 ];
		listValue = getFieldsFromModules (moduleName);
		if (listValue !== '') {
			listFieldName = Object.keys (listValue);
			listFieldLabel = Object.values (listValue);
			for (k = 0; k < moduleFieldValue.length; k++) {
				...
				option = row.find ('select').eq (0);
				for (op = 0; op < listFieldName.length; op++) {
					if (moduleAndField[ 1 ] == listFieldName[ op ]) {
						option.append (jQuery ('<option>', {
							value: listFieldName[ op ],
							text:  listFieldLabel[ op ]
						}).attr ('selected', true));
				...
				for (op = 0; op < gridProperties.typeField.length; op++) {
					if (jQuery.inArray (gridProperties.typeField[ op ], [ '1', '7', '9' ]) !== -1) {
						...
						if (FieldValue[ k ] == gridProperties.nameField[ op ]) {
							option.append (jQuery ('<option>', {
								value: gridProperties.nameField[ op ],
								text:  gridProperties.labelField[ op ]
							}).attr ('selected', true));
	...
	var addEditImportRelationship = function () {
		var table = modal.find ('#editGridImportValuesTable'),

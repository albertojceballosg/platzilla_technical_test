<?php
	...
	$parentProfileId = ($mode != 'create') || ($radioButton == 'baseprofile') ? SettingsUtils::purify ($_REQUEST, 'parentprofile') : '';
	...
	$smarty->assign ('PROFILE_DESCRIPTION', ($profileDescription) && ($mode == 'create') ? to_html ($profileDescription) : getProfileDescription ($profileId));
	$smarty->assign ('PROFILE_NAME', ($profileName) && ($mode == 'create') ? to_html ($profileName) : getProfileName ($profileId));
	...
	$smarty->display ($mode == 'view' ? 'ProfileDetailView.tpl' : 'EditProfile.tpl');

<?php
	...
	if (isset ($_SESSION ['flashmessage']['data'])) {
		$useCase = $_SESSION ['flashmessage']['data'];
		unset ($_SESSION ['flashmessage']['data']);
	} else if (!empty ($recordId)) {
		$useCase = HelpSettingsHelper::fetchHelpUseCase ($adb, $recordId);
	} else {
		$useCase = null;
	}

	$smarty->assign ('AVAILABLE_APPLICATIONS', HelpSettingsHelper::fetchApplications ($adb));
	$smarty->assign ('USE_CASE', $useCase);
	if (isset ($_SESSION ['flashmessage'])) {
		$smarty->assign ('IS_ERROR', $_SESSION ['flashmessage']['iserror']);
		$smarty->assign ('MESSAGE', $_SESSION ['flashmessage']['message']);
		unset ($_SESSION ['flashmessage']);
	}
	$smarty->display ('Settings/HelpSettingsUseCaseEditView.tpl');

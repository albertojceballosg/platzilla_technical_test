<?php
	...
	try {
		...
		$availableModules = GanttTaskUtils::fetchAvailableModules ($adb);
		if (empty($availableModules)) {
			throw new Exception ('No hay modulos disponibles');
		}
		if (!empty ($viewId)) {
			$kanbanTask = GanttTaskUtils::fecthGanttById ($adb, $viewId);
		}
		...
		$smarty->display ('Settings/GanttTaskEditView.tpl');
	} catch (Exception $e) {
		$smarty->assign ('LABEL', 'Volver');
		$smarty->assign ('MESSAGE', $e->getMessage ());
		$smarty->assign ('TYPE', 'ERROR');
		$smarty->assign ('URL', 'index.php?module=Settings&action=KanbanViewListView&parenttab=Settings&tab=gantt-task');

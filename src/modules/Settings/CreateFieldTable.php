<?php
	...
	try {
		...
		$tableName    = TableFieldHelper::getVtigerTableName ($adb, $moduleName);
		$idColumnName = "{$moduleName}tfid";
		
		DatabaseUtils::createModuleTableIfNotExists ($adb, $tableName, $idColumnName);
		
		$fields[] = Field::getInstance ('X~O')
			->setId (null)
			->setDeleted (false)
		...
		TableFieldHelper::changeKeyToIndex ($adb, $tableName, $idColumnName);
		TableFieldHelper::buildTableField ($adb, $fieldTableName, $buildData, 'CREATE');
		TableFieldHelper::buildTableFieldData ($adb, $buildData, $tableName, $summaryRow);

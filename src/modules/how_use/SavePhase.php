<?php
	...
	try {
		...
		$sector = CompanyPhase::getInstance ()
			->setName ($name)
			->setDescription ($description)
			->setId ($record);
		HowToUseHelper::saveCompanyPhase ($adb, $sector);
		$_SESSION ['flashmessage'] = array (
			'iserror' => false,
			'message' => (!empty($record)) ? 'Se ha actualizado la fase de desarrollo' : 'Se ha creado la fase de desarrollo!',
		);
	} catch (Exception $e) {
		$_SESSION ['flashmessage'] = array (
			'iserror' => true,
	...
	header ("Location: index.php?module={$returnModule}&action={$returnAction}&tab=company_phase&parenttab=Settings");

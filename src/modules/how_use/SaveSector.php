<?php
	...
	try {
		...
		$sector = CompanySector::getInstance ()
			->setName ($name)
			->setDescription ($description)
			->setId ($record);
		HowToUseHelper::saveCompanySector ($adb, $sector);
		$_SESSION ['flashmessage'] = array (
			'iserror' => false,
			'message' => (!empty($record)) ? 'Se ha actualizado el sector' : 'Se ha creado el sector!',
		);
	} catch (Exception $e) {
		$_SESSION ['flashmessage'] = array (
			'iserror' => true,
	...
	header ("Location: index.php?module={$returnModule}&action={$returnAction}&tab=company_sector&parenttab=Settings");

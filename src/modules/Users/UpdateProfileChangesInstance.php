...
foreach($modArr as $fld_module => $fld_label)
{
	...
	for($i=0; $i<$noofrows; $i++)
	{
		...
		$adb->pquery($update_query, array((int)$visible_value, (int)$readOnlyValue, $fieldid, $profileid, $tab_id));

	}
	
	//Se actualizan los permisos de los usuarios que estan dentro del profile
	$sql = "SELECT id FROM vtiger_users INNER JOIN vtiger_user2role ON (id = userid) INNER JOIN vtiger_role2profile USING(roleid)
				WHERE profileid = ?";
	...
	header("Location: index.php?action=".$return_action."&module=Settings&mode=view&parenttab=Settings&profileid=".$profileid."&selected_tab=".$def_tab."&selected_module=".$def_module);

<?php
	...
	$emailwelcome = obtenerValorVariable ('EMAIL_BIENVENIDA_USERS_CODE', 'Users');
	...
	if (((empty ($_SESSION ['Users_FORM_TOKEN'])) || ($_SESSION ['Users_FORM_TOKEN'] !== intval ($_REQUEST ['form_token']))) && ($_REQUEST ['deleteImage'] != 'true') && ($_REQUEST['changepassword'] != 'true')) {
		header ("Location: index.php?action=Error&module=Users&error_string=" . urlencode ($app_strings ['LBL_PERMISSION']));
		exit ();
	}

	if ((isset ($_POST ['record'])) && (!is_admin ($current_user)) && (!in_array ($current_user->column_fields['roleid'], $lstRoles)) && ($_POST ['record'] != $current_user->id)) {
		exit ('Unauthorized access to user administration.');
	} elseif ((!isset ($_POST ['record'])) && ((!is_admin ($current_user)) || (!in_array ($current_user->column_fields['roleid'], $lstRoles)))) {
		exit ('Unauthorized access to user administration.');
	}
	...
	if ($_REQUEST ['changepassword'] == 'true') {
		...
		if (isset ($_REQUEST ['new_password'])) {
			if (!$focus->change_password ($_REQUEST['old_password'], $_REQUEST['new_password'])) {
				...
				header ('Location: index.php?module=Home&action=CustomerView&tab=user-profile');
				exit ();
			}
		}
	}
...
//save user Image
	if (!$_REQUEST['changepassword'] == 'true') {
		if (strtolower ($current_user->is_admin) == 'off' && $current_user->id != $focus->id && !in_array ($current_user->column_fields['roleid'], $lstRoles)) {
	...
	if (isset($_REQUEST['activity_mode'])) {
		$activitymode = '&activity_mode=' . vtlib_purify ($_REQUEST['activity_mode']);
	}
	if (isset($_POST['parenttab'])) {
...
//$location = "Location: index.php?action=".vtlib_purify($return_action)."&module=".vtlib_purify($return_module)."&record=".vtlib_purify($return_id);
	$location = "Location: index.php?action=index&module=panelusuarios&record=" . vtlib_purify ($return_id);

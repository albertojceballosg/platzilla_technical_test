...
if($_REQUEST['return_action']=='CallRelatedList' && $_REQUEST['record']!='' && $_REQUEST['return_id']!=''){
	$sql= 'delete from `vtiger_crmentityrel` where crmid=? and module = ? and relcrmid= ? ';
	$adb->pquery($sql, array($_REQUEST['return_id'], $_REQUEST['return_module'], $_REQUEST['record']));

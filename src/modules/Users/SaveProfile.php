...
foreach($modArr as $fld_module => $fld_label)
{
	...
	header ("Location: index.php?action=ListProfiles&module=Settings&mode=view&parenttab=Settings&profileid=" . vtlib_purify ($profileid) . "&selected_tab=" . vtlib_purify ($def_tab) . "&selected_module=" . vtlib_purify ($def_module));

	/** returns value 0 if request permission is on else returns value 1
	 *
	 * @param $req_per -- Request Permission:: Type varchar
	 *
	 * @return int $permission - can have value 0 or 1:: Type integer
	 *
	 */

...
<script language='JavaScript' type='text/javascript' src='include/js/slider.js'></script>
	...
	if ((is_admin ($current_user) || $_REQUEST['record'] == $current_user->id)
	...
	} elseif (is_admin ($current_user) || $_REQUEST['record'] == $current_user->id || in_array ($current_user->column_fields['roleid'], $lstRoles)) {
		$buttons = "<input title='" . $app_strings['LBL_EDIT_BUTTON_TITLE'] . "' accessKey='" . $app_strings['LBL_EDIT_BUTTON_KEY'] . "' class='crmButton small edit' onclick=\"this.form.return_module.value='Users'; this.form.return_action.value='DetailView'; this.form.return_id.value='$focus->id'; this.form.action.value='EditView';\" type='submit' name='Edit' value='  " . $app_strings['LBL_EDIT_BUTTON_LABEL'] . "  '>";
		$smarty->assign ('EDIT_BUTTON', $buttons);
		...
		if (in_array ($current_user->column_fields['roleid'], $lstRoles)) {
	...
	$data           = EditViewUtils::splitValidationData ($validationData);

	if (is_admin ($current_user)) {
		$idDlgSupportPermissions = "dlgSupportPermissions";

		$dlgSupportPermissions = getDlgSupportPermissions ($focus->id);

		$smarty->assign ("ID_DLG_SUPPORT_PERMISSIONS", $idDlgSupportPermissions);
		$smarty->assign ("DLG_SUPPORT_PERMISSIONS", escribeDlgModal ($idDlgSupportPermissions, $dlgSupportPermissions));
	}

	if ($current_user->id == $_REQUEST['record'] || is_admin ($current_user) == true || in_array ($current_user->column_fields['roleid'], $lstRoles)) {
		...
		$smarty->assign ("USER", $focus->getInfoUserProfile ($focus->id));
		...
		if (isPermitted ("Users", "EditView", $_REQUEST['record']) == 'yes') {

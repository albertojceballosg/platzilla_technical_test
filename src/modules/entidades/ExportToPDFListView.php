	...
	try {
		...
		if ((!empty ($from)) && (!empty ($to))) {
			...
			if (!empty ($supportType)) {
				$whereClauses [] = 'e.tipo_de_soporte=?';
			...
			switch ($orderBy) {
				case 1:
					$orderByClause = 'e.tematica_del_med';
					break;
				case 2:
					$orderByClause = 'e.tematica_del_med, e.tipo_de_soporte';
					break;
				case 3:
					$orderByClause = 'e.tematica_del_med, e.tipo_de_soporte, r.tipo_repercusion';
					break;
				case 4:
			...
			$result = $adb->pquery (
				...
				FROM
					...
					INNER JOIN vtiger_entidades e ON e.entidadesid=r.relacionado_con
					LEFT JOIN vtiger_entidades m ON m.entidadesid=r.medio_donde_apar
				WHERE
					{$whereClauses}
					crmer.deleted=0
				{$orderByClause}",

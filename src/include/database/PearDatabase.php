...
// See function convertPS2Sql in PearDatabase below
	...
	class PearDatabase {
		...
		public $exceptionInsteadOfDying = false;
		...
		function checkError ($msg = '', $dieOnError = false) {
			// Agregado por AV para integrar BUGSNAG - 20190922
			...
			$exception = '';
			// Agregado por AV para integrar BUGSNAG - 20190922
			if ($this->exceptionInsteadOfDying) {
				// Agregado por AV para integrar BUGSNAG - 20190922
				//$bugsnag->notifyException(new RuntimeException('ADODB error ' . $msg . '->[' . $this->database->ErrorNo () . ']' . $this->database->ErrorMsg ()));
				$bugsnag->notifyError('Nativo', 'ADODB error ' . $msg . '->[' . $this->database->ErrorNo () . ']' . $this->database->ErrorMsg (), null, 'error');
				// Agregado por AV para integrar BUGSNAG - 20190922
				throw new Exception ("ADODB error {$msg}->{$this->database->ErrorMsg ()}");
			} else if ($this->dieOnError || $dieOnError) {
				require_once ('modules/Settings/DataBaseError.php');
				$bt = debug_backtrace ();
				$ut = array ();
				foreach ($bt as $t) {
				...
				$body = '<pre>' . var_export ($ut, true) . '</pre> <br>ADODB error ' . $msg . '->[' . $this->database->ErrorNo () . ']' . $this->database->ErrorMsg ();
				//sendMessaje ($body);
				echo $body;
				error_log (var_export ($ut, true) . "\nADODB error " . $msg . "->[" . $this->database->ErrorNo () . "]" . $this->database->ErrorMsg ());    // EGC si no se est� escribiendo a la salida puede ser muy jodido encontrar estos errores
				//$this->println ("ADODB error " . $msg . "->[" . $this->database->ErrorNo () . "]" . $this->database->ErrorMsg ());
				// Agregado por AV para integrar BUGSNAG - 20190922
				//$bugsnag->notifyException(new RuntimeException($msg . 'ADODB error ' . $msg . '->' . $this->database->ErrorMsg ()));
				//$bugsnag->notifyError('Nativo', $msg . 'ADODB error ' . $msg . '->' . $this->database->ErrorMsg (), null, 'error');
				// Agregado por AV para integrar BUGSNAG - 20190922
		...
		// LOG TEMPORAL: Queries de facturas
		if (strpos($sql, 'vtiger_facturas') !== false) {
			...
			//error_log("[PearDatabase::query] FROM: $caller | SQL: " . substr(preg_replace('/\s+/', ' ', $sql), 0, 500));
		}
			$this->checkConnection ();
		...
		function pquery ($sql, $params, $dieOnError = false, $msg = '') {
			...
			if (!$result) {
				$sqlForLog = $sql;
				if (!$this->avoidPreparedSql && !empty($params)) {
					$sqlForLog = $this->convert2Sql ($sql, $params);
				}
				$this->checkError ($msg . ' Query Failed:' . $sqlForLog . '::', $dieOnError);
			}
		...
		function resetSettings ($dbtype, $host, $dbname, $username, $passwd) {
			...
			if ($host == '') {
				...
				//LCL - Punto de conexion de las plataformas.
				if (isset($_REQUEST['plat_customer'])) {
					$_SESSION['plat'] = $_REQUEST['plat_customer'];
				}
				if (isset($_SESSION['plat'])) {
					unset($dbconfig['db_name']);
					if (file_exists ($_SESSION['plat'] . '/config.inc.php')) {
						require ($_SESSION['plat'] . '/config.inc.php');
					}

...
if(!function_exists('GetRelatedList')) {
	function GetRelatedList($module, $relatedmodule, $focus, $query, $button, $returnset, $id = '', $edit_val = '',$del_val = '',$skipActions = false, $isCard = false) {
		return GetRelatedListBase($module, $relatedmodule, $focus, $query, $button, $returnset, $id, $edit_val, $del_val, $skipActions, $isCard);
	}
}
...
function GetRelatedListBase($module, $relatedmodule, $focus, $query, $button, $returnset, $id = '', $edit_val = '', $del_val = '', $skipActions = false, $isCard = false)
{
	...
	if($relatedmodule != 'Faq' && $relatedmodule != 'Vendors' && $relatedmodule != 'Users' && $relatedmodule != 'notificaciones') {
		global $current_user;
	...
	$url_qry = (isset($url_qry) ? $url_qry : '') . "&order_by=".$order_by."&sorder=".$sorder;
	$computeCount = isset($_REQUEST['withCount']) ? $_REQUEST['withCount'] : null;
	...
	$navigationOutput[] = getRelatedTableHeaderNavigation($navigation_array, $url_qry, $module, $relatedmodule, $id, $isCard);
...
function getHistory($parentmodule,$query,$id)
{
	...
	else
	{
		...
		while($row = $adb->fetch_array($result))
		{
			...
			$i++;
			$entries_list[] = array('records' => $entries, 'color' => 'white');
		}

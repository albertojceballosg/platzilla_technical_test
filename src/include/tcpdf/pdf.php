...
function addImage( $logo_name, $location=array('10','10','0','0') ) {
	if($logo_name)//error checking just in case, by OpenCRM
	{
		...
		$stretchx = $location[2];
		$stretchy = $location[3];
		if(file_exists($_SESSION['plat'].'/test/logo/p'.$logo_name))
			$this->Image($_SESSION['plat'].'/test/logo/p'.$logo_name,$x1,$y1,$stretchx,$stretchy);
	}
}
...
function addTextBlock( $title,$text,$positions,$align = 'J')
{
	$r1  = $positions[0];
	$y1  = $positions[1];
	$this->SetXY( $r1, $y1);
	...
	$this->MultiCell( $positions[2], 4, $text, 0, $align);
}
...
function tableWrapperRec($position)
{
	$r1  = $position[0];
	$r2  = $r1 + 19 + $position[2] ;
	$y1  = $position[1];
...
// description blocks
function addDescBlock( $data, $title, $position, $tamano=10)
{
	$lengthtitle = strlen($title);
	...
	$this->SetFont( "Helvetica", "B", $tamano);
...
function addCols( $tab ,$positions ,$bottom, $taxtype = 'group', $num_pages=NULL, $page_num=NULL)
{
...
function addColsPedido( $tab ,$positions ,$bottom, $taxtype = 'group', $num_pages=NULL, $page_num=NULL)
{
	global $columns,$app_strings;
	...
	while ( list( $lib, $pos ) = each ($tab) )
	{
		...
		switch($lib) {
	  		case 'Total':
			break;
	  		case 'Qty' /*&& $num_pages==$page_num*/:
	  		case 'Price' /*&& $num_pages==$page_num*/:
				if($taxtype == "individual" && $num_pages==$page_num)
					$this->Line( $colX, $y1, $colX, (($y1+$y2)-43));
				else if($num_pages==$page_num)
					$this->Line( $colX, $y1, $colX, (($y1+$y2)-43));
				else $this->Line( $colX, $y1, $colX, ($y1+$y2)-0);
			break;
	  		default:
				if($taxtype == "individual" && $lib == 'Discount')
					$this->Line( $colX, $y1, $colX, (($y1+$y2)-22));
				else
					$this->Line( $colX, $y1, $colX, ($y1+($y2-0)));
	  		break;
		}
	}
}

function addColsInvoice( $tab ,$positions ,$bottom, $taxtype = 'group', $num_pages=NULL, $page_num=NULL,$movimiento=0,$largointervalo=0)
{
	global $columns,$app_strings;
	...
	while ( list( $lib, $pos ) = each ($tab) )
	{
		...
		switch($lib) {
			...
			break;
	  		...
	  		case 'Price' /*&& $num_pages==$page_num*/:
				if($taxtype == "individual" && $num_pages==$page_num)
					$this->Line( $colX, $y1, $colX, (($y1+$y2)-32));
				else if($num_pages==$page_num)
					$this->Line( $colX, $y1, $colX, (($y1+$y2)-15));
				else $this->Line( $colX, $y1, $colX, ($y1+$y2));
			break;
	  		default:
				if($taxtype == "individual" && $lib == 'Discount')
					$this->Line( $colX, $y1, $colX, (($y1+$y2)-37));
...
function addCols_footer( $tab ,$positions ,$bottom, $taxtype = 'group', $num_pages=NULL, $page_num=NULL)
{
	global $columns,$app_strings;

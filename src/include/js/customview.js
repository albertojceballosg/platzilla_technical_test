...
function validate (blockid) {
	var nummaxlength = 255,
		fieldtype = document.getElementById ('fieldType_' + blockid).value,
		mode = document.getElementById ('cfedit_mode').value,
		lengthLayer = document.getElementById ('lengthdetails_' + blockid),
		decimalLayer = document.getElementById ('decimaldetails_' + blockid),
		pickListLayer = document.getElementById ('fldPickList_' + blockid);
	...
	var flabel = document.getElementById ('fldLabel_' + blockid).parentNode.previousElementSibling.innerText;
	if (!emptyCheck ('fldLabel_' + blockid, flabel, 'text')) {
		return false;
	}
	...
	var flength = document.getElementById ('fldLength_' + blockid).parentNode.previousElementSibling.innerText;
	if (lengthLayer != null && lengthLayer.style.display != 'none') {
		if (!emptyCheck ('fldLength_' + blockid, flength)) {
			return false
		} else if (!intValidate ('fldLength_' + blockid, flength)) {
			return false
		} else if (!numConstComp ('fldLength_' + blockid, flength, 'G', 0)) {
			return false
		}
	}

	var fdecimal = document.getElementById ('fldDecimal_' + blockid).parentNode.previousElementSibling.innerText;
	if (decimalLayer != null && decimalLayer.style.display != 'none') {
		if (document.getElementById ('fldDecimal_' + blockid).value.replace (/^\s+/g, '').replace (/\s+$/g, '').length > 0) {
			if (!intValidate ('fldDecimal_' + blockid, fdecimal)) {
				return false
			}
		} else if (!numConstComp ('fldDecimal_' + blockid, fdecimal, 'GE', 0)) {
			return false
		} else if (!numConstComp ('fldDecimal_' + blockid, fdecimal, 'LE', 30)) {
			return false
		}
	}
	var decimallength = '';
	if (decimalLayer != null && decimalLayer.style.display != 'none' && document.getElementById ('fldDecimal_' + blockid) != null) {
	...
	if (lengthObj != null && lengthObj.style.display != 'none' && !numConstComp ('fldLength_' + blockid, flength, 'LE', nummaxlength)) {
		return false
	}
	var picklistObj = document.getElementById ('fldPickList_' + blockid);
	if (pickListLayer != null && getObj ('picklistdetails_' + blockid).style.display == 'visible') {
		var pickListAry = splitValues (pickListLayer);
		if (emptyCheck ('fldPickList_' + blockid, 'Picklist values')) {

			//Empty Check validation
			for (var i = 0; i < pickListAry.length; i++) {
				if (pickListAry[ i ] == "") {
			...
			for (i = 0; i < pickListAry.length; i++) {
				for (var j = i + 1; j < pickListAry.length; j++) {
...
var fieldValueArr=new Array('Text','Number','Percent','Currency','Date','Email','Phone','Picklist','URL','Checkbox','TextArea','MultiSelectCombo','Skype','Time','RelatedModule','Progress_Bar', 'RelatedRecords', 'Attachments');
var fieldTypeArr=new Array('text','number','percent','currency','date','email','phone','picklist','url','checkbox','textarea','multiselectcombo','skype','time','relatedmodule','progress_bar', 'relatedrecords', 'attachments');
...
function selFieldType(id,scrollLayer,bool,blockid) {
	...
	var relatedModuleLayer=document.getElementById("relatedmodule_"+blockid);
	var relatedRecordsLayer=document.getElementById("relatedrecords_"+blockid);
	var progressbar_min_=document.getElementById("progressbar_min_"+blockid);
	var progressbar_max_=document.getElementById("progressbar_max_"+blockid);
	var progressbar_ini_=document.getElementById("progressbar_ini_"+blockid);
	var progressbar_ord_=document.getElementById("progressbar_ord_"+blockid);
	//JA 29/07/2016 [ TT11239 ] Restringir tipos de datos con propiedad de unicidad en el editor
  var uniqueValue=document.getElementById("uniquevalue_"+blockid);
	progressbar_min_.style.display="none";
	progressbar_max_.style.display="none";
	progressbar_ini_.style.display="none";
	progressbar_ord_.style.display="none";
	if (type=='text') {
		lengthLayer.style.display="table-row";
		decimalLayer.style.display="none";
		pickListLayer.style.display="none";
		relatedModuleLayer.style.display="none";
		relatedRecordsLayer.style.display = 'none';
	} else if (type=='date' || type=='percent' || type=='email' || type=='phone' || type=='url' || type=='checkbox' || type=='textarea' || type=='skype' || type=='time') {
		document.getElementById("lengthdetails_"+blockid).style.display="none";
		decimalLayer.style.display="none";
		pickListLayer.style.display="none";
		relatedModuleLayer.style.display="none";
		relatedRecordsLayer.style.display = 'none';
		uniqueValue.style.display="none";
		...
		relatedRecordsLayer.style.display = 'none';
		uniqueValue.style.display="none";
	} else if (type=='relatedmodule') {
		lengthLayer.style.display = "none";
		decimalLayer.style.display = "none";
		pickListLayer.style.display = "none";
		uniqueValue.style.display = "none";
		relatedModuleLayer.style.display = "table-row";
		relatedRecordsLayer.style.display = 'none';
	} else if (type === 'relatedrecords') {
		lengthLayer.style.display = "none";
		decimalLayer.style.display = "none";
		pickListLayer.style.display = "none";
		uniqueValue.style.display = "none";
		relatedModuleLayer.style.display = "none";
		relatedRecordsLayer.style.display = 'table-row';
		...
		relatedModuleLayer.style.display="none";
		uniqueValue.style.display="none";
		progressbar_min_.style.display="table-row";
		progressbar_max_.style.display="table-row";
		progressbar_ini_.style.display="table-row";
		progressbar_ord_.style.display="table-row";
	...
	//Tipos de campos permitidos para valor unico
	if (type=='text' || type=='email' || type=='phone' || type=='url' || type=='skype'){
		uniqueValue.style.display="table-row";
	}

...
function vtlib_setvalue_from_popup(recordid,value,target_fieldname) {
    if(window.opener.document.EditView) {
        ...
        if(domnode_id) {
				...
				jQuery(window.opener.document).find('#'+target_fieldname).val(recordid);
				jQuery(window.opener.document).find('#'+target_fieldname+'_display').val(value);
			}
			eval("if (window.opener.onchange"+target_fieldname+") window.opener.onchange"+target_fieldname+"();");
        return true;

...
function priceBookPickList(currObj, row_no) {
	...
	window.open("index.php?module=PriceBooks&action=Popup&html=Popup_picker&form=EditView&popuptype=inventory_pb&fldname=listPrice"+rowId+"&productid="+productId+"&currencyid="+currencyid,"priceBookWin","width=640,height=565,resizable=0,scrollbars=1,top=150,left=200");
}
...
// [TT11315] En las siguientes líneas se elimino el atributo value, y se coloco fue placeholder-->
	colone.innerHTML="<input type='text' id='"+label_name+"' name='"+label_name+"' placeholder='"+tax_labelarr.TAX_NAME+"' class='form-control' onclick=\"this.form."+label_name+".value=''\";/>";
	coltwo.innerHTML="<input type='text' id='"+label_val+"' name='"+label_val+"' placeholder='"+tax_labelarr.TAX_VALUE+"' class='form-control' onclick=\"this.form."+label_val+".value=''\";/>";
// fin pedido [TT11315]//
	colthree.innerHTML="<input type='submit' name='Save' value=' "+tax_labelarr.SAVE_BUTTON+" ' class='btn btn-primary btn-sm' onclick=\"this.form.action.value='TaxConfig'; this.form."+add_tax_flag+".value='true'; this.form.parenttab.value='Settings'; return validateNewTaxType('"+label_name+"','"+label_val+"');\">&nbsp;<input type='submit' name='Cancel' value=' "+tax_labelarr.CANCEL_BUTTON+" ' class='btn btn-warning btn-sm' onclick=\"this.form.action.value='TaxConfig'; this.form.module.value='Settings'; this.form."+add_tax_flag+".value='false'; this.form.parenttab.value='Settings';\">";

}

// [TT11315] Metodo Creado para validar campos cuando se crean nuevos registros por los impuestos de productos o los impuestos de transporte -->
function validateNewTaxType(fieldname, fieldvalue)
{

// Modificado por Keyla Rodriguez Realizar validaciones
//----------------------Campos Vacíos------------------------
	...
	{
		alert(alert_arr.LABEL_TAX_VALUE_EMPTY);
		return false;
	}
//----------------Campo valor impuesto solo tipo número-----------------------
	if ((isNaN(trim(document.getElementById(fieldvalue).value)))==true){

		var regex  = /^[0-9]+([,][0-9]*)?$/;
		//---Campo valor impuesto que el separador sea solo el punto
		if (regex.test(document.getElementById(fieldvalue).value)){
				alert(alert_arr.ENTER_PUNT_TAX_VALUE);
				return false;
		}


		if ((isNaN(trim(document.getElementById(fieldvalue).value)))==true){
			alert(alert_arr.ENTER_TAX_VALUE_NUMERIC);

			return false;
		}
	}

//----------------Campo valor impuesto no superior a 100-----------------------
	if (trim(document.getElementById(fieldvalue).value) >= 100) {
		alert(alert_arr.ENTER_TAX_VALUE_SUPERIOR);
		return false;
	}

//----------------Campo valor impuesto valor no debe ser negativo-----------------------
	if (trim(document.getElementById(fieldvalue).value) <= 0 )  {

		if (trim(document.getElementById(fieldvalue).value) == 0) {
			alert(alert_arr.ENTER_CERO_TAX_VALUE);
		}

		if (trim(document.getElementById(fieldvalue).value) < 0) {
			alert(alert_arr.ENTER_POSITIVE_TAX_VALUE);
		}
		return false;
	}
...
// [TT11315] Metodo Creado para validar campos cuando se esta editando los impuestos de productos o los impuestos de transporte -->
function validateTaxes(countname)
{
	...
	{
		taxprefix = 'taxvalor_';
		taxLabelPrefix = 'taxlabel_';
	}
	else
	{
		taxprefix = 'sh_tax_';
		taxLabelPrefix = 'taxlabel_shtax_';
	}
	for(var i=0;i<taxcount;i++)
	{
			//----------------------Campos Vacíos------------------------
			if(trim(document.getElementById(taxLabelPrefix+i).value)== '')
			{
				alert(alert_arr.LABEL_SHOULDNOT_EMPTY);
				return false;
			}
			if(trim(document.getElementById(taxprefix+i).value)== '')
			{
				alert(alert_arr.LABEL_TAX_VALUE_EMPTY);
				return false;
			}
		...
		//----------------Campo valor impuesto solo tipo número-----------------------
			if ((isNaN(trim(document.getElementById(taxprefix+i).value)))==true){

				var regex  = /^[0-9]+([,][0-9]*)?$/;
				//---Campo valor impuesto que el separador sea solo el punto
				if (regex.test(document.getElementById(taxprefix+i).value)){
						alert(alert_arr.ENTER_PUNT_TAX_VALUE);
						return false;
				}
		...
		//----------------Campo valor impuesto no superior a 100-----------------------
			if (trim(document.getElementById(taxprefix+i).value) >= 100) {
				alert(alert_arr.ENTER_TAX_VALUE_SUPERIOR);
				return false;
			}

		//----------------Campo valor impuesto valor no debe ser negativo-----------------------
			if (trim(document.getElementById(taxprefix+i).value) <= 0 )  {
...
function fnAddProductRow(module,image_path){
	...
	coltwo.innerHTML= '<table border="0" cellpadding="1" cellspacing="0" width="100%"><tr><td class="small"><input id="productName'+count+'" name="productName'+count+'" class="small" style="width: 70%;" value="" readonly="readonly" type="text">'+
						...
						*/
	coltwo.innerHTML= '<table width="100%"  border="0" cellspacing="0" cellpadding="1">'+
				'	   <tr>'+
				'		<td class="mini-products">'+
				'			<div class="input-group">'+
				'				<input type="text" id="productName'+count+'" name="productName'+count+'" class="form-control" value="" readonly />'+
				'				<div class="input-group-addon" id="searchIcon'+count+'" onclick="productPickList(this,\''+module+'\','+count+')">'+
				'					<i class="fa fa-cogs"></i>'+
				'				</div>'+
				'			</div>'+
				'			<input type="hidden" id="hdnProductId'+count+'" name="hdnProductId'+count+'" value="" />'+
				'		</td>'+
				'	</tr>'+
				'	<tr>'+
				'		<td class="mini-products">'+
				...
				'	   </tr>'+
				'	   <tr valign="bottom">'+
				'		<td class="mini-products" id="setComment'+count+'">'+
				'			<textarea id="comment'+count+'" name="comment'+count+'" class="form-control"></textarea>'+
				'		</td>'+
				'	   </tr>'+
				'	</table>';
	...
	if(module != "PurchaseOrder"){
	colthree.className = "text-center"
	colthree.innerHTML='<span id="qtyInStock'+count+'">&nbsp;</span>';
	}

	//Quantity
	var temp='';
	colfour.className = "text-center"
...
//	colfive.innerHTML='<table width="100%" cellpadding="0" cellspacing="0"><tr><td align="right"><input id="listPrice'+count+'" name="listPrice'+count+'" value="0.00" type="text" class="small " style="width:70px" onBlur="calcTotal();setDiscount(this,'+count+');callTaxCalc('+count+'); calcTotal();"/>&nbsp;<img src="themes/images/pricebook.gif" onclick="priceBookPickList(this,'+count+')"></td></tr><tr><td align="right" style="padding:5px;" nowrap>		(-)&nbsp;<b><a href="javascript:doNothing();"
	...
	colfive.innerHTML='<table width="100%" cellpadding="0" cellspacing="0">'+
		'   <tr>'+
		'	<td class="mini-products" align="right">'+
		'		<input id="listPrice'+count+'" name="listPrice'+count+'" value="0.00" type="text" class="form-control" onBlur="calcTotal();setDiscount(this,'+count+'); callTaxCalc('+count+');calcTotal();"/>&nbsp;<img src="" onclick="priceBookPickList(this,'+count+')">'+
		'	</td>'+
		'   </tr>'+
		'   <tr>'+
		...
		'		<div class="discountUI" id="discount_div'+count+'" style="display:none">'+
		'			<input type="hidden" id="discount_type'+count+'" name="discount_type'+count+'" value="">'+
		'			<table width="100%" border="0" cellpadding="5" cellspacing="0" class="small">'+
		'			   <tr>'+
		'				<td class="mini-products" id="discount_div_title'+count+'" nowrap align="left" ></td>'+
		'				<td class="mini-products" align="right"><img src="themes/images/close.gif" border="0" onClick="fnHidePopDiv(\'discount_div'+count+'\')" style="cursor:pointer;"></td>'+
		'			   </tr>'+
		'			   <tr>'+
		'				<td class="mini-products" align="left"><input type="radio" name="discount'+count+'" checked onclick="setDiscount(this,'+count+'); callTaxCalc('+count+');calcTotal();">&nbsp; '+product_labelarr.ZERO_DISCOUNT+'</td>'+
		'				<td class="mini-products">&nbsp;</td>'+
		'			   </tr>'+
		'			   <tr>'+
		'				<td class="mini-products" align="left"><input type="radio" name="discount'+count+'" class="" onclick="setDiscount(this,'+count+'); callTaxCalc('+count+');calcTotal();">&nbsp; '+product_labelarr.PERCENT_OF_PRICE+'</td>'+
		'				<td class="mini-products" align="right"><input type="text" class="form-control" size="6" id="discount_percentage'+count+'" name="discount_percentage'+count+'" value="0" style="visibility:hidden" onBlur="setDiscount(this,'+count+'); callTaxCalc('+count+');calcTotal();">&nbsp;%</td>'+
		'			   </tr>'+
		'			   <tr>'+
		'				<td class="mini-products" align="left" nowrap><input type="radio" name="discount'+count+'" class="" onclick="setDiscount(this,'+count+'); callTaxCalc('+count+');calcTotal();">&nbsp; '+product_labelarr.DIRECT_PRICE_REDUCTION+' </td>'+
		'				<td class="mini-products" align="right"><input type="text" id="discount_amount'+count+'" class="form-control" name="discount_amount'+count+'" size="6" value="0" style="visibility:hidden" onBlur="setDiscount(this,'+count+'); callTaxCalc('+count+');calcTotal();"></td>'+
		'			   </tr>'+
		'			</table>'+
		'		</div>'+
		'	</td>'+
		...
		'		<div class="discountUI" id="tax_div'+count+'">'+
		'		</div>'+
		'	</td>'+
		'  </tr>'+
		'</table> ';
	...
	//Net Price
	colseven.className = "text-center";
...
function calcCurrentTax(tax_name, curr_row, tax_row)
{
	...
	return document.getElementById("taxTotal"+curr_row).innerHTML = roundValue(tax_total);
}

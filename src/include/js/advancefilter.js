...
var cadvft_column_index_count = -1;
var cadvft_group_index_count = 0;
var ccolumn_index_array = [];
var cgroup_index_array = [];
...
function deleteGroupColor(groupIndex) {
    removeElement('cconditiongroup_'+groupIndex);

    var keyOfTheGroup = cgroup_index_array.indexOf(groupIndex);
    var isLastElement = true;

    for(var i=keyOfTheGroup; i<isLastElement; ++i) {
        var nextGroupIndex = cgroup_index_array[i];
        var nextGroupBlockId = "cconditiongroup_"+nextGroupIndex;
        if(document.getElementById(nextGroupBlockId)) {
            isLastElement = false;
            break;
        }
...
function addNewConditionGroupColor(parentNodeId) {
    addConditionGroupColor(parentNodeId);
    addConditionRowColor(cadvft_group_index_count);
}

function deleteColumnRow(groupIndex, columnIndex, color) {
	var param1 = 'columnconditionglue_';
    var param2 = 'conditioncolumn_';
    var param3 = column_index_array[groupIndex];
    var param4 = "fcon";

    if(color != undefined){
        param1 = 'ccolumnconditionglue_';
        param2 = 'colorconditioncolumn_';
        param3 = ccolumn_index_array[groupIndex];
        param4 = "fccon";
	}

	removeElement(param1+groupIndex);
	removeElement(param2+groupIndex+'_'+columnIndex);

	var groupColumns = param3;
	...
	for(var i=keyOfTheColumn; i<groupColumns.length; ++i) {
		...
		var nextColumnRowId = param2+groupIndex+'_'+nextColumnIndex;
	...
	if(isLastElement) {
		for(var i=keyOfTheColumn-1; i>=0; --i) {
			...
			var prevColumnGlueId = param4+prevColumnIndex;
...
function addRequiredElements(columnindex, color) {
		...
		switch(fieldtype) {
			...
			case 'T':
				if(fieldtype=='T' && tableName.indexOf('vtiger_crmentity')<0){
					defaultRequiredElements(columnindex,color);
					break;
				}
				var dateformat = $('jscal_dateformat').value;
						...
						if(!document.getElementById('jscal_trigger_'+fval+columnindex)) {
							var node = document.createElement('img');
							node.setAttribute('src',$('image_path').value+'btnL3Calendar.gif');
							node.setAttribute('id','jscal_trigger_'+fval+columnindex);
						...
						Calendar.setup ({
							inputField : fval+columnindex, ifFormat : dateformat+' '+timeformat, showsTime : showtime, button : "jscal_trigger_"+fval+columnindex, singleClick : true, step : 1
                        });

                        if(currOp.value == 'bw') {
                        	if(!document.getElementById(fval+'_ext'+columnindex)) {
	                        	var fillernode = document.createElement('br');

	                        	var node1 = document.createElement('input');
	                        	node1.setAttribute('class', 'repBox small');
	                        	node1.setAttribute('type', 'text');
	                        	node1.setAttribute('id',fval+'_ext'+columnindex);
	                        	node1.setAttribute('name',fval+'_ext'+columnindex);
							...
							if(!document.getElementById('jscal_trigger_'+fval+'_ext'+columnindex)) {
								var node2 = document.createElement('img');
								node2.setAttribute('src',$('image_path').value+'btnL3Calendar.gif');
								node2.setAttribute('id','jscal_trigger_'+fval+'_ext'+columnindex);
							...
							if(!document.getElementById('clear_text_ext'+columnindex)) {
								...
								node3.onclick = function() {
													document.getElementById(fval+'_ext'+columnindex).value='';
													return false;
												}
							...
							Calendar.setup ({
								inputField : fval+'_ext'+columnindex, ifFormat : dateformat+' '+timeformat, showsTime : showtime, button : "jscal_trigger_"+fval+"_ext"+columnindex, singleClick : true, step : 1
	                        });
                       	} else {
							if(document.getElementById(fval+'_ext'+columnindex)) removeElement(fval+'_ext'+columnindex);
							if(document.getElementById('jscal_trigger_'+fval+'_ext'+columnindex)) removeElement('jscal_trigger_'+fval+'_ext'+columnindex);
							if(document.getElementById('clear_text_ext'+columnindex)) removeElement('clear_text_ext'+columnindex);
                       	}

                        break;
			default	:defaultRequiredElements(columnindex, color);
		}
	}
}
function defaultRequiredElements(columnindex, color) {
	var fval = 'fval';
    ...
    if(document.getElementById('jscal_trigger_'+fval+columnindex)) removeElement('jscal_trigger_'+fval+columnindex);
	if(document.getElementById(fval+'_ext'+columnindex)) removeElement(fval+'_ext'+columnindex);
	if(document.getElementById('jscal_trigger_'+fval+'_ext'+columnindex)) removeElement('jscal_trigger_'+fval+'_ext'+columnindex);
	if(document.getElementById('clear_text_ext'+columnindex)) removeElement('clear_text_ext'+columnindex);
}
...
function checkAdvancedFilter() {
	...
	var conditionColumns = vt_getElementsByName('li', "conditionColumn");
	if (conditionColumns.length === 0) {
		conditionColumns = vt_getElementsByName ('tr', "conditionColumn");
	}
	...
	var conditionGroups = vt_getElementsByName('li', "conditionGroup");
	if (conditionGroups.length === 0) {
		conditionGroups = vt_getElementsByName ('div', "conditionGroup");
	}
	var criteriaGroups = [];
...
function checkColorCriteria() {

    var escapedOptions = new Array('account_id','contactid','contact_id','product_id','parent_id','campaignid','potential_id','assigned_user_id1','quote_id','accountname','salesorder_id','vendor_id','time_start','time_end','lastname');

    var conditionColumns = vt_getElementsByName('li', "colorconditioncolumn");
    if (conditionColumns.length === 0) {
        conditionColumns = vt_getElementsByName ('tr', "colorconditioncolumn");
    }
    ...
    for(var i=0;i < conditionColumns.length ; i++) {
        ...
        var columnId = "fccol"+columnIndex;
        var columnObject = getObj(columnId);
        var selectedColumn = trim(columnObject.value);
        var selectedColumnIndex = columnObject.selectedIndex;
        var selectedColumnLabel = columnObject.options[selectedColumnIndex].text;

        var comparatorId = "fcop"+columnIndex;
        var comparatorObject = getObj(comparatorId);
        var comparatorValue = trim(comparatorObject.value);

        var valueId = "fcval"+columnIndex;
        var valueObject = getObj(valueId);
        var specifiedValue = trim(valueObject.value);

        var extValueId = "fcval_ext"+columnIndex;
        var extValueObject = getObj(extValueId);
        if(extValueObject) {
            extendedValue = trim(extValueObject.value);
        }

        var glueConditionId = "fccon"+columnIndex;
        var glueConditionObject = getObj(glueConditionId);
        var glueCondition = '';
        if(glueConditionObject) {
            glueCondition = trim(glueConditionObject.value);
    ...
    $('advft_criteria_color').value = JSON.stringify(criteriaConditions);

    var conditionGroups = vt_getElementsByName('li', "cconditionGroup");
    if (conditionGroups.length === 0) {
        conditionGroups = vt_getElementsByName ('div', "cconditionGroup");
    }
    var criteriaGroups = [];
    for(var i=0;i < conditionGroups.length ; i++) {
        var groupTableId = conditionGroups[i].getAttribute("id");
        ...
        var groupConditionId = "gcpcon"+groupIndex;
        var groupConditionObject = getObj(groupConditionId);
        var groupCondition = '';
        if(groupConditionObject) {
            groupCondition = trim(groupConditionObject.value);
        }
        criteriaGroups[groupIndex] = {"groupid":groupIndex, "groupcolor":groupCondition};

    }

    $('advft_criteria_groups_color').value = JSON.stringify(criteriaGroups);

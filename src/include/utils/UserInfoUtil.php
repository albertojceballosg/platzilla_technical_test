<?php
	 ...
	 *
	 * @returns string vtiger_roleid :: Type String
	 */
	...
	function getDefaultSharingAction () {
		...
		$result = $adb->pquery (
			'SELECT dos.* FROM vtiger_def_org_share dos INNER JOIN vtiger_tab t ON t.tabid=dos.tabid AND t.presence=0 WHERE editstatus IN (0, 1)',
			array ()
		);
		...
		while ($row = $adb->fetchByAssoc ($result, -1, false)) {
			$permissions [ $row ['tabid'] ] = $row ['permission'];
		}
	...
	function createRole ($roleName, $parentRoleId, $roleProfileArray, $isCustomer, $isPartner) {
		global $log;
		...
		$query   = "INSERT INTO vtiger_role (roleid,rolename,parentrole,depth,iscustomer,ispartner) VALUES(?,?,?,?,?,?)";
		$qparams = array ($roleId, $roleName, $nowParentRoleHr, $nowRoleDepth, $isCustomer, $isPartner);
	 ...
	 *
	 */
	function updateRole ($roleId, $roleName, $roleProfileArray, $isCustomer, $isPartner) {
		global $log;
		...
		global $adb;
		$sql1 = "UPDATE vtiger_role SET rolename=?, iscustomer=?, ispartner=? WHERE roleid=?";
		$adb->pquery ($sql1, array ($roleName, $isCustomer, $isPartner, $roleId));
	...
	function updateRoleColor ($roleId, $color_letra, $color_background) {
		global $adb;
		if (!existeTabla ('vtiger_role_prop')) {
			return null;
		}
		$sql = "INSERT INTO `vtiger_role_prop` (`roleid`,`color_letra`,`color_background`)
			VALUES ('" . $roleId . "','#" . $color_letra . "','#" . $color_background . "')
			ON DUPLICATE KEY UPDATE `color_letra`='#" . $color_letra . "',`color_background`='#" . $color_background . "'";
		$adb->query ($sql);
	}
	...
	function updateUser2RoleMapping ($roleid, $userid) {
		...
		if ((!empty ($_SESSION ['platInstancia'])) && (empty ($roleid))) {
			$sql_user_name = "SELECT user_name FROM vtiger_users WHERE id = ?";
			$rol_id        = $adb->query_result ($adb->pquery ($sql_user_name, array ($userid)), 0, 'user_name');
			$params        = array ($userid, $rol_id);
		} else {
	...
	function isPermitted ($module, $actionname, $record_id = '') {
		...
		$privilegesFile = 'user_privileges/user_privileges.php';
		if (isset($_SESSION['plat'])) {
			$platPrivilegesFile = $_SESSION['plat'] . '/user_privileges/user_privileges.php';
			if (file_exists($platPrivilegesFile)) {
				$privilegesFile = $platPrivilegesFile;
			}
		}
		require ($privilegesFile);
		$sharingFile = 'user_privileges/sharing_privileges.php';
		if (isset($_SESSION['plat'])) {
			$platSharingFile = $_SESSION['plat'] . '/user_privileges/sharing_privileges.php';
			if (file_exists($platSharingFile)) {
				$sharingFile = $platSharingFile;
			}
		}
		require ($sharingFile);
		...
		$permission = "no";
		if (($module == 'Users' || $module == 'Home' || $module == 'uploads') && (!isset($_REQUEST['parenttab']) || $_REQUEST['parenttab'] != 'Settings')) {
		...
		if ($module == 'Settings' || $module == 'Administration' || $module == 'System' || (isset($_REQUEST['parenttab']) && $_REQUEST['parenttab'] == 'Settings')) {
			if ($actionname == 'ModuleManager' || substr ($actionname, 0, 6) == 'wizard' || $actionname == "LayoutBlockList" || $actionname == "LayoutPanelList" || $actionname == "RolePopup") {
				$lstRoles      = explode (',', obtenerValorVariable ('ROLES_ADM_MODULOS', 'Users'));
				$lstRolesPanel = explode (',', obtenerValorVariable ('ROLES_PANEL_USERS', 'Users'));
				if ($is_admin || in_array ($current_user->column_fields['roleid'], $lstRoles) || in_array ($current_user->column_fields['roleid'], $lstRolesPanel)) {
					$permission = "yes";
				} else {
					$permission = "no";
				}
		...
		if (0) {//Esto est� basado en el esquema viejo
			if (esVistaCliente ($current_user->id) && ($current_user->id != $recOwnId)) {
		...
		if ($others_permission_id === 0) {
			...
			if ($actionid == 3 || $actionid == 4) {
				...
				if ($module == 'Calendar') {
					$permission = isCalendarPermittedByInviteesId ($record_id);
				} else {
					$permission = isReadWritePermittedBySharing ($module, $tabid, $actionid, $record_id);
				}
				$log->debug ("Exiting isPermitted method ...");
	...
	function createProfile ($profilename, $parentProfileId, $description) {
		...
		//Inserting values into Profile2CustomView vtiger_table
		$result = $adb->pquery ('SELECT * FROM vtiger_profile2customview WHERE profileid=?', array ($parentProfileId));
		if (($result) && ($adb->num_rows ($result) > 0)) {
			while ($row = $adb->fetchByAssoc ($result, -1, false)) {
				$adb->pquery (
					'INSERT INTO vtiger_profile2customview (profileid, cvid, tabid, permissions) VALUES (?, ?, ?, ?)',
					array ($current_profile_id, $row ['cvid'], $row ['tabid'], $row ['permissions'])
				);
			}
		}
	...
	function deleteProfile ($prof_id, $transfer_profileid = '') {
		...
		$adb->pquery ('DELETE FROM vtiger_profile2customview WHERE profileid=?', array ($prof_id));
	...
	function getRoleColors ($roleId = '') {
		...
		$query  = "SELECT * FROM `vtiger_role_prop` WHERE TRUE " . $where;
		$result = $adb->pquery ($query, $p);
	...
	function getRoleUserIdNames ($roleId) {
		global $log;
		$log->debug ("Entering getRoleUserIds(" . $roleId . ") method ...");
		global $adb;
		$query            = "SELECT vtiger_user2role.*,vtiger_users.user_name, vtiger_users.first_name, vtiger_users.last_name FROM vtiger_user2role INNER JOIN vtiger_users ON vtiger_users.id=vtiger_user2role.userid WHERE roleid=?";
		$result           = $adb->pquery ($query, array ($roleId));
		$num_rows         = $adb->num_rows ($result);
		$roleRelatedUsers = Array ();
		// Get the id and the name.
		while ($row = $adb->fetchByAssoc ($result)) {
			$roleRelatedUsers[ $row['userid'] ] = $row['first_name'] . ' ' . $row['last_name'];
		}
	...
	function addSharingRule ($tabid, $shareEntityType, $toEntityType, $shareEntityId, $toEntityId, $sharePermission) {
		...
		// Auto-corrección: Si shareid es 0, la tabla de secuencia está vacía o corrupta
		if ($shareid == 0) {
			// Obtener el máximo shareid existente y reinicializar la secuencia
			$maxResult = $adb->pquery ("SELECT COALESCE(MAX(shareid), 0) as max_id FROM vtiger_datashare_module_rel", array ());
			$maxShareId = ($maxResult && $adb->num_rows ($maxResult) > 0) ? $adb->query_result ($maxResult, 0, 'max_id') : 0;

			// Reinicializar la tabla de secuencia
			$adb->pquery ("DELETE FROM vtiger_datashare_module_rel_seq", array ());
			$adb->pquery ("INSERT INTO vtiger_datashare_module_rel_seq (id) VALUES (?)", array ($maxShareId));

			// Obtener el nuevo shareid
			$shareid = $adb->getUniqueId ("vtiger_datashare_module_rel");
			$log->debug ("addSharingRule: Tabla de secuencia auto-corregida, nuevo shareid: " . $shareid);
		}
	...
	function getCurrentUserProfileList () {
		...
		$profList = array ();
		$i        = 0;
		$current_user_profiles = isset($current_user_profiles) ? $current_user_profiles : array();
		if (is_array ($current_user_profiles)) {
	...
	function getPermittedModuleNames () {
		...
		if ($is_admin == false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1) {
			foreach ($tab_seq_array as $tabid => $seq_value) {
				...
				$permValue = isset($profileTabsPermission[$tabid]) ? $profileTabsPermission[$tabid] : 'NOT SET';
				if ($seq_value == 0 && $profileTabsPermission[ $tabid ] == 0) {
					$permittedModules[] = $moduleName;
				}
			}
		} else if (($tab_seq_array) && (is_array ($tab_seq_array))) {
			foreach ($tab_seq_array as $tabid => $seq_value) {
				$moduleName = getTabModuleName ($tabid);
				if ($seq_value == 0) {
					$permittedModules[] = $moduleName;
	...
	function getPermittedModuleIdList () {
		...
		//[ TT11181 ] Ajustes Menú Izquierdo (Módulos/APP´s) Producto Interno (Platzilla)
		//DM 22/06/2016
		//Leyendo de variable de session. No se escriben los archivos tabdata.php ni parent_tabdata.php
		...
		if ($is_admin == false && $profileGlobalPermission[1] == 1 &&
			...
			foreach ($tab_seq_array as $tabid => $seq_value) {
				//Cambiando validación ya que le valor no es un string sino un número $seq_value === 0
				if (($seq_value != '' && $seq_value == 0) && $profileTabsPermission[ $tabid ] === 0) {
					$permittedModules[] = ($tabid);
				}
			}
		} else if (is_array ($tab_seq_array)) {
			foreach ($tab_seq_array as $tabid => $seq_value) {
				if (($seq_value != '' && $seq_value == 0)) {
					$permittedModules[] = ($tabid);
				}
			}
		}
	 ...
	 *
	 * @param array|null $eliminateModules
	 *
	 * @return array
	 */
	function getSharingModuleList ($eliminateModules = null) {
		global $adb;
		...
		$questionMarks = str_repeat ('?, ', (count ($eliminateModules) - 1)) . '?';
		$result        = $adb->pquery (
			"SELECT
				name
			FROM
	...
	function isCalendarPermittedByInviteesId ($recordId) {
		global $adb;
		global $current_user;
		$permission = 'no';
		$query      = "SELECT * FROM vtiger_salesmanactivityrel WHERE activityid=? AND smid=?";
...
//end
	...
	function getDlgSupportPermissions ($id) {
		global $default_language, $adb, $mod_strings, $app_strings;

		$query               = "SELECT customerroleid FROM vtiger_users WHERE id = ?";
		$res                 = $adb->pquery ($query, array ($id));
		$customerroleid      = $adb->query_result ($res, 0, "customerroleid");
		$selectRolesCustomer = '
		<select class="small" id="customerroleid" name="customerroleid">';

		$roles = getSupportRoles ();
		for ($i = 0; $i < count ($roles); $i++) {
			$selected = '';
			if ($customerroleid == $roles[ $i ]['roleid']) {
				$selected = "selected";
			}
			$selectRolesCustomer .= '<option value="' . $roles[ $i ]['roleid'] . '" ' . $selected . '>' . $roles[ $i ]['rolename'] . '</option>';
		}
		$selectRolesCustomer .= '</select>';

		$bufferSalida = '
	<table class="tableHeading" border="0" cellpadding="5" cellspacing="0" width="100%">
	<tbody><tr>
		<td class="big"><strong>' . getTranslatedString ('LBL_SUPPORT_PERMISSIONS') . '</strong></td><td class="small" align="right">&nbsp;</td>
	</tr>
	</tbody></table>
	<table border="0" cellpadding="5" cellspacing="0" width="100%">
		<tbody>
		<tr>
		<td class="dvtCellLabel" align="right" width="25%"><input type="hidden" id="hdtxt_IsAdmin" value="1">' . getTranslatedString ('LBL_ROLE_CUSTOMER') . '</td>
		<td class="dvtCellInfo" align="left" width="25%&quot;">' . $selectRolesCustomer . '
		</td>
		</tr>
		</tbody>
	</table>
	<input type="button" class="crmbutton small create" value="' . getTranslatedString ('LBL_SAVE_BUTTON_TITLE') . '" title="' . getTranslatedString ('LBL_SAVE_BUTTON_TITLE') . '" onclick="jQuery.ajax({type: \'POST\',url: \'index.php\',data: { id: \'' . $id . '\', customerroleid: jQuery( \'#customerroleid\' ).val(), module: \'Users\',  action: \'UsersAjax\', file: \'customerRole\', Ajax: \'true\' }}).done(function() {jQuery(\'#dlgSupportPermissions\').slideUp(function(){OpenClosecortina()});});" />
	';
	...
	function getSupportRoles () {
		global $gInstanciaEmpresaFacil, $client, $Server_Path;
		if ($gInstanciaEmpresaFacil) {
			include_once ('webservice.empresafacil.php');
		} else {
			include_once ('webservice.time.php');
		}

		$params     = array (
			'user_name'     => _WEBSERVICE_USER,
		...
		$result = $client->call ('getCustomerRoles', $params, $Server_Path, $Server_Path);

		return $result[0]['roles'];
	}

	function updateValidateConfirmation ($number, $estado = 'false') {
		global $adb;

		$sql = "UPDATE vtiger_variables SET value = '" . $estado . "|" . (date ('Y-m-d H:i:s', strtotime ("+15 day"))) . "|" . $number . "' WHERE varname = 'CORREO_VERIFICADO'";
		$adb->query ($sql);
	}

	function getValidateMail () {
		global $current_user;
		if (!isset($_SESSION['validatemail']) && $current_user->id == 1) {//Solo el user admin debe validar el correo por plataforma
			$var = obtenerValorVariable ('CORREO_VERIFICADO', 'Users');
			list($estado, $date, $code) = explode ('|', $var);
			if ($estado == 'true' || !isset($var)) {
				return false;
			}
			$_SESSION['validatemail'] = true;

			updateValidateConfirmation ('');
	...
	function getProfileCustomViewsPermissions ($profileId, $userId) {
		global $adb;

		$result = $adb->pquery ('SELECT * FROM vtiger_profile2customview WHERE profileid=?', array ($profileId));
		if ((!$result) || ($adb->num_rows ($result) == 0)) {
			return array ();
		}
		$permissions = array ();
		while ($row = $adb->fetchByAssoc ($result, -1, false)) {
			$permissions [ $row ['cvid'] ] = $row ['permissions'];
		}

		$result = $adb->pquery ('SELECT * FROM vtiger_customview WHERE userid=?', array ($userId));
		if ((!$result) || ($adb->num_rows ($result) == 0)) {
			return $permissions;
		}
		while ($row = $adb->fetchByAssoc ($result, -1, false)) {
			$permissions [ $row ['cvid'] ] = 0;
		}
		return $permissions;
	}

	function getCombinedUserCustomViewsPermissions ($userId) {
		$profiles = getUserProfile ($userId);
		if (empty ($profiles)) {
			return array ();
		}
		$permissions = getProfileCustomViewsPermissions ($profiles [0], $userId);
		if (count ($profiles) == 1) {
			return $permissions;
		}
		foreach ($profiles as $profile) {
			$dummy = getProfileCustomViewsPermissions ($profile, $userId);
			foreach ($dummy as $cvId => $permission) {
				if ((array_key_exists ($cvId, $permissions)) && ($permissions [ $cvId ] == 0)) {
					continue;
				}
				$permissions [ $cvId ] = $permission;
			}
		}
		return $permissions;
	}

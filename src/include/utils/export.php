...
//Security Check
	...
	function export ($type) {
		...
		if (isset($_REQUEST['modexp'])) {

			$formatExport = $_REQUEST['modexp'];
		} else {
		...
		if (($search_type == 'withoutsearch' || $search_type == 'includesearch') && $export_data == 'selecteddata') {
			...
			if ($type == 'Documents' && count ($idstring) > 0) {
		...
		if ($formatExport == 'XML') {
			...
			$sqli       = "SELECT parent FROM vtiger_tab WHERE name = ?";
			$resulti    = $adb->pquery ($sqli, array ($moduleName));

			$parent = $adb->query_result ($resulti, 0, "parent");

			$sqlm    = "SELECT fieldlabel, fieldname FROM vtiger_field WHERE tabid = (SELECT tabid FROM vtiger_tab WHERE name = ?)";
			$resultm = $adb->pquery ($sqlm, array ($moduleName));
			$count   = $adb->num_rows ($resultm);
			...
			$label_fields_array = array ();

			for ($i = 0; $i < $count; $i++) {
				$new_fields_array               = array ();
				$new_fields_array['fieldlabel'] = $adb->query_result ($resultm, $i, "fieldlabel");
				$new_fields_array['fieldname']  = $adb->query_result ($resultm, $i, "fieldname");

				$label_fields_array[] = $new_fields_array;
			}
			...
			$xml .= '<!DOCTYPE ' . $parent . ' [ ';
			$xml .= "\r\n";
			$xml .= '<!ELEMENT ' . $parent . ' (' . $moduleName . '*)>';
			$xml .= "\r\n";

			$xml1 = "";
			while ($val = $adb->fetchByAssoc ($result, -1, false)) {
				$name_fields_array = array ();
				$val               = $__processor->sanitizeValues ($val);
				foreach ($val as $key => $value) {
					$arrayLabel = searchForFieldlabel ($key, $label_fields_array);

					if ($arrayLabel['fieldname'] != '') {
						$name_fields_array[] = $arrayLabel['fieldname'];
						$xml1 .= '<!ELEMENT ' . $arrayLabel['fieldname'] . ' (#PCDATA)>';
						$xml1 .= "\r\n";
						$xml1 .= '<!ATTLIST ' . $arrayLabel['fieldname'] . ' fieldlabel CDATA #IMPLIED>';
						$xml1 .= "\r\n";
					}
				}
				break;
			...
			$header = implode (", ", array_values ($name_fields_array));

			$xml .= '<!ELEMENT ' . $moduleName . ' (' . $header . ')>';
			$xml .= "\r\n";
			...
			while ($val = $adb->fetchByAssoc ($result, -1, false)) {
				...
				foreach ($val as $key => $value) {
					...
					if ($type == 'Documents' && $key == 'description') {
						...
						$xml .= "<" . $arrayLabel['fieldname'] . " fieldlabel='" . $arrayLabel['fieldlabel'] . "'>" . $value . "</" . $arrayLabel['fieldname'] . ">\n\t\t";
	...
	if (isset($_REQUEST['modexp']) && $_REQUEST['modexp'] == 'XML') {

		header ("Content-Disposition:attachment;filename=$moduleName.xml");
	...
	function searchForFieldlabel ($label, $array) {
		foreach ($array as $key => $val) {
			if ($val['fieldlabel'] === $label || strtolower ($val['fieldlabel']) === $label) {
				return $val;
			}
		}
		return null;

<?php
	...
	function is_superadmin ($user) {
		if (($user->id != 1) || (!empty ($_SESSION ['platInstancia']))) {
			return false;
		} else {
			return true;
		}
	...
	function get_select_options_array (&$option_list, $selected_key, $advsearch = 'false') {
		global $log;
	    //$log->debug ("Entering get_select_options_array (" . $selected_key . "," . $advsearch . ") method ...");
	...
	function get_options_array_seperate_key (&$label_list, &$key_list, $selected_key, $advsearch = 'false') {
		global $log;
		//$log->debug ("Entering get_options_array_seperate_key (" . $selected_key . "," . $advsearch . ") method ...");
		global $app_strings;
		if ($advsearch == 'true') {
	...
	function get_select_options_with_id_separate_key (&$label_list, &$key_list, $selected_key, $advsearch = 'false') {
		global $log;
		//$log->debug ("Entering get_select_options_with_id_separate_key(" . $selected_key . "," . $advsearch . ") method ...");
		global $app_strings;
		if ($advsearch == 'true') {
			$select_options = "\n<OPTION value=''>--NA--</OPTION>";
		} else {
	...
	function getAppCodeByTabId ($tabId) {
		global $adb;
		...
		$result = $adb->pquery (
			'SELECT
				  ...
				  INNER JOIN vtiger_configapps_tab cappt ON capp.config_applicationsid = cappt.config_applicationsid
				  WHERE
				  	capp.app_status=? AND
				  	cappt.tabid=?
				  	LIMIT 1',
	...
	function getTabid ($module) {
		...
		global $log, $bPlatHija, $modulesBasicosPlatzilla;//Esta variable se usa para poder obtener un tabid de una plataforma hija
		$log->debug ("Entering getTabid(" . $module . ") method ...");

		// Lookup information in cache first
		$tabid = VTCacheUtils::lookupTabid ($module);

		//if ($tabid === false && !$bPlatHija) {
		if ($tabid === false and !in_array ($module, $modulesBasicosPlatzilla)) {
			...
			if (isset($_SESSION['authenticated_user_menu']['tabdata']) && isset($_SESSION['authenticated_user_menu']['tabdata']['tab_info_array'])
				&& count ($_SESSION['authenticated_user_menu']['tabdata']['tab_info_array'] > 0)
			) {
				...
				// Validar que $adb esté inicializado
				if ($adb === null) {
					require_once('include/utils/AdbManager.class.php');
	...
	function getTabOwnedBy ($module) {
		...
		//DM 22/06/2016
		//Leyendo de variable de session. No se escriben los archivos tabdata.php
		if (isset($_SESSION['authenticated_user_menu']['tabdata']) && isset($_SESSION['authenticated_user_menu']['tabdata']['tab_ownedby_array'])
			&& count ($_SESSION['authenticated_user_menu']['tabdata']['tab_ownedby_array'] > 0)
		) {
			$tab_ownedby_array = $_SESSION['authenticated_user_menu']['tabdata']['tab_ownedby_array'];
	...
	/**
	 * Get the user's role name by giving the user id.   This method expects the user id
	 * DG 30 Aug 2006
	 */
	function getUserRoleName ($userid) {
		global $log;
		$log->debug ("Entering getUserRoleName(" . $userid . ") method ...");
		$log->info ("in getUserRoleName " . $userid);
		global $adb;
		$rolename = null;
		if ($userid != '') {
			list($rolename) = $adb->fetch_row ($adb->pquery ("SELECT rolename FROM vtiger_role INNER JOIN vtiger_user2role USING (roleid) WHERE userid=?", array ($userid)));
		}
		$log->debug ("Exiting getUserRoleName method ...");
		return $rolename;
	}
	 ...
	 * @param string $disp_view display view (edit,detail or create)
	 * @param string $mode edit
	 * @param string $col_fields * column vtiger_fields
	 ...
	 * @throws Exception
	 */
	function getBlocks ($module, $disp_view, $mode, $col_fields = '', $info_type = '', array $profileIds = null) {
		global $adb, $current_user, $local_user, $log;
		...
		if ($mode == 'edit') {
			$display_type_check = 'f.displaytype = 1';
		} elseif ($mode == 'mass_edit') {
			$display_type_check = 'f.displaytype = 1 AND f.masseditable NOT IN (0)';
		} else {
			$display_type_check = 'f.displaytype in (1, 4)';
		}

		if (!empty ($profileIds)) {
			$profileIds = join (', ', $profileIds);
		}
		...
		if ($disp_view == "detail_view") {
			if (($is_admin == true) || ($profileGlobalPermission [1] == 0) || ($profileGlobalPermission [2] == 0) || ($module == 'Users') || ($module == 'Emails')) {
				if (!empty ($profileIds)) {
					$profileJoinClause  = 'INNER JOIN vtiger_profile2field p2f ON p2f.fieldid=f.fieldid';
					$profileWhereClause = ' AND p2f.visible=0 AND p2f.profileid IN (' . generateQuestionMarks ($profileIds) . ')';
					$params             = array ($tabid, $blockid_list, $profileIds);
				} else {
					$profileJoinClause  = 'INNER JOIN vtiger_profile2field p2f ON p2f.fieldid=f.fieldid';
					$profileWhereClause = ' AND p2f.visible=0 AND p2f.profileid IN (?)';
					$params             = array ($tabid, $blockid_list, 1);
				}
				$sql = "SELECT
								f.*,
								'0' AS readonly
							...
							WHERE
								f.tabid=? AND
								f.block IN (" . generateQuestionMarks ($blockid_list) . ") AND
				...
				$sql    = "SELECT
								...
								WHERE
									...
									p2f.visible=0 AND
									dof.visible=0 AND
									p2f.profileid IN (" . generateQuestionMarks ($profileIds) . ")
								GROUP BY
									f.fieldid
								ORDER BY
									block, sequence";
				$params = array_merge (array ($tabid), $blockid_list, $profileIds);
				//Postgres 8 fixes
				if ($adb->dbType == "pgsql") {
			...
			if ($info_type != '') {
				if (($is_admin == true) || ($profileGlobalPermission [1] === 0) || ($profileGlobalPermission [2] === 0) || ($module == 'Users') || ($module == 'Emails')) {
					if (!empty ($profileIds)) {
						$profileJoinClause  = 'INNER JOIN vtiger_profile2field p2f ON p2f.fieldid=f.fieldid';
						$profileWhereClause = ' AND p2f.visible=0 AND p2f.profileid IN (' . generateQuestionMarks ($profileIds) . ') AND p2f.visible=0';
						$params             = array ($tabid, $blockid_list, $info_type, $profileIds);
					} else {
						$profileJoinClause  = 'INNER JOIN vtiger_profile2field p2f ON p2f.fieldid=f.fieldid';
						$profileWhereClause = ' AND p2f.visible=0 AND p2f.profileid IN ( ? ) AND p2f.visible=0';
						$params             = array ($tabid, $blockid_list, $info_type, 1);
					}
					$sql = "SELECT
									f.*
								FROM
					...
					$sql    = "SELECT
									...
									WHERE
										...
										p2f.readonly=0 AND
										dof.visible=0 AND
										p2f.profileid IN (" . generateQuestionMarks ($profileIds) . ") AND
										f.presence IN (0, 2)
									GROUP BY
										f.fieldid
									ORDER BY
				...
				if (($is_admin == true) || ($profileGlobalPermission [1] === 0) || ($profileGlobalPermission [2] === 0) || ($module == 'Users') || ($module == 'Emails')) {
					...
					if (!empty ($_SESSION ['platInstancia'])) { //NO MUESTRA EL ROL
						$sql = "SELECT
									f.*
								FROM
									vtiger_field f
...
//			$getBlockInfo = getBlockInformation ($module, $result, $col_fields, $tabid, $block_label, $mode);
			...
			$getBlockInfo = EditViewUtils::getBlockOptions ($adb, $module, $result, $col_fields, $block_label, $mode, $_SESSION ['plat'], $current_user);
		}
		$log->debug ("Exiting getBlocks method ...");
		if (count ($getBlockInfo) > 0) {
			$masterAdb = AdbManager::getInstance ()->getMasterAdb ();
			$helpField = HelpSettingsHelper::fetchHelpFieldByModule ($masterAdb, $module);
			foreach ($getBlockInfo as $label => $contents) {
				if (empty ($getBlockInfo[ $label ])) {
					unset ($getBlockInfo[ $label ]);
				} else if (!empty($helpField)) {
					foreach ($contents as $index => $fields){
						foreach ($helpField as $help) {
							$totalFields = count ($fields);
							for ($k = 0; $k < $totalFields; $k++) {
								if (isset($getBlockInfo[ $label ][ $index ][$k][2][0]) && $getBlockInfo[ $label ][ $index ][$k][2][0] == $help->getFieldName ()) {
									$getBlockInfo[ $label ][ $index ][$k][2][] = array (
										'idHelp' => $help->getId (),
										'title'  => $help->getTitle (),
										'module' => $help->getModuleName (),
									);
									break;
								}
							}
	...
	/**
	 * This function returns the vtiger_blocks and its related information for given label module.
	 * This function returns an array
	 */
	function getBlocksLabel ($module, $disp_view, $col_fields = '', $label_block) {
		global $log;
		//$log->debug ("[include\utils\CommonUtils.php::getBlocks] Entering getBlocksLabel(" . $module . "," . $disp_view . "," . $col_fields . "," . $label_block . ") method ...");
		global $adb, $current_user;
		global $mod_strings;
		$tabid = getTabid ($module);
		...
		$block_detail = Array ();
		$getBlockinfo = "";
		$query        = "select blockid,blocklabel,show_title,display_status from vtiger_blocks where tabid=? and blocklabel=? and $disp_view=0 and visible = 0 order by sequence";
		$result       = $adb->pquery ($query, array ($tabid, $label_block));
		...
		$log->debug ("Exiting getBlocksLabel method ...");
		if (count ($getBlockInfo) > 0) {
			foreach ($getBlockInfo as $label => $contents) {
				if (empty($getBlockInfo[ $label ])) {
					unset($getBlockInfo[ $label ]);
	...
	function getView ($mode) {
		global $log;
		//$log->debug ("[include\utils\CommonUtils.php::getView] Entering getView(" . $mode . ") method ...");
		if ($mode == "edit") {
			$disp_view = "edit_view";
		} else {
			$disp_view = "create_view";
		}
		//$log->debug ("[include\utils\CommonUtils.php::getView] Exiting getView method ...");
		return $disp_view;
	}
	...
	function getBlockId ($tabid, $label) {
		global $log;
		//$log->debug ("[include\utils\CommonUtils.php::getBlockId] Entering getBlockId(" . $tabid . "," . $label . ") method ...");
		global $adb;
		$blockid  = '';
		...
		//$log->debug ("[include\utils\CommonUtils.php::getBlockId] Exiting getBlockId method ...");
		return $blockid;
	}

	function getAppsColors ($coloresBarraMenu) {

		global $adb;
		$appColors = array ();

		$sql    = "SELECT config_applicationsid FROM `vtiger_config_applications`";
		$result = $adb->query ($sql);
		$i      = 0;
		while ($r = $adb->fetchByAssoc ($result)) {
			$appColors[ $r['config_applicationsid'] ] = $coloresBarraMenu[ $i++ ];
		}

		return $appColors;
	}

	function getAppIdforTabId ($tabId) {
		global $adb;

		$sql                   = "SELECT config_applicationsid FROM vtiger_configapps_tab WHERE tabid = ?";
		$result                = $adb->pquery ($sql, array ($tabId));
		$config_applicationsid = $adb->query_result ($result, 0, "config_applicationsid");

		return $config_applicationsid;
	}

	function getTabNameForMenu ($tabId) {
		global $adb;
		// this where option is temporary until we have a better way to handle this
		$isInstance = (!empty($_SESSION ['platInstancia'])) ? true : false;
		$tempWhere = '';
		if ($isInstance) {
			$tempWhere = " AND name NOT IN('questionnaire','answers')";
		}
		$sql      = "SELECT name,tablabel FROM vtiger_tab WHERE tabid = ? {$tempWhere}";
		$result   = $adb->pquery ($sql, array ($tabId));
		$tabName  = $adb->query_result ($result, 0, "name");
		$tabLabel = $adb->query_result ($result, 0, "tablabel");

		return array ('name' => $tabName, 'label' => $tabLabel);
	}

	function getParentTabNameForMenu ($tabId) {
		global $adb;
		$sql     = "SELECT parenttab_label FROM vtiger_parenttab WHERE parenttabid = ?";
		$result  = $adb->pquery ($sql, array ($tabId));
		$tabName = $adb->query_result ($result, 0, "parenttab_label");
		return $tabName;
	}

	function getStatusTab ($tabId) {
		global $adb;
		$avaliable      = 1;
		$isAdminVisible = 1;
		if (existeCampoTabla ('avaliable', 'vtiger_tab')) {
			if (empty ($_SESSION ['platInstancia'])) {
				$sql            = "SELECT avaliable, isvisibleinadmin FROM vtiger_tab WHERE tabid = ?";
				$result         = $adb->pquery ($sql, array ($tabId));
				$avaliable      = $adb->query_result ($result, 0, "avaliable");
				$isAdminVisible = empty ($_SESSION ['platInstancia']) ? $adb->query_result ($result, 0, "isvisibleinadmin") : 1;
			} else {
				$sql       = "SELECT avaliable FROM vtiger_tab WHERE tabid = ?";
				$result    = $adb->pquery ($sql, array ($tabId));
				$avaliable = $adb->query_result ($result, 0, "avaliable");
			}
		}

		return $avaliable && $isAdminVisible;
	}

	function getStatusParentTab ($parentTabId) {
		global $adb;
		$avaliable = 1;
		if (existeCampoTabla ('avaliable', 'vtiger_parenttab')) {
			$sql       = "SELECT avaliable FROM vtiger_parenttab WHERE parenttabid = ?";
	...
	function getParentTabIcoForMenu ($tabId) {
		global $adb;
		$sql    = "SELECT color,iconclass FROM vtiger_parenttab WHERE parenttabid = ?";
		$result = $adb->pquery ($sql, array ($tabId));
		$icon   = $adb->query_result ($result, 0, "iconclass");
		$color  = $adb->query_result ($result, 0, "color");
		return array ('iconclass' => $icon, 'color' => $color);
	}

	function getLastLogoutByCurrentUserid () {

		global $adb, $current_user;
		$login_id = 0;

		$sql      = "SELECT max(login_id) AS login_id FROM `vtiger_loginhistory` WHERE user_name = ? AND status = 'Signed off' ";
		$result   = $adb->pquery ($sql, array ($current_user->user_name));
		$login_id = $adb->query_result ($result, 0, "login_id");

		if ($login_id <> 0) {

			$sql    = "SELECT logout_time FROM `vtiger_loginhistory` WHERE login_id = ? AND status = 'Signed off' ";
			$result = $adb->pquery ($sql, array ($login_id));
			$fecha  = $adb->query_result ($result, 0, "logout_time");
		} else {
	...
	function getLastLoginByCurrentUserid () {

		global $adb, $current_user;
		$login_id = 0;

		$sql      = "SELECT max(login_id) AS login_id FROM `vtiger_loginhistory` WHERE user_name = ? AND status = 'Signed in' ";
		$result   = $adb->pquery ($sql, array ($current_user->user_name));
		$login_id = $adb->query_result ($result, 0, "login_id");

		if ($login_id <> 0) {

			$sql    = "SELECT login_time FROM `vtiger_loginhistory` WHERE login_id = ? AND status = 'Signed in' ";
			$result = $adb->pquery ($sql, array ($login_id));
			$fecha  = $adb->query_result ($result, 0, "login_time");
		} else {
...
//Funcion para contar los registro por el nombre de la entidad y agregar al menu.
	function countFilesByEntity ($name) {
		global $adb;

		$lastLogoutDate = getLastLogoutByCurrentUserid ();

		$lastLoginDate = getLastLoginByCurrentUserid ();

		$entityData = getEntityFieldNames ($name);

		$tablajoin = $entityData['tablename'];

		if ($tablajoin != '') {
			$result = $adb->pquery ('SHOW TABLES LIKE ?', array ($tablajoin));
			if ((!$result) || ($adb->num_rows ($result) == 0)) {
				return 0;
			}

			$tablaid = $entityData['entityidfield'];
			$sql     = "SELECT count( * ) total_r
			FROM vtiger_crmentity crm join $tablajoin on (crm.crmid = $tablaid)
			WHERE  crm.deleted=0  and crm.createdtime > ?";

			$result = $adb->pquery ($sql, array ($lastLoginDate));
			$icon   = $adb->query_result ($result, 0, "total_r");
			return $icon;
		} else {
			return false;
		}
	 ...
	 */
	function getHeaderArray ($hidden = array()) {
		global $log;
		$log->debug ("Entering getHeaderArray() method ...");
		$coloresBarraMenu = array (
			0 => '#3498db',
			1 => '#e74c3c',
			2 => '#f1c40f',
			3 => '#2ecc71',
		...
		$defaultColor     = '#2ecc71';
		$appColors        = getAppsColors ($coloresBarraMenu);
		global $current_user;
		$local_user = clone $current_user;
		//arrays de parent_tabdata_madre.php
		$parent_tab_info_array             = $_SESSION['authenticated_user_menu']['parent_tabdata_madre']['parent_tab_info_array'];
		$nombre_child_tab_rel_array        = $_SESSION['authenticated_user_menu']['parent_tabdata_madre']['nombre_child_tab_rel_array'];
		$parent_child_tab_rel_array_level4 = $_SESSION['authenticated_user_menu']['parent_tabdata_madre']['parent_child_tab_rel_array_level4'];
		require ('user_privileges/user_privileges.php');
		$contadorNivel1 = 0;

		// Nivel 1 del menú
		if (is_array ($parent_tab_info_array)) {
			foreach ($parent_tab_info_array as $parid => $parval) {
				if (in_array ($parid, $hidden)) {
					continue;
				}
				$contadorNivel1++;
				$temp1['id']        = $parid;
				$temp1['name']      = getTranslatedString ($parval);
				$tmpic              = getParentTabIcoForMenu ($parid);
				$temp1['iconclass'] = $tmpic['iconclass'];
				$temp1['color']     = $tmpic['color'];
				$relatedtabs2       = "";
				$contadorNivel2     = 0;
				// Si el nivel 1 tiene hijos
				$temp1['tieneHijos'] = 0;
				if (count ($parent_child_tab_rel_array_level4[ $parid ]) > 0) {
					$temp1['tieneHijos'] = 1;
					foreach ($parent_child_tab_rel_array_level4[ $parid ] as $paridN3) {
						if ($profileTabsPermission[ $paridN3 ] == 0) {
							$temp2['id']                       = $paridN3;
							$temp2['tieneHijos']               = 0;
							$info                              = getTabNameForMenu ($paridN3);
							$temp2['name']                     = $info['name']; // 'nombre nivel 3';
							$temp2['label']                    = getTranslatedString ($info['label']);
							$temp2['habilitado']               = getStatusTab ($paridN3);
							$temp2['count_registro']           = contadorAvanzado ($paridN3, $temp2['name']);
							$temp2['moduleaction']             = 'index';
							$temp2['color']                    = ($appColors[ getAppIdforTabId ($paridN3) ] != '') ? $appColors[ getAppIdforTabId ($paridN3) ] : $defaultColor;
							$temp2['parentabName']             = $nombre_child_tab_rel_array[ $paridN2 ];
							$temp2['esmodulodecampos']         = esModuloDeCampos ($paridN3);
							$temp2['app_code']                 = getAppCodeByTabId ($paridN3);
							$relatedtabs2[ $contadorNivel2++ ] = $temp2;
							$temp2                             = "";
						}
					}

					$temp1['elementos'] = $relatedtabs2;
				}

				$relatedtabs[ $contadorNivel1 ] = $temp1;
				$temp1                          = "";
			}
		}
	...
	function contadorAvanzado ($tabid, $name) {
		if (esModuloDeCampos ($tabid) == 'null') {
			return 'null';
		} else {
			$contador = countFilesByEntity ($name);
			if ($contador == 0) {
				return 'null';
			} else {
				return $contador;
	...
	function esModuloDeCampos ($tabid) {
		global $adb;

		$sql = "SELECT * FROM vtiger_entityname WHERE tabid = ? ";

		$q        = $adb->pquery ($sql, array ($tabid));
		$modulos  = array ();
	...
	function getOtrosModulos () {

		global $adb;
		global $current_user;

		$sql = "SELECT * FROM vtiger_tab WHERE isplatzilla = 1 AND presence = -1 AND avaliable = 1 ";
	...
	function getHeaderArray_old () {
		...
		foreach ($parent_tab_info_array as $parid => $parval) {
			...
			$subparid    = '';
			$submodule   = '';
			if (is_array ($sub_parent_tab_info_array)) {
				foreach ($sub_parent_tab_info_array[ $parid ] as $ksp => $subval) {
					$subparid  = $ksp;
					$submodule = $subval;
				}
			}
			$tablist_sub = $parent_child_tab_rel_array[ $subparid ];
			if (is_array ($tablist_sub)) {
				foreach ($tablist_sub as $chlid) {
					$module                      = array_search ($chlid, $tab_info_array);
					$subtabs_sub[ $submodule ][] = $module;
				}
			}
			...
			if (!empty($subtabs_sub)) {
				unset($parent_child_tab_rel_array[ $subparid ]);
				$subtabs[] = $subtabs_sub;
			}

			$parenttab = getParentTabName ($parid);

			$lstRoles = explode (',', obtenerValorVariable ('ROLES_ADM_MODULOS', 'Users'));
			if ($parenttab == 'Settings' && ($is_admin || in_array ($current_user->column_fields['roleid'], $lstRoles))) {
				$subtabs[] = 'Settings';
			}

			if ($parenttab != 'Settings' || ($parenttab == 'Settings' && ($is_admin || in_array ($current_user->column_fields['roleid'], $lstRoles)))) {
				if (!empty($subtabs) || checkSubTabs ($parenttab) > 0) {
					$relatedtabs[ $parenttab ] = $subtabs;
				}
			}
		}
		// si quito esto.. si carga el submenu en el menu cuando no hay modulos... pero no recuerdo ahora que otro efecto causaba
		// E.V. 2015-06-30
		// $relatedtabs=array_filter($relatedtabs);
		if (isset($relatedtabs['Web'])) {
			$addfrontendstomenu = getFrontends ();
			// El parenttab es quitado por codigo porque los nuevos parenttabs deben primero tomar sus modulos
			if (!empty($addfrontendstomenu)) {
				foreach ($addfrontendstomenu as $fr) {
					$pos      = array_search ('frontends', $relatedtabs['Web']);
					$addarray = $relatedtabs['Web'];
					unset($addarray[ $pos ]);
					$addarray['frontendsid']              = $fr['frontendsid'];
					$relatedtabs[ $fr['nombre_fontend'] ] = $addarray;
				}
				$relatedtabs['Web']['nonfrontend'] = true;
				unset($relatedtabs['Web']);
			}
		}
		$log->debug ("Exiting getHeaderArray method ...");
		return $relatedtabs;
	...
	function getColoredHeaders () {
		$pestanas = obtenerValorVariable ('MENU_COLOR_PESTANAS', 'Home');
		$pestanas = explode (';', $pestanas);
		$items    = array ();

		foreach ($pestanas as $k => $pestana) {
			if (empty($pestana)) {
				continue;
			}

			$pestana                            = explode (':', $pestana);
			$items[ $pestana[0] ]['from_color'] = $pestana[1];
			$items[ $pestana[0] ]['to_color']   = $pestana[2];
			$items[ $pestana[0] ]['text_color'] = $pestana[3];
		}

		return $items;
	}

	function getColoredHeadersFromParenttab () {
		global $adb;
		existeCampoTabla ('padre', 'vtiger_parenttab', 'ALTER TABLE `vtiger_parenttab` ADD COLUMN `padre` VARCHAR(50) NULL;');
		existeCampoTabla ('color', 'vtiger_parenttab', 'ALTER TABLE `vtiger_parenttab` ADD COLUMN `color` VARCHAR(50) NULL;');
		existeCampoTabla ('iconclass', 'vtiger_parenttab', 'ALTER TABLE `vtiger_parenttab` ADD COLUMN `iconclass` VARCHAR(50) NULL;');
		$sql          = "SELECT parenttab_label,color,iconclass FROM vtiger_parenttab WHERE visible=0";
		$result       = $adb->query ($sql);
		$menu_replace = array (
			'�'        => 'a', '�' => 'e', '�' => 'i', '�' => 'o', '�' => 'u', '�' => 'A', '�' => 'E', '�' => 'I', '�' => 'O', '�' => 'U',
		...
		while ($r = $adb->fetchByAssoc ($result)) {
			foreach ($menu_replace as $kr => $fr) {
				$r['parenttab_label'] = str_ireplace ($kr, $fr, $r['parenttab_label']);
			}
			$items[ $r['parenttab_label'] ] = array ('color' => $r['color'], 'iconclass' => $r['iconclass']);
		}
	...
	function getParentTabName ($parenttabid) {
		...
		//[ TT11181 ] Ajustes Menú Izquierdo (Módulos/APP´s) Producto Interno (Platzilla)
		//DM 28/06/2016
		//Leyendo de variable de session. No se escriben los archivos parent_tabdata.php
		if (isset($_SESSION['authenticated_user_menu']['parent_tabdata']) && isset($_SESSION['authenticated_user_menu']['parent_tabdata']['parent_tab_info_array'])
			&& count ($_SESSION['authenticated_user_menu']['parent_tabdata']['parent_tab_info_array'] > 0)
		) {

			$parent_tab_info_array = $_SESSION['authenticated_user_menu']['parent_tabdata']['parent_tab_info_array'];
	 ...
	 * checkSubTabs
	 */
	function checkSubTabs ($parenttab) {
		global $adb;
		// echo "<pre>".print_r($parenttab,true)."</pre>";
		if (!existeCampoTabla ('padre', 'vtiger_parenttab')) {
			return 0;
		}
		// $sql = "SELECT COUNT(1) AS subsubmenus FROM `vtiger_parenttab` WHERE `padre`!=0 AND `padre` IS NOT NULL";
		$sql         = "SELECT COUNT(1) AS subsubmenus FROM `vtiger_parenttab` ptb
			INNER JOIN `vtiger_parenttab` ptb1 ON ptb1.`parenttabid`=ptb.`padre` AND ptb1.`parenttab_label`='" . $parenttab . "' ";
		$result      = $adb->query ($sql);
		$subsubmenus = $adb->query_result ($result, 0, "subsubmenus");
		return $subsubmenus;
	}
	...
	function getParentTabFromModule ($module) {
		...
		if ((isset($_SESSION['authenticated_user_menu']['tabdata']) && count ($_SESSION['authenticated_user_menu']['tabdata']) > 0) &&
			(isset($_SESSION['authenticated_user_menu']['parent_tabdata']) && count ($_SESSION['authenticated_user_menu']['parent_tabdata']) > 0)
		) {

			$tab_info_array             = $_SESSION['authenticated_user_menu']['tabdata']['tab_info_array'];
			$parent_child_tab_rel_array = $_SESSION['authenticated_user_menu']['parent_tabdata']['parent_child_tab_rel_array'];
			$parent_tab_info_array      = $_SESSION['authenticated_user_menu']['parent_tabdata']['parent_tab_info_array'];

			$parent_tabname = null; // Initialize variable to avoid undefined variable notice
			$tabid = $tab_info_array[ $module ];
	...
	function checkParentTabExists ($parenttab) {
		...
		//[ TT11181 ] Ajustes Menú Izquierdo (Módulos/APP´s) Producto Interno (Platzilla)
		//DM 22/06/2016
		//Eliminando la lectura del archivo parent_tabdata.php y leyendo a trav[es de lavariable de session
		if ((isset($_SESSION['authenticated_user_menu']['parent_tabdata']) && count ($_SESSION['authenticated_user_menu']['parent_tabdata']) > 0)) {
			$parent_tab_info_array = $_SESSION['authenticated_user_menu']['parent_tabdata']['parent_tab_info_array'];
	...
	function create_tab_data_file () {
		...
		//arrays de tabdata.php
		$_SESSION['authenticated_user_menu']['tabdata']['tab_info_array']    = $result_array;
		$_SESSION['authenticated_user_menu']['tabdata']['tab_seq_array']     = $seq_array;
		$_SESSION['authenticated_user_menu']['tabdata']['tab_ownedby_array'] = $ownedby_array;
		$_SESSION['authenticated_user_menu']['tabdata']['action_id_array']   = $actionid_array;
		$_SESSION['authenticated_user_menu']['tabdata']['action_name_array'] = $actionname_array;

		//arrays de tabdata_madre.php
		$_SESSION['authenticated_user_menu']['tabdata_madre']['tab_info_array']    = $result_array;
		$_SESSION['authenticated_user_menu']['tabdata_madre']['tab_seq_array']     = $seq_array;
		$_SESSION['authenticated_user_menu']['tabdata_madre']['tab_ownedby_array'] = $ownedby_array;
		$_SESSION['authenticated_user_menu']['tabdata_madre']['action_id_array']   = $actionid_array;
		$_SESSION['authenticated_user_menu']['tabdata_madre']['action_name_array'] = $actionname_array;

		//echo "<pre>".print_r($_SESSION['authenticated_user_menu']['tabdata']['tab_info_array'],true)."</pre>";
	...
	function create_parenttab_data_file () {
		...
		global $adb;
		$sql             = "SELECT parenttabid,parenttab_label FROM vtiger_parenttab WHERE visible=0 AND padre = 0 ORDER BY sequence";
		$result          = $adb->pquery ($sql, array ());
		$num_rows        = $adb->num_rows ($result);
		$result_array    = Array ();
		$result_array_L1 = array ();
		for ($i = 0; $i < $num_rows; $i++) {
			...
			$result_array_L1[ $parenttabid ] = $parenttab_label;
		}
		require_once ('modules/Users/CreateUserPrivilegeFile.php');
	...
	foreach ($result_array as $parid => $parvalue) {
		...
		$sql = "select * from vtiger_parenttab where padre=? and visible=0
					order by sequence";
		// END
		...
		$result_array = Array();
		for ($i = 0; $i < $num_rows; $i++) {
			$tabid = $adb->query_result($result, $i, 'parenttabid');
			$childArray[] = $tabid;

			// Llenando nombres
			$parenttabid_ = $adb->query_result($result, $i, 'parenttabid');
			$parenttab_label_ = $adb->query_result($result, $i, 'parenttab_label');
			$result_array_name[$parenttabid_] = $parenttab_label_;
	...
	*/
		...
		/*
	// consultndo nivel 3 parenttab
	$L4 = array();
	foreach ($result_array_name as $parid => $parvalue) {
		$childArrayL3 = Array();
		...
		$parChildTabRelArrayL3[$parid] = $childArrayL3;
	}

	// consultndo nivel 3 parenttabrel
	foreach ($result_array_name as $parid => $parvalue) {
		$childArrayL3Single = Array();

		$sql = "select * from vtiger_parenttabrel where parenttabid=? and tabid in (select tabid from vtiger_tab where presence in (0,2))
					order by sequence";
		// END
		$result = $adb->pquery($sql, array($parid));
		$num_rows = $adb->num_rows($result);
		...
		for ($i = 0; $i < $num_rows; $i++) {
			$tabid = $adb->query_result($result, $i, 'tabid');
			$childArrayL3Single[] = $tabid;
			$L4[] = $tabid;

		}
		$parChildTabRelArrayL3Single[$parid] = $childArrayL3Single;
	}
	...
	// nivel 4
	foreach ($L4 as $parid => $parvalue) {
		$childArrayL4 = Array();

		$sql = "select * from vtiger_parenttabrel where parenttabid=? and tabid in (select tabid from vtiger_tab where presence in (0,2))
					order by sequence";
		// END
		$result = $adb->pquery($sql, array($parvalue));
		$num_rows = $adb->num_rows($result);
		for ($i = 0; $i < $num_rows; $i++) {
			$tabid = $adb->query_result($result, $i, 'tabid');
			$childArrayL4[] = $tabid;
		...
		$parChildTabRelArrayL4[$parvalue] = $childArrayL4;
	}

	*/

		//tomando hijos del nivel 2 en parenttabrel
		//foreach ($result_array_L1 as $parid => $parvalue) {
		foreach ($result_array as $parid => $parvalue) {
			...
			for ($i = 0; $i < $num_rows; $i++) {
				$tabid             = $adb->query_result ($result, $i, 'tabid');
				$result_array_name = $adb->query_result ($result, $i, 'name');
				$childArrayL4[]    = $tabid;
			}
			$parChildTabRelArrayL4[ $parid ] = $childArrayL4;
			//parent_child_tab_rel_array_level4
			//parChildTabRelArrayL4
		}

		$parent_child_tab_rel_array = $parChildTabRelArray;
		if (!empty($result_array_name)) {
			$nombre_child_tab_rel_array = $result_array_name;
		} else {
			$nombre_child_tab_rel_array = '';
		}

		if (!empty($parChildTabRelArrayL3)) {
			$parent_child_tab_rel_array_level3 = $parChildTabRelArrayL3;
		} else {
			$parent_child_tab_rel_array_level3 = '';
		}

		if (!empty($parChildTabRelArrayL3Single)) {
			$parent_child_tab_rel_array_level3_single = $parChildTabRelArrayL3Single;
		} else {
			$parent_child_tab_rel_array_level3_single = '';
		}

		$parent_child_tab_rel_array_level4 = $parChildTabRelArrayL4;

		//arrays de parent_tabdata.php
		$_SESSION['authenticated_user_menu']['parent_tabdata']['parent_tab_info_array']                    = $parent_tab_info_array;
		$_SESSION['authenticated_user_menu']['parent_tabdata']['parent_child_tab_rel_array']               = $parent_child_tab_rel_array;
		$_SESSION['authenticated_user_menu']['parent_tabdata']['nombre_child_tab_rel_array']               = $nombre_child_tab_rel_array;
		$_SESSION['authenticated_user_menu']['parent_tabdata']['parent_child_tab_rel_array_level3']        = $parent_child_tab_rel_array_level3;
		$_SESSION['authenticated_user_menu']['parent_tabdata']['parent_child_tab_rel_array_level3_single'] = $parent_child_tab_rel_array_level3_single;
		$_SESSION['authenticated_user_menu']['parent_tabdata']['parent_child_tab_rel_array_level4']        = $parent_child_tab_rel_array_level4;

		//arrays de parent_tabdata_madre.php
		$_SESSION['authenticated_user_menu']['parent_tabdata_madre']['parent_tab_info_array']                    = $parent_tab_info_array;
		$_SESSION['authenticated_user_menu']['parent_tabdata_madre']['parent_child_tab_rel_array']               = $parent_child_tab_rel_array;
		$_SESSION['authenticated_user_menu']['parent_tabdata_madre']['nombre_child_tab_rel_array']               = $nombre_child_tab_rel_array;
		$_SESSION['authenticated_user_menu']['parent_tabdata_madre']['parent_child_tab_rel_array_level3']        = $parent_child_tab_rel_array_level3;
		$_SESSION['authenticated_user_menu']['parent_tabdata_madre']['parent_child_tab_rel_array_level3_single'] = $parent_child_tab_rel_array_level3_single;
		$_SESSION['authenticated_user_menu']['parent_tabdata_madre']['parent_child_tab_rel_array_level4']        = $parent_child_tab_rel_array_level4;
	}
	 ...
	 * returns null
	 */
	function create_parenttab_data_file_old () {
		global $log;
		$log->debug ("Entering create_parenttab_data_file() method ...");
		$log->info ("creating parent_tabdata file");
		global $adb;
		...
		if (isset($_SESSION['plat'])) {
			if (!is_dir ($_SESSION['plat'])) {
				$oldumask = umask (0);
				mkdir ($_SESSION['plat'], 0777, true);
				umask ($oldumask);
			}
			$filename = $_SESSION['plat'] . '/parent_tabdata.php';
		} else {
			$filename = 'parent_tabdata_madre.php';
		}
 ...
 */
	...
	function getAllParenttabmoduleslist () {
		...
		// vtlib customization: Disabling the tab item based on presence
		// $query = 'select name,tablabel,parenttab_label,vtiger_tab.tabid,vtiger_parenttabrel.sequence,vtiger_parenttab.`parenttabid` from vtiger_parenttabrel inner join vtiger_tab on vtiger_parenttabrel.tabid = vtiger_tab.tabid inner join vtiger_parenttab on vtiger_parenttabrel.parenttabid = vtiger_parenttab.parenttabid and vtiger_tab.presence in (0,2) order by vtiger_parenttab.sequence, vtiger_parenttabrel.sequence';
		$query = 'SELECT NAME,tablabel,parenttab_label,vtiger_tab.tabid,vtiger_parenttabrel.sequence,vtiger_parenttab.`parenttabid`
				FROM vtiger_parenttab
				LEFT JOIN vtiger_parenttabrel ON vtiger_parenttabrel.parenttabid = vtiger_parenttab.parenttabid
				LEFT JOIN vtiger_tab ON vtiger_parenttabrel.tabid = vtiger_tab.tabid AND vtiger_tab.presence IN (0,2)
				ORDER BY vtiger_parenttab.sequence, vtiger_parenttabrel.sequence';
		// END
		...
		for ($i = 0; $i < $adb->num_rows ($result); $i++) {
			...
			$tabid         = $adb->query_result ($result, $i, 'tabid');
			$parenttabid   = $adb->query_result ($result, $i, 'parenttabid');
			$sequence      = (int) $adb->query_result ($result, $i, 'sequence');
			if (!isset($resultant_array[ $parenttabname ])) {
				$subparent = array ();
				$subparent = getParentTabChild ($parenttabid);
				if (!empty($subparent)) {
					$saved_parenttabname[ $parenttabname ] = $subparent;
					// echo "<pre>".print_r($saved_parenttabname,true)."</pre>";
				}
			}
			if (!$tabid) {
				$resultant_array[ $parenttabname ] = Array ();
			} elseif ($is_admin) {
				$resultant_array[ $parenttabname ][ $sequence ] = Array ($modulename, $tablabel, $sequence);
			} elseif ($profileGlobalPermission[2] == 0 || $profileGlobalPermission[1] == 0 || $profileTabsPermission[ $tabid ] === 0) {
				$resultant_array[ $parenttabname ][ $sequence ] = Array ($modulename, $tablabel, $sequence);
			}
		}
		// echo "<pre>".print_r($saved_parenttabname,true)."</pre>";
		// echo "<pre>".print_r($resultant_array,true)."</pre>";exit;
		if (!empty($saved_parenttabname)) {
			foreach ($saved_parenttabname as $svpt => $subm) {
				foreach ($subm as $seq => $submb) {
					// echo "<pre>".print_r($resultant_array[$svpt][$submb['parenttab_label']],true)."</pre>";
					// echo "<pre>".print_r($resultant_array[$submb['parenttab_label']],true)."</pre>";
					// echo "<pre>".print_r($resultant_array[$submb['parenttab_label']],true)."</pre>";
					// $resultant_array[$svpt][$seq]=$resultant_array[$submb['parenttab_label']];
					unset($reindex);
					foreach ($resultant_array[ $submb['parenttab_label'] ] as $resarr) {
						$reindex[] = $resarr;
					}
					if (!empty($reindex)) {
						$resultant_array[ $svpt ][ $seq ] = $reindex;
						$submenu_name[ $svpt ][ $seq ]    = $submb['parenttab_label'];
					}
				}
			}
		}
		foreach ($resultant_array as $pt => $seq) {
			ksort ($resultant_array[ $pt ]);
		}
		// echo "<pre>".print_r($resultant_array,true)."</pre>";
		// exit;
		...
		if ($is_admin) {
			if ($empresaFacil != 'true' && $gestoriaFacil != 'true' && $clienteGestoriaFacil != 'true') {
				$resultant_array['Settings'][] = Array ('Settings', 'Settings');
			}
		}
		$lstRoles = explode (',', obtenerValorVariable ('ROLES_ADM_MODULOS', 'Users'));
		if ($is_admin || in_array ($current_user->column_fields['roleid'], $lstRoles)) {
			if ($empresaFacil != 'true' && $gestoriaFacil != 'true' && $clienteGestoriaFacil != 'true') {
				$resultant_array['Settings'][] = Array ('Settings', getTranslatedString ('VTLIB_LBL_MODULE_MANAGER', 'Settings'), '0', 'ModuleManager');
			}
		}
		// Added for get frontends.
		$addfrontendstomenu = getFrontends ();
		if (!empty($addfrontendstomenu)) {
			foreach ($addfrontendstomenu as $fr) {
				$addarray                              = $resultant_array['Web'];
				$resultant_array['Web']['nonfrontend'] = true;
				if (is_array ($addarray)) {
					foreach ($addarray as $k => $aa) {
						if (preg_match ('/frontends/i', $aa[0])) {
							unset($addarray[ $k ]);
						}
					}
				}
				$addarray['frontendsid']                  = $fr['frontendsid'];
				$resultant_array[ $fr['nombre_fontend'] ] = $addarray;
			}
			unset($resultant_array['Web']);
		}
		$resultant_array = array_filter (array_map ('array_filter', $resultant_array));
		return array ('menu' => $resultant_array, 'trans_subm' => $submenu_name);
	}

	function getParentTabChild ($parenttabid) {
		global $adb;
		...
		$sql = "SELECT * FROM `vtiger_parenttab` WHERE padre='" . $parenttabid . "'";
		$q   = $adb->query ($sql);
		if (!$adb->num_rows ($q)) {
			return null;
		}
	...
	function decideFilePath () {
		...
		$filepath = '';
		if (isset($_SESSION['plat'])) {
			if (!is_dir ($_SESSION['plat'] . '/storage')) {
				$oldumask = umask (0);
				mkdir ($_SESSION['plat'] . '/storage', 0777, true);
				umask ($oldumask);
			}
	...
	function getTemplateDetails ($templateid) {
		...
		$returndata   = Array ();
		$result       = $adb->pquery ("SELECT * FROM vtiger_emailmanager_template WHERE templateid=?", array ($templateid)); // MA | 08/10/2015
...
//End : Custom date & time fields
	 ...
	 */
	function getPickListValues ($tablename, $roleid, $encode = true) {
		global $adb;
		...
		$fldVal = Array ();
		while ($row = $adb->fetch_array ($result, $encode)) {
	...
	/**
	 * Obtiene los valores disponibles para el picklist estimated_time_unit
	 * @return array Array asociativo con key=>value de las unidades de tiempo
	 */
	function getAvailableEstimatedTimeUnits() {
		global $adb;

		$sql = "SELECT estimated_time_unit FROM vtiger_estimated_time_unit ORDER BY estimated_time_unitid";
		$result = $adb->query($sql);
		$units = array();
	...
	function checkFileAccessForInclusion ($filepath) {
		...
		if (stripos ($realfilepath, $rootdirpath) !== 0 || in_array ($filePathParts[0], $unsafeDirectories)) {
			throw new Exception ('Sorry! Attempt to access restricted file. !' . $filepath);
		}
	}

	/** Function to check the file access is made within web root directory and whether it is not from unsafe directories */
	function boleanCheckFileAccessForInclusion ($filepath) {
		global $root_directory;
		// Set the base directory to compare with
		$use_root_directory = $root_directory;
		if (empty($use_root_directory)) {
...
// END
	...
	function getCustomHeadIcons () {
		global $adb;

		$query  = "SELECT * FROM `vtiger_links` WHERE linktype='HEADERBUTTON' AND tabid=0 ORDER BY sequence; ";
		$result = $adb->pquery ($query, array ());
		while ($r = $adb->fetchByAssoc ($result)) {
			$icons[] = $r;
		}
	...
	function esVistaCliente ($id) {
		global $adb, $current_user, $plat_madre_empresafacil;

		if (existeCampoTabla ('roleid_client', 'vtiger_organizationdetails')) {

			$sql    = "SELECT roleid_client FROM vtiger_organizationdetails";
			$result = $adb->pquery ($sql, array ());
			$roleid = $adb->query_result ($result, 0, 'roleid_client');

			if ($roleid == $current_user->column_fields['roleid']) {
				return true;
			}
		}

		if (existeCampoTabla ('iscustomer', 'vtiger_role')) {

			$sql        = "SELECT iscustomer FROM vtiger_role WHERE roleid = ?";
			$result     = $adb->pquery ($sql, array ($current_user->column_fields['roleid']));
			$iscustomer = $adb->query_result ($result, 0, 'iscustomer');
		...
		// if ($_SESSION['plat'] != $plat_madre_empresafacil) {
		// echo "en el segundo if";//Ojo ac� hay que ver como luego se extrapola este nuevo esquema a relaciones madre/hijas
		// return true;
		// }
		return false;
	}

	function esVistaPartner ($id) {
		global $adb, $current_user;

		if (existeCampoTabla ('iscustomer', 'vtiger_role')) {
			$sql       = "SELECT ispartner FROM vtiger_role WHERE roleid = ?";
			$result    = $adb->pquery ($sql, array ($current_user->column_fields['roleid']));
			$ispartner = $adb->query_result ($result, 0, 'ispartner');
	...
	function sendNotificationButton ($module, $recordid) {
		global $adb;
		$sendNotifications      = null;
		$sendNotificationButton = "";

		$result = $adb->query ("SHOW COLUMNS FROM vtiger_tab where field='sends_notifications'");

		if ($adb->num_rows ($result) > 0) {
			$result = $adb->pquery ("SELECT sends_notifications FROM vtiger_tab WHERE name=?", array ($module));
			if ($adb->num_rows ($result) > 0) {
				list($sendNotifications) = $adb->fetch_row ($result);
			}
			if ($sendNotifications == '1') {
				$lbl                    = getTranslatedString ("LBL_SEND_NOTIFICATION");
				$sendNotificationButton = "<input type=\"button\" value=\"$lbl\" onclick=\"loadComposeUI('$recordid');\" class=\"crmbutton small cancel\">";
			}
		}
	...
	/*
	Function giving the list of date type fields
	Return:
		List date fields: Array;
*/
	function getDataFields ($module) {

		global $adb;

		$lstDataFields = array ();

		$query = "SELECT fieldname, fieldlabel FROM vtiger_field INNER JOIN vtiger_tab ON (vtiger_field.tabid = vtiger_tab.tabid AND vtiger_field.uitype IN (5,6,23,70) AND vtiger_field.presence IN (0,2))
		...
		return $lstDataFields;
	}

	function saveRecordModule ($input_array) {
		global $adb, $log;
	...
	function encrypt_password ($user_password, $username, $crypt_type = '') {
	...
	function getFileFieldValue ($module, $fieldname, $recordid) {
		global $adb;
		...
		$fieldres   = $adb->pquery ($sql, array ($tabid, $fieldname));
		$tablename  = $adb->query_result ($fieldres, 0, 'tablename');
		$columnname = $adb->query_result ($fieldres, 0, 'columnname');

		$sql      = "SELECT " . $columnname . " FROM " . $tablename . " WHERE " . $focus->tab_name_index[ $tablename ] . " = ?";
		$valueres = $adb->pquery ($sql, array ($recordid));
		$value    = $adb->query_result ($valueres, 0, $columnname);
	 ...
	 **/
	function downloadContent ($strFileType, $strFileName, $fileContent) {
	 ...
	 */
	function getFieldInformation ($tabid, $fieldname) {
		global $adb;
	...
	/**
	 * Function to chack if a table exists
	 *
	 * @param String $tablename
	 */
	function checkDBTableExist ($tablename) {
		/*global $adb;
	...
	if($result!==FALSE)
		return true;
	return false;*/
		return existeTabla ($tablename);
	}

	function existeCampoTabla ($field, $table, $altertable = '') {
		global $adb;
		$sqlcheck    = "SHOW COLUMNS FROM $table WHERE Field = '$field'";
		$checkresult = $adb->pquery ($sqlcheck, array ());
		if ($adb->num_rows ($checkresult) < 1) {
			if ($altertable != '') {
				if ($adb->query ($altertable)) {
					return true;
				}
			}
			return false;

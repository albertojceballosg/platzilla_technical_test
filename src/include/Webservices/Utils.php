<?php
	...
	function vtws_getModuleNameList () {
		...
		$sql       = "SELECT name FROM vtiger_tab WHERE isentitytype=1 AND name NOT IN ('Rss','Webmails','Events') ORDER BY tabsequence";
	...
	function vtws_addDefaultActorTypeEntity ($actorName, $actorNameDetails, $withName = true) {
		$actorHandler = array (
			'file'  => 'include/Webservices/VtigerActorOperation.php',
	...
	function vtws_addWebserviceOperation ($name, $handlerFilePath, $handlerMethodName, $requestType, $preLogin = 0) {
		...
		$result      = $adb->pquery ($createOperationQuery, array (
			$operationId, $name, $handlerFilePath, $handlerMethodName,
	...
	function vtws_transferOwnership ($ownerId, $newOwnerId, $delete = true) {
		...
		*/
		...
		foreach ($it as $row) {
			...
			if (!in_array ($column, $columnList) && !empty($row->tablename) && !empty($row->columnname)) {
				$columnList[] = $column;

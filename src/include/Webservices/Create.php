...
function vtws_create($elementType, $element, $user) {
	...
	/** this is validation errar always active */
    if (!in_array($elementType, $types['types']) && false) {
    ...
    foreach ($referenceFields as $fieldName => $details) {
        if (isset($element[$fieldName]) && strlen($element[$fieldName]) > 0) {
			...
			// validation error for the reference field
            if (!in_array($referenceObject->getEntityName(), $types['types']) && $referenceObject->getEntityName() != 'Users' && false) {

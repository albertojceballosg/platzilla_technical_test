...
class CurrencyField {
    ...
    public static function convertToUserFormat($value, $user=null, $skipConversion=false) {
	 ...
	 public static function currencyValue2Number($n) {
		global $current_user, $default_charset;
		
		$thousands_sep = html_entity_decode($current_user->currency_grouping_separator, ENT_QUOTES, $default_charset);
		$dec_sep = html_entity_decode($current_user->currency_decimal_separator, ENT_QUOTES, $default_charset);
		
		
		$n = str_replace($thousands_sep, '', $n);
		$n = str_replace($dec_sep, '.',$n);

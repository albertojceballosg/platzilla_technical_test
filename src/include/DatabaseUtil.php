...
function mkCountQuery($query)
{
	...
	$lstSelects = explode(' UNION ',$query);
	$query = '';
	for ($i = 0;$i < count($lstSelects);$i++) {
		$queryTmp = "SELECT count(*) AS counts ".substr($lstSelects[$i], stripos($lstSelects[$i],' FROM '),strlen($lstSelects[$i]));
	...
	$query = "SELECT sum(counts) as count FROM (".$query.") as t";

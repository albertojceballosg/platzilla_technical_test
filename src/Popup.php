...
//--------- pedido [TT11231] Modificado por Keyla Rodriguez----------------------------------------------------------------------
...
if ( (isset($_REQUEST['return_module'])) && ($_REQUEST['return_module'] !='')
	&& ($_REQUEST['recordid'] != '') && ($_REQUEST['es_patron'] == 1) &&
	 (isset($_REQUEST['module']))  && ($_REQUEST['module'] !='') ) {

		$tabid 			= PlatformUtils::getModuleId ($adb, $_REQUEST['module']);
		$query 		 	= $adb->pquery("SELECT * FROM vtiger_entityname  WHERE tabid = ?", Array($tabid));
		$tablename		= $adb->query_result($query,0,'tablename');

			$where.= $tablename.".es_patron=1";
}
...
$parameters = html_entity_decode(obtenerValorVariable($_REQUEST['forfield'],$_REQUEST['srcmodule']),ENT_QUOTES);
...
switch($currentModule)
{
	case 'Contacts':
		...
		//--------- pedido [TT11231] Modificado por Keyla Rodriguez----------------------------------------------------------------------
		if (isset($_REQUEST['patron'])) $smarty->assign("PATRON",'enable');
		//----------------------------------------------------------------------------------------------------------------
	...
	case 'proveedor':
		...
		if(isset($_REQUEST['return_module']) && $_REQUEST['return_module'] !='')
			$smarty->assign("RETURN_MODULE",vtlib_purify($_REQUEST['return_module']));
		$alphabetical = AlphabeticalSearch($currentModule,'Popup','proveedorid','true','basic',$popuptype,"","",$url);
		break;
	case 'SalesOrder':
	...
	default:
		if (file_exists($_SESSION['plat']."/modules/$currentModule/$currentModule.php")) {
			...
			$smarty->assign("PLATJS", $_SESSION['plat'].'/');
		}
		else {
			require_once("modules/$currentModule/$currentModule.php");
		}
...
else
{
	...
	$filtro_proyectos=obtenerValorVariable('POPUP_FILTRO_QUERY','proyectos');
	if($currentModule == 'proyectos' && $_REQUEST['recordid']!='' && $filtro_proyectos!=''){
		$where_relquery.=html_entity_decode($filtro_proyectos);
		$smarty->assign("RECORDID",vtlib_purify($_REQUEST['recordid']));
	}
...
if (isset($lstParameters['filter']) && !empty($lstParameters['filter'])) {
	if (strstr($query,strtoupper("WHERE"))) {
		$query .= ' and ';
	} else {
		$query .= ' where ';
	}
	$query.= $lstParameters['filter'];
}
...
//----- pedido [TT11231]-----//
if(isset($_REQUEST['patron']) && $_REQUEST['patron'] == 'enable')
	$url_string .='&patron=enable';
//-----fin pedido [TT11231]-----//
if(isset($_REQUEST['return_module']) && $_REQUEST['return_module'] != '')
...
$smarty->assign("CTRLID", vtlib_purify($_REQUEST['ctrlid']));
$smarty->assign("CTRLNAME", vtlib_purify($_REQUEST['ctrlname']));

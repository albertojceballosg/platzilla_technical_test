...
function GetForgotPasswordUI($mail_send_message='')
{
    ...
    $list .= '<input type="hidden" name="login_language" value="'.$default_language.'">';  
    $list .= '<input type="hidden" name="param" value="forgot_password">';
    $list .= '<table width="80%" border="0" cellspacing="2" cellpadding="2" align="center">';
	...
	if($mail_send_message != '') {
		$list .= '<tr><td colspan=2 class="dvtCellInfo"><br><br>'.getTranslatedString($mail_send_message).'</td></tr>';
	}	
	$list .= '</table></form>';
...
if($_REQUEST['mail_send_message'] != '')
{
	...
	if($mail_send_message[0] == 'true')
	{
		...
		$list = '<link rel="stylesheet" type="text/css" href="css/style.css">';
		$list .= '<table width="80%" border="0" cellspacing="2" cellpadding="2" align="center">';
		$list .= '<tr><td class="detailedViewHeader" nowrap colspan=2 align="center"><b>'.getTranslatedString('LBL_FORGOT_LOGIN').'</b></td></tr>';
		$list .= '<tr><td colspan=2 class="dvtCellInfo">&nbsp;</td></tr>';
		$list .= '<tr><td class="dvtCellInfo">'.getTranslatedString($mail_send_message[1]).'</td></tr>';
		$list .= '<br><tr><td>&nbsp;</td></tr>';

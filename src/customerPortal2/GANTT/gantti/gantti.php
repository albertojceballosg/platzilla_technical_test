...
class Gantti {
  ...
  function render() {
    ...
    // sidebar with labels
    $html[] = '<aside>';
    // $html[] = '<ul class="gantt-labels" style="margin-top: ' . (($this->options['cellheight']*2)+1) . 'px">';   // 2 renglones cuando tiene numeros de dias!!
    $html[] = '<ul class="gantt-labels" style="margin-top: ' . $this->options['cellheight'] . 'px">';
    ...
    foreach($this->months as $month) {
      $html[] = '<li class="gantt-month" style="width: ' . ($this->options['cellwidth'] * $month->countDays()) . 'px"><strong ' . $cellstyle . '>' . $month->name() . '-' . $month->yearINT . '</strong></li>';
    }
    $html[] = '</ul>';
    ...
    foreach($this->blocks as $i => $block) {
      ...
      // $left   = round($offset * $this->options['cellwidth']);  // comienza a la mitad del dia!
      // $width  = round($days * $this->options['cellwidth'] - 9);  // llega a la mitad del dia!
      $left   = round($offset * $this->options['cellwidth'] - 4);
      $width  = round($days * $this->options['cellwidth'] -1);
      $height = round($this->options['cellheight']-8);
      $class  = ($block['class']) ? ' ' . $block['class'] : '';
      // $html[] = '<span class="gantt-block' . $class . '" style="left: ' . $left . 'px; width: ' . $width . 'px; height: ' . $height . 'px"><strong class="gantt-block-label">' . $days . '</strong></span>';   // barra con la cantidad de dias
      $html[] = '<span class="gantt-block' . $class . '" style="left: ' . $left . 'px; width: ' . $width . 'px; height: ' . $height . 'px"></span>';
      $html[] = '</li>';
    ...
    if($this->options['today']) {
      ...
      if($today->timestamp > $this->first->month()->firstDay()->timestamp && $today->timestamp < $this->last->month()->lastDay()->timestamp) {
        $html[] = '<time style="top: ' . $this->options['cellheight'] . 'px; left: ' . $left . 'px" datetime="' . $today->format('Y-m-d') . '">Today</time>';
      }
